...
residualize_predictor <- function(W, rho, delta){
  ...
  W_res <- W
  for (i in 1:n){
    W_res_i <-subtr.predictor_hybrid(subset_predictor_hybrid(W, i), delta,  rho[i]) 
...
```{r}
delta <- get_delta(W,rho)
res <- residualize_predictor(W, rho, delta)
zero<-get_delta(res, rho)
```
