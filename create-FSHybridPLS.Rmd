...
# Example 1: All matrices are identical
mat1 <- matrix(c(1, 2, 3, 4), nrow = 2)
mat2 <- matrix(c(1, 2, 3, 4), nrow = 2)
mat3 <- matrix(c(1, 2, 3, 4), nrow = 2)
identical_list <- list(mat1, mat2, mat3)
print("Test Case 1: All matrices identical")
print(are_all_gram_matrices_identical(identical_list)) # Expected: TRUE

# Example 2: Matrices with slight floating-point differences (within tolerance)
mat4 <- matrix(c(1.000000000000001, 2, 3, 4), nrow = 2)
mat5 <- matrix(c(1, 2, 3, 4), nrow = 2)
tolerance_list <- list(mat4, mat5)
print("Test Case 2: Matrices with slight floating-point differences")
print(are_all_gram_matrices_identical(tolerance_list)) # Expected: TRUE

# Example 3: Matrices are not identical
mat6 <- matrix(c(1, 2, 3, 5), nrow = 2) # Different value
non_identical_list <- list(mat1, mat6, mat3)
print("Test Case 3: Matrices are not identical")
print(are_all_gram_matrices_identical(non_identical_list)) # Expected: FALSE

# Example 4: List with different dimensions
mat7 <- matrix(c(1, 2, 3, 4, 5, 6), nrow = 3)
diff_dim_list <- list(mat1, mat7)
print("Test Case 4: Matrices with different dimensions")
print(are_all_gram_matrices_identical(diff_dim_list)) # Expected: FALSE (all.equal will catch this)

# Example 5: Empty list
empty_list <- list()
print("Test Case 5: Empty list")
print(are_all_gram_matrices_identical(empty_list)) # Expected: TRUE

# Example 6: List with a single matrix
single_matrix_list <- list(mat1)
print("Test Case 6: Single matrix list")
print(are_all_gram_matrices_identical(single_matrix_list)) # Expected: TRUE

# Example 7: List containing non-matrix elements (will stop with an error)
# non_matrix_list <- list(mat1, "not a matrix", mat3)
# print("Test Case 7: List with non-matrix element (will throw error)")
# print(are_all_gram_matrices_identical(non_matrix_list))
```
...
```{r}
testthat::test_that("replace_obs_hybrid correctly replaces an observation", {
  # Setup: Create a multi-sample predictor_hybrid object
  n_samples <- 5
  n_scalar <- 2
  n_functional <- 3
  n_basis <- 5 # Assuming a fixed number of basis functions for simplicity

  # Basis object
  basis_obj <- create.bspline.basis(c(0, 1), nbasis = n_basis)

  # Generate sample functional data
  functional_list_initial <- lapply(1:n_functional, function(f_idx) {
    fd(matrix(rnorm(n_basis * n_samples), ncol = n_samples), basis_obj)
  })

  # Generate sample scalar data
  Z_initial <- matrix(rnorm(n_samples * n_scalar), nrow = n_samples, ncol = n_scalar)

  W_original <- predictor_hybrid(Z = Z_initial, functional_list = functional_list_initial)

  # Create a single-sample predictor_hybrid object for replacement
  new_Z_scalar <- matrix(c(100, 200), nrow = 1)
  new_functional_list_single <- lapply(1:n_functional, function(f_idx) {
    fd(matrix(c(rep(10, n_basis), rep(20, n_basis), rep(30, n_basis))[f_idx * n_basis - (n_basis - 1):0], ncol = 1), basis_obj)
  })
  new_W_single <- predictor_hybrid(Z = new_Z_scalar, functional_list = new_functional_list_single)

  # Test Case 1: Replace a middle observation (e.g., index 3)
  index_to_replace <- 3
  W_modified <- replace_obs_hybrid(W_original, index_to_replace, new_W_single)

  # Verify scalar part
  testthat::expect_equal(W_modified$Z[index_to_replace, ], new_W_single$Z[1, ],
               info = "Scalar part at replaced index should match new_W_single$Z")
  # Verify other scalar rows are unchanged
  if (index_to_replace > 1) {
    testthat::expect_equal(W_modified$Z[1:(index_to_replace - 1), ], W_original$Z[1:(index_to_replace - 1), ],
                 info = "Scalar part before replaced index should be unchanged")
  }
  if (index_to_replace < n_samples) {
    testthat::expect_equal(W_modified$Z[(index_to_replace + 1):n_samples, ], W_original$Z[(index_to_replace + 1):n_samples, ],
                 info = "Scalar part after replaced index should be unchanged")
  }

  # Verify functional part
  for (j in seq_len(n_functional)) {
    testthat::expect_equal(coef(W_modified$functional_list[[j]])[, index_to_replace],
                 coef(new_W_single$functional_list[[j]])[, 1],
                 info = paste0("Functional predictor ", j, " at replaced index should match new_W_single"))
    # Verify other functional columns are unchanged
    if (index_to_replace > 1) {
      testthat::expect_equal(coef(W_modified$functional_list[[j]])[, 1:(index_to_replace - 1)],
                   coef(W_original$functional_list[[j]])[, 1:(index_to_replace - 1)],
                   info = paste0("Functional predictor ", j, " before replaced index should be unchanged"))
    }
    if (index_to_replace < n_samples) {
      testthat::expect_equal(coef(W_modified$functional_list[[j]])[, (index_to_replace + 1):n_samples],
                   coef(W_original$functional_list[[j]])[, (index_to_replace + 1):n_samples],
                   info = paste0("Functional predictor ", j, " after replaced index should be unchanged"))
    }
  }

  # Test Case 2: Replacing the first observation (index 1)
  index_to_replace <- 1
  W_modified_first <- replace_obs_hybrid(W_original, index_to_replace, new_W_single)
  testthat::expect_equal(W_modified_first$Z[index_to_replace, ], new_W_single$Z[1, ])
  for (j in seq_len(n_functional)) {
    testthat::expect_equal(coef(W_modified_first$functional_list[[j]])[, index_to_replace],
                 coef(new_W_single$functional_list[[j]])[, 1])
  }

  # Test Case 3: Replacing the last observation (index n_samples)
  index_to_replace <- n_samples
  W_modified_last <- replace_obs_hybrid(W_original, index_to_replace, new_W_single)
  testthat::expect_equal(W_modified_last$Z[index_to_replace, ], new_W_single$Z[1, ])
  for (j in seq_len(n_functional)) {
    testthat::expect_equal(coef(W_modified_last$functional_list[[j]])[, index_to_replace],
                 coef(new_W_single$functional_list[[j]])[, 1])
  }

  # Test Case 4: Error handling for invalid index
  testthat::expect_error(replace_obs_hybrid(W_original, 0, new_W_single), "Index i must be between 1 and")
  testthat::expect_error(replace_obs_hybrid(W_original, n_samples + 1, new_W_single), "Index i must be between 1 and")

  # Test Case 5: Error handling for new_W with multiple samples
  multi_sample_new_W <- predictor_hybrid(Z = matrix(rnorm(2*n_scalar), nrow=2),
                                         functional_list = lapply(1:n_functional, function(f_idx) {
                                           fd(matrix(rnorm(n_basis*2), ncol = 2), basis_obj)
                                         }))
  testthat::expect_error(replace_obs_hybrid(W_original, 1, multi_sample_new_W), "new_W must be a single-sample predictor_hybrid object.")

  # Test Case 6: Error handling for mismatch in n_scalar
  mismatch_scalar_new_W <- predictor_hybrid(Z = matrix(rnorm(1*1), nrow=1),
                                            functional_list = new_functional_list_single)
  testthat::expect_error(replace_obs_hybrid(W_original, 1, mismatch_scalar_new_W), "Mismatch in number of scalar predictors.")

  # Test Case 7: Error handling for mismatch in n_functional
  mismatch_functional_new_W <- predictor_hybrid(Z = new_Z_scalar,
                                                functional_list = new_functional_list_single[1]) # Only one functional predictor
  testthat::expect_error(replace_obs_hybrid(W_original, 1, mismatch_functional_new_W), "Mismatch in number of functional predictors.")

  # Test Case 8: Error handling for mismatch in basis objects
  different_basis_obj <- create.bspline.basis(c(0, 1), nbasis = n_basis + 1) # Different basis
mismatch_basis_new_W_list <- lapply(1:n_functional, function(f_idx) {
  # Use the correct number of rows: nbasis of different_basis_obj
  fd(matrix(rnorm(different_basis_obj$nbasis), ncol = 1), different_basis_obj)
})
  mismatch_basis_new_W <- predictor_hybrid(Z = new_Z_scalar,
                                          functional_list = mismatch_basis_new_W_list)
  testthat::expect_error(replace_obs_hybrid(W_original, 1, mismatch_basis_new_W), "Functional predictors must have the same basis objects.")
...
output of the residualize_y and rho must be orthogonal.
```{r}
#sum(residualize_y(y, rho )*rho) #should output a very small number
```
...
```{r}
residualize_predictor <- function(W, delta){
...
####################################################
function(regcoef, pls_score) {
  for (ii in 1 : (regcoef@n_functional) ){
    regcoef@functional_list[[ii]] <- fd(
        coef = coef(regcoef@functional_list[[ii]]) %*% matrix(pls_score, nrow=1),
        basisobj = regcoef@functional_list[[ii]]$basis
      )}
  regcoef@Z <-  pls_score %*% regcoef@Z
  return(regcoef)
  }
####################################################
)
