...
```{r}
# Test script to showcase R's built-in broadcasting (recycling) rules.

# --- 1. Setup ---

# An N x P matrix (4 rows, 3 columns)
N <- 4
P <- 3
A <- matrix(
  1:12, 
  ...
  ncol = P, 
  byrow = TRUE,
  dimnames = list(paste0("Row", 1:N), paste0("Col", 1:P))
)

# CRITICAL CHANGE: Define the small operand as a VECTOR of length P.
B_vec <- c(100, 200, 300)

# A scalar multiplier
alpha <- 0.5

cat("--- R Broadcasting Demonstration (Vector Correction) ---\n\n")
...
cat("When B was a 1x3 matrix, R required 4x3 + 4x3. The error was correct for matrix addition.\n")
cat("When B is a vector, R uses the 'recycling rule' and repeats the vector row-wise.\n")


# --- 2. Demonstration: Simple Addition (A + B_vec) ---

# R automatically repeats B_vec 4 times across the rows of A. This works!
C_sum <- A + B_vec

cat("\n--- Result 1: A + B_vec (Native R Broadcasting Successful) ---\n")
cat("Expected: B_vec added to every row of A.\n")
print(C_sum)

# Verification Test: Check the first row
expected_row1 <- A[1, ] + B_vec
actual_row1 <- C_sum[1, ]
stopifnot(isTRUE(all.equal(actual_row1, expected_row1)))
cat("\nVerification: First row matches (1 + 100, 2 + 200, 3 + 300). SUCCESS.\n")


# --- 3. Demonstration: Addition with Scaling (A + alpha * B_vec) ---

# R performs the scalar multiplication first, then broadcasts the resulting vector.
C_scaled_sum <- add_broadcast(A, B_vec, alpha)
 
cat("\n--- Result 2: A + alpha * B_vec (Native R Broadcasting) ---\n")
cat("Expected: Scaled B_vec ([50, 100, 150]) added to every row of A.\n")
print(C_scaled_sum)

# Verification Test: Check the second row
expected_scaled_row2 <- A[2, ] + alpha * B_vec
actual_scaled_row2 <- C_scaled_sum[2, ]
stopifnot(isTRUE(all.equal(actual_scaled_row2, expected_scaled_row2)))
