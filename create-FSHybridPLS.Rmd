...
```{r}
#'Split Functional, Scalar, and Response Data
#' @param data_fun_1 fd object (fda package). Functional predictor 1.
#' @param data_fun_2 fd object (fda package). Functional predictor 2.
#' @param data_scalar Matrix. Scalar predictors.
#' @param response Vector. Response variable.
#' @param train_idx Vector. Indices for train data.
#' @return List containing split train and test sets.
#' @export
split.all <- function(data_fun_1, data_fun_2, data_scalar, response, train_idx) {
  # --- Type Checks ---
  stopifnot(
    # Functional data checks (assuming 'fd' class from the 'fda' package)
    inherits(data_fun_1, "fd"),
    inherits(data_fun_2, "fd"),
    
    # Scalar predictor check
    is.matrix(data_scalar),
    
    # Response variable check
    is.vector(response) || is.numeric(response) || is.factor(response),
    
    # Training index check
    is.vector(train_idx),
    is.numeric(train_idx),
    all(train_idx > 0), # Indices should be positive
    all(train_idx == round(train_idx)) # Indices should be integers
  )
  # -------------------
  
  return(list(
    data_fun_1_train = data_fun_1[train_idx],
    data_fun_1_test = data_fun_1[-train_idx],
    data_fun_2_train = data_fun_2[train_idx],
    data_fun_2_test = data_fun_2[-train_idx],
    data_scalar_train = data_scalar[train_idx, , drop = FALSE],
    data_scalar_test = data_scalar[-train_idx, , drop = FALSE],
    response_train = response[train_idx],
    response_test = response[-train_idx]
  ))
}
```
...
#'   \item{train}{The normalized training `predictor_hybrid` object.}
#'   \item{test}{The normalized testing `predictor_hybrid` object.}
#'   \item{deno_train}{A numeric vector of scaling factors used for each functional predictor.}
#' @export
curve_normalize_train_test <- function(train, test) {
  ...
  for (k in seq_len(train$n_functional)) { 
    ...
    mean_train <- fda::mean.fd(train_fd) 
    ...
    test_normalized$functional_list[[k]] <-
      curve_normalize(
        ...
        mean_train, # Centered by training mean
...
library(fda) # Required for mock functional data

# Placeholder for a minimal predictor_hybrid constructor for testing purposes
...
# Create Functional Data (Needs to be present but remain untouched)
basis_obj <- create.bspline.basis(rangeval = c(0, 10), nbasis = 7)
train_coefs <- matrix(rnorm(N_train * 7), nrow = 7)
test_coefs <- matrix(rnorm(N_test * 7), nrow = 7)
mock_fd_train <- fd(train_coefs, basis_obj)
mock_fd_test <- fd(test_coefs, basis_obj)

# Create Scalar Data (Crucial for the test)
# Training data has a clear mean and SD
Z_train <- matrix(rnorm(N_train * N_scalar, mean = c(5, 10), sd = c(2, 3)), 
                  nrow = N_train, ncol = N_scalar, byrow = TRUE)
# Testing data has different properties (must be normalized by train stats)
Z_test <- matrix(rnorm(N_test * N_scalar, mean = c(1, 1), sd = c(10, 10)), 
                 nrow = N_test, ncol = N_scalar, byrow = TRUE)

# Create Hybrid Objects
W_train <- predictor_hybrid(Z = Z_train, functional_list = list(mock_fd_train))
W_test <- predictor_hybrid(Z = Z_test, functional_list = list(mock_fd_test))

# Store original Z for comparison
Z_train_original <- W_train$Z
Z_test_original <- W_test$Z

# 2. Run the Function Under Test
W_normalized_list <- scalar_normalize_train_test(W_train, W_test)
W_train_norm <- W_normalized_list$train
W_test_norm <- W_normalized_list$test

# 3. Verification and Assertions

cat("--- Running Verification Checks ---\n")

# Check 1: Training set is correctly normalized (Mean ~ 0, SD ~ 1)
train_mean_norm <- colMeans(W_train_norm$Z)
train_sd_norm <- apply(W_train_norm$Z, 2, sd)

stopifnot(
  "Train mean Col 1 not near 0" = all(abs(train_mean_norm[1]) < 1e-10),
  "Train mean Col 2 not near 0" = all(abs(train_mean_norm[2]) < 1e-10),
  "Train SD Col 1 not near 1" = all(abs(train_sd_norm[1] - 1) < 1e-10),
  "Train SD Col 2 not near 1" = all(abs(train_sd_norm[2] - 1) < 1e-10)
)
cat("Check 1: Training set Z-score normalization successful (Mean~0, SD~1).\n")


# Check 2: Functional components are untouched
stopifnot(
  "Functional list should not change" = isTRUE(all.equal(
    W_train$functional_list[[1]]$coefs, 
    W_train_norm$functional_list[[1]]$coefs
  ))
)
cat("Check 2: Functional components successfully preserved.\n")


# Check 3: Test set is normalized using TRAIN statistics (crucial consistency check)

# Recalculate the training statistics
train_mean_recalc <- get_mean(Z_train_original)
train_sd_recalc <- get_sd(Z_train_original)

# Manually normalize the test set using TRAIN stats
Z_test_expected <- t( (t(Z_test_original) - train_mean_recalc) / train_sd_recalc)

# Compare the function's output (W_test_norm$Z) against the manual calculation
stopifnot(
  "Test set normalization mismatch" = isTRUE(all.equal(W_test_norm$Z, Z_test_expected))
)
cat("Check 3: Test set correctly normalized using only Training statistics.\n")
...
#
#   omega <- 0
#   for (k in 1:train@n_functional){ # for kth functional predictor
#     omega <- omega + sum(diag(inprod(
...
#
#     train_normalized@Z <- sqrt(omega) * (train@Z)
#     test_normalized@Z <- sqrt(omega) * (test@Z)
#
#     return(
#       list(
#         train = train_normalized,
...
R6 class to split functional and scalar predictors into train and test sets and apply normalization techniques including curve normalization, scalar normalization, and between-functional-scalar normalization.
```{r}
#' @title Dataset Splitting and Normalization
#' @description 
DataSplitterNormalizer <- R6::R6Class(
  "DataSplitterNormalizer",
  ...
  public = list(
    ...
    #' @description Creates train/test indices and splits functional, scalar, and response variables.
    #' @return None. Updates train and test variables.
    #' @export
    split_data = function(idx_train = NULL) {
      # Use provided idx_train if available; otherwise, generate it
      if (is.null(idx_train)) {
        self$idx_train <- get_idx_train(self$n_sample, self$train_ratio)
      } else {
        self$idx_train <- idx_train
      }


      split_packed <- split.all(
        self$data_original$functional_1,
        self$data_original$functional_2,
        self$data_original$scalar,
        self$data_original$response,
        self$idx_train
      )

      # Assign train/test sets
      self$train <- list(
        functional_1 = split_packed$data_fun_1_train,
        functional_2 = split_packed$data_fun_2_train,
        scalar = split_packed$data_scalar_train,
        response = split_packed$response_train
      )
      self$test <- list(
        functional_1 = split_packed$data_fun_1_test,
        functional_2 = split_packed$data_fun_2_test,
        scalar = split_packed$data_scalar_test,
        response = split_packed$response_test
      )


    },

    #' @title Normalize Train and Test Data
    #' @description Applies curve, scalar, and between-functional-scalar normalization.
    #' @return None. Updates train and test sets.
    #' @export
    normalize_data = function() {
      # Initialize normalizers
      normalizer_functional <- normalizer_functional_predictor$new()
      normalizer_scalar <- normalizer_scalar_predictor$new()

      # Normalize first functional predictor
      normalizer_functional$normalize(self$train$functional_1, self$test$functional_1)
      self$train$functional_1 <- normalizer_functional$get_normalized()$train_normalized
      self$test$functional_1 <- normalizer_functional$get_normalized()$test_normalized

      # Normalize second functional predictor
      normalizer_functional$normalize(self$train$functional_2, self$test$functional_2)
      self$train$functional_2 <- normalizer_functional$get_normalized()$train_normalized
      self$test$functional_2 <- normalizer_functional$get_normalized()$test_normalized

      # Normalize scalar predictors
      normalizer_scalar$normalize(self$train$scalar, self$test$scalar)
      self$train$scalar <- normalizer_scalar$get_normalized()$train_normalized
      self$test$scalar<- normalizer_scalar$get_normalized()$test_normalized

      # Apply between-functional-scalar normalization
      self$normalize_between()

      # Normalize response variable
      self$normalize_response()
    },

    #' @title Normalize Between Functional and Scalar Predictors
    #' @description Scales scalar predictors to ensure comparability with functional predictors.
    #' @export
    normalize_between = function() {
      omega <- sum(diag(inprod(self$train$functional_1, self$train$functional_1))) +
        sum(diag(inprod(self$train$functional_2, self$train$functional_2)))

      omega <- omega / sum(self$train$scalar^2)

      self$train$scalar <- sqrt(omega) * self$train$scalar
      self$test$scalar <- sqrt(omega) * self$test$scalar
    },

    #' @title Normalize Response Variable
    #' @description Centers response variable based on training mean.
    #' @export
    normalize_response = function() {
      response_mean_train <- mean(self$train$response)
      self$train$response <- self$train$response - response_mean_train
      self$test$response <- self$test$response - response_mean_train
    } #must not end with ,
