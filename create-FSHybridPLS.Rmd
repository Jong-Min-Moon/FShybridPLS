...
```{r}
get_xi_direct <- function(W, y) {
  K <- W$n_functional
  C_J <- get_CJ(W)
  A <- cbind(C_J, W$Z)                      # shape: n × d
  b <- t(y) %*% A / W$n_sample   # equivalent to V_star

  J_star <- get_gram_matrix_block(W)
  alpha <- solve(J_star, t(b))                # unconstrained maximizer
  xi_hat <- alpha / sqrt(t(alpha) %*% J_star %*% alpha)  # normalize
...
```{r}
  ...
  J_star <- get_gram_matrix_block(W)
  alpha <- solve(J_star, t(b))                # unconstrained maximizer
  xi_hat <- alpha / sqrt(sum((alpha) * (J_star %*% alpha) )) # normalize
t(y) %*% A %*% xi_hat
```
...
cat("\n✅ Successful runs with real eigenvalues:\n")
print(subset(results, is_real == TRUE))

# --- Fastest and slowest ---
cat("\n⏱️ Top 5 fastest settings:\n")
print(head(results[order(results$time_sec), ], 5))

cat("\n🐢 Top 5 slowest settings:\n")
...
ggplot(results, aes(x = nbasis, y = lambda, fill = reason)) +
  ...
  labs(
    title = "get_pls_comp Outcomes: λ vs. # Basis Functions",
    x = "# of Basis Functions",
    y = "Smoothing Parameter λ (log scale)",
    fill = "Result Type"
  )
...
get_pls_comp_simple <- function(W, y) {
  ...
  xi_sum <- scalar_mul.predictor_hybrid(subset_predictor_hybrid(W, 1), y_centered[1])
  ...
  # Normalize
  xi_sum <- scalar_mul.predictor_hybrid(xi_sum, 1/n)
  norm_val <- sqrt(inprod.predictor_hybrid(xi_sum))
  xi_hat <- scalar_mul.predictor_hybrid(xi_sum, 1 / norm_val)
...
# --- Step 1: Zero functional predictors ---
zero_basis <- create.bspline.basis(c(0, 1), nbasis = 4)
zero_fd <- fd(coef = matrix(0, nrow = 4, ncol = n_sample), basisobj = zero_basis)
functional_list <- list(zero_fd, zero_fd)  # two zero-valued functional predictors

# --- Step 2: Simulate scalar predictors and response ---
Z <- matrix(rnorm(n_sample * n_scalar), ncol = n_scalar)
Z <- scale(Z, center = TRUE, scale = FALSE)
W <- predictor_hybrid(Z = Z, functional_list = functional_list)

beta_true <- rnorm(n_scalar)
y <- Z %*% beta_true + rnorm(n_sample, sd = 0.1)
y <- scale(y, center = TRUE, scale = FALSE)

# --- Step 3: Compute constraint matrix ---
lambda <- rep(0, W$n_functional)
L <- chol(get_constraint_matrix_hybrid(W, lambda))

# --- Step 4: PLS with get_pls_comp ---
pls <- get_pls_comp(W, y, L)
xi_hat <- pls$xi_hat

# --- Step 5: PLS via raw Euclidean eigenproblem ---
V_star_vec <- as.vector(t(Z) %*% y)     # equivalent to colSums(Z * y)

J <- crossprod(Z)                       # J = Z^T Z
L_Z <- chol(J)
invL <- solve(L_Z)
E <- invL %*% tcrossprod(V_star_vec) %*% t(invL)

eig <- eigen(E)
e <- eig$vectors[, 1]
xi_manual <- backsolve(L_Z, e, transpose = TRUE)  # xi = L^T \ e

# --- Step 6: get_pls_comp_simple ---
xi_simple <- get_pls_comp_simple(W, y)
xi_simple_vec <- tail(xi_simple$Z, 1)
xi_direct <- get_xi_direct(W, y)
# --- Step 7: Compare all three scalar parts ---

# From get_pls_comp
xi_hat_vec <- tail(xi_hat$Z, 1)

# Cosine similarities
cos_eig_manual <- sum(xi_manual * xi_hat_vec) / (sqrt(sum(xi_manual^2)) * sqrt(sum(xi_hat_vec^2)))
cos_simple_manual <- sum(xi_manual * xi_simple_vec) / (sqrt(sum(xi_manual^2)) * sqrt(sum(xi_simple_vec^2)))
cos_eig_simple <- sum(xi_simple_vec * xi_hat_vec) / (sqrt(sum(xi_simple_vec^2)) * sqrt(sum(xi_hat_vec^2)))

# --- Step 8: Report ---
cat("Cosine similarity (get_pls_comp vs manual eig):", cos_eig_manual, "\n")
cat("Cosine similarity (get_pls_comp_simple vs manual eig):", cos_simple_manual, "\n")
cat("Cosine similarity (get_pls_comp vs get_pls_comp_simple):", cos_eig_simple, "\n")
...
# --- Step 6 (Revised): Manually compute direct PLS component and wrap with predictor_hybrid_from_coef ---

# Scalar part

# Functional part: stack the weighted coefficients
func_weighted_vec <- unlist(lapply(functional_list, function(fd_j) {
  coef(fd_j) %*% y  # nbasis × 1
}))

# Full coefficient vector
Z_weighted <- colSums(Z * y)

xi_raw_vec <- c(func_weighted_vec, Z_weighted)

# Wrap into predictor_hybrid object using format template
xi_hat_direct <- predictor_hybrid_from_coef(format = W_template, coef = xi_raw_vec)

# Normalize
norm_val <- sqrt(inprod.predictor_hybrid(xi_hat_direct))
xi_hat_direct <- scalar_mul.predictor_hybrid(xi_hat_direct,  1 / norm_val)

xi_new <- get_xi_direct(W, y)
