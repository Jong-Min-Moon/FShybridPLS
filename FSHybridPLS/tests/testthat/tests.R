...
testthat::test_that("get_pls_comp runs and returns valid output with penalty-based L", {
  suppressPackageStartupMessages(library(fda))
  set.seed(123)
  ...
  for (nbasis in c(3, 4, 5)) {
    basis <- create.bspline.basis(c(0, 1), nbasis = nbasis)

    # Functional predictors
    fd1 <- make_fd(matrix(rnorm(nbasis * n_sample), nbasis, n_sample), basis)
    fd2 <- make_fd(matrix(rnorm(nbasis * n_sample), nbasis, n_sample), basis)
    functional_list <- list(fd1, fd2)

    # Scalar predictors and response
    Z <- matrix(rnorm(n_sample * n_scalar), n_sample, n_scalar)
    y <- rnorm(n_sample)
    ...
    lambda <- rep(1e-2, n_functional)
    L <- get_constraint_matrix_hybrid(W, lambda)

    # Run and validate
    result <- get_pls_comp(W, y, L)

    # Structure check
    expect_type(result, "list")
    expect_named(result, c("xi_hat", "E", "V_star", "eigen_val"))

    # Component checks
    expect_s3_class(result$xi_hat, "predictor_hybrid")
    expect_true(is.matrix(result$E))
    expect_true(is.matrix(result$V_star))
    expect_true(is.numeric(result$eigen_val))
    expect_length(result$eigen_val, 1)
    expect_false(is.complex(result$eigen_val))
  }
})
