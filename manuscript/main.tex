...
%% Addresses                                %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\address[A]{University of Southern California\printead[presep={,\ }]{e1}}

\address[B]{University of Texas Medical Branch\printead[presep={,\ }]{e2}}
\end{aug}
...
As an illustrative example, the Emory University renal study \citep{changBayesianLatentClass2020, jangPrincipalComponentAnalysis2021}  records both multiple renogram curves (functional data) and multiple renogram variables (vector data)  for each kidney. These modalities provide distinct yet related physiological signals. To effectively analyze such heterogeneous data, we need statistical methods capable of jointly modeling functional and vector-valued covariates. This approach accounts for both shared and modality-specific variations linked to clinical outcomes. Our work focuses on developing a method to fit the following hybrid functional linear regression model:
\begin{equation}	\label{eq: functional model}
	...
	\sum 
	\limits_{j=1}^K 
	\int  \beta_j(t) X_{ij}(t) dt + \epsilon_i,
	\quad
	i = 1, \ldots, n,
	%= \langle \boldsymbol{\alpha}, \mathbf{Z} \rangle +  \langle \beta, X \rangle_{\mathcal{F}}  + \epsilon
\end{equation}
where $Y$ is a scalar response, $\mathbf{Z}$ is the Euclidean vector covariate and 
$X_{i1}(t), \ldots, X_{iK}(t)$ are functional predicters.
...
\subsubsection{Iterative step 1: Regularized estimation of smoothed PLS component direction}  \label{section:sub:compute_PLS_component} 
...
$
and $\hat{\boldsymbol{\zeta}}$ is the scalar part. This maximizer can also be represented as a concatenated Euclidean vector
\begin{equation}\label{def:maximizer_euclidean_form}
	( \hat{\boldsymbol{\gamma}}_1^\top, \ldots, \hat{\boldsymbol{\gamma}}_K^\top, \hat{\boldsymbol{\zeta}}^\top )^\top
	\in \mathbb{R}^{MK+p},
\end{equation}
...
\begin{proposition}\label{proposition:eigen_noregul}
	...
	\begin{equation}	\label{eq: Unregularized generalized rayleigh quotient equation}
		\left(
		\hat{\boldsymbol{\gamma}}_1^\top, \ldots, \hat{\boldsymbol{\gamma}}_K^\top, \hat{\boldsymbol{\zeta}}^\top
		\right)^\top
		=
...
\begin{proposition}\label{proposition:eigen_regul}
	...
	\begin{equation}	\label{eq: Regularized generalized rayleigh quotient equation}
		...
		%
		% constraint
		\boldsymbol{\xi}^\top (\mathbb{B} + \Lambda \mathbb{B}^{\prime \prime}) \boldsymbol{\xi} = 1.
	\end{equation}
...
Using these PLS scores as scalar predictor observations, the $(l+1)$-th iteration's residualized response is computed as:
\begin{equation}\label{algorithm_step:residualization_response}
	Y_i^{[l+1]} := Y_i^{[l]} - 
	\frac{
		( \mathbf{y}^{[l]} )^\top  \hat{\boldsymbol{\rho}}^{[l]}
	}{
		( \hat{\boldsymbol{\rho}}^{[l]} )^\top  \hat{\boldsymbol{\rho}}^{[l]}
	}
	\widehat{\rho}_{il},
\end{equation}
which is equivalent to using the least squares estimate from regressing $Y_i^{[l]}$ on $\widehat{\rho}_{il}$,
a procedure identical to that found in the naive PLS algorithm (Section \ref{sec: Naive PLS Algorithm}). 
Since $Y_i^{[l]}$ are assumed to have a sample mean of zero, these residuals $Y_i^{[l+1]}$   also have a sample mean of zero.

For predictor residualization, we fit a hybrid-on-scalar regression model where the predictor is the PLS score for each observation. Specifically, we fit $\widetilde{W}^{[l]}_i = \hat{\rho}^{[l]}_i\, \delta^{[l]} + E_i^{[l]}$, and then take the residuals as the predictor for the next iteration.
Here, $\delta^{[l]}$ and $E_i^{[l]}$ are the regression coefficient and error terms, respectively.
Performing this pointwisely, as in Section \ref{sec: Naive PLS Algorithm}, is highly inefficient and potentially infeasible for dense or irregular functional data. Moreover, the results may lack smoothness, introducing instability that propagates to subsequent results and compromises the entire algorithm.
Therefore, we employ a basis expansion approach to fit the entire hybrid object in one step. We assume both $\delta^{[l]}$ and $E_i^{[l]}$ are elements of $\tilde{\mathcal{H}}$, as defined in \eqref{def:truncated_hilbert_space}. To ensure the stability of the entire PLS estimation procedure, the estimated regression coefficient $\hat{\delta}^{[l]} = (\hat{\delta}^{[l]}_1, \ldots, \hat{\delta}^{[l]}_K, \hat{\boldsymbol{\chi}}^{[l]} ) \in \tilde{\mathcal{H}}$ should have smooth functional parts, consistent with Section \ref{sec: Regularization of PLS Components}. Consequently, $\delta^{[l]}$ is obtained by minimizing a penalized least squares criterion, where the functional part is penalized by the integrated squared second derivative defined in \eqref{def:roughness_penalty}:
\begin{equation}\label{def:argmin_pensse}
	\hat{\delta}^{[l]} := \arg \min_{\delta \in \tilde{\mathcal{H}}} \operatorname{PENSSE}( \delta),
\end{equation}
where
...
\end{equation}
where $\tau>0$ is a  smoothing parameter controlling the trade-off between fit accuracy and functional regression coefficient smoothness.
The following proposition shows that the solution of the problem \ref{def:argmin_pensse} can be obtained by solving linear equations.
...
The $(l+1)$-th iteration's residualized predictor is then computed as:
\begin{equation}\label{algorithm_step:residualization_predictor}
	\widetilde{W}_i^{[l+1]} 
	= 
	\widetilde{W}_i^{[l]}  - \widehat{\rho}_{il}  \widehat{ {\delta}}_{l}.
	%%%%%%%%%%%
\end{equation}
Since $\widetilde{W}_i^{[l]} $ are assumed to have a sample mean of zero, these residuals $\widetilde{W}_i^{[l+1]}$   also have a sample mean of zero.
...
\subsection{Geometric Properties}\label{section:sub:prop_geom}\jongmin{currently revising}
...
\begin{definition}[Roughness-sensitive inner product]
		Given two hybrid predictors 	$W_1 = (X_{11}, \ldots, X_{1K}, \mathbf{Z}_1)$
	and
	$W_2 = (X_{21}, \ldots, X_{2K}, \mathbf{Z}_2)$ belonging to $\mathcal{H}$ defined in Definition \ref{def:hybid_predictor}
	and roughness penalty matrix 	$\Lambda  = \operatorname{blkdiag}(\lambda_1 I_M, \ldots, \lambda_K I_M, 0_{p \times p})$,
	The inner product between two hybrid predictors, taking into the roughness peanlties,  is then defined as:
\begin{equation}
	\langle \langle h_l, h_j \rangle \rangle = \langle h_l, h_j \rangle_{\mathcal{H}} + \sum_{k=1}^K \lambda_k \langle \ddot{f}_l^{(k)}, \ddot{f}_j^{(k)} \rangle_{L^2}
\end{equation}
	\begin{equation}		\label{eq: hybrid inner product}
	\langle W_1, W_2\rangle_{\mathcal{H}, \Lambda}
	:=
	\sum \limits_{k=1}^K \int_0^1 X_{1k}(t) X_{2k}(t) \, dt
	\sum \limits_{k=1}^K \int_0^1 X^{\prime \prime}_{1k}(t) X^{\prime \prime}_{2k}(t) \, dt
	+ 
	\omega
	\mathbf{Z}_1^\top \mathbf{Z}_2,
\end{equation}
...
\end{definition}
  ...
  ($\widehat{\xi}_1, \widehat{\xi}_2, \ldots$)  based on the modified inner product  which now takes into account the roughness of the functions:
\begin{align*}
	...
	& = \mathbb{1}(l=j). 
\end{align*}
This modified orthonormality condition is one of the main geometric properties of the proposed PLS algorithm and is formally stated with proof in Section \ref{section:main:properties_of_our_method} (see Proposition \ref{proposition: modified orthnormality of PLS components}), along with other properties. 
...
where
$\hat{\rho}_1$ is defined in \eqref{def:plsscore},
