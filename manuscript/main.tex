...
\end{corollary}
Let $\beta_{\mathrm{OLS}}$ denote a minimizer of  the mean squared error $ \mathbb{E}[\|Y - \langle \beta, W \rangle_{\mathbb{H}}\|_2^2]$. It
satisfies the normal equation
\begin{equation}\label{normal_equation}
	\mathbb{E}[(Y - \langle \beta_{\mathrm{OLS}}, W \rangle_{\mathbb{H}})W] = 0 \in \mathbb{H}.
\end{equation}
If the operator $\mathcal{T}^\ast \mathcal{T}$ is invertible,  the minimizer is uniquely determined as 
$\beta_{\mathrm{OLS}} = (\mathcal{T}^\ast \mathcal{T})^{-1} \mathcal{T}^\ast Y$.
