...
\rangle_{\mathcal{H}}
-
\frac{\widehat{{\rho}}_i^{[l_1-1]}}{\| \widehat{\boldsymbol{\rho}}^{[l_1-1]} \|_2^2}
\widehat{\boldsymbol{\rho}}^{[l_1-1] \top}
(
\sum_{k=1}^K {\Theta}_{k}^{[l_1-1]} 
B \boldsymbol{\gamma}_k^{[l_2]}
+
Z^{[l_1-1]} 
\boldsymbol{\zeta}^{[l_2]}
)
...
\end{align*}
where 
step $(i)$ uses the computation of the $i$th entry in \eqref{score_inner_product_all_n},
step $(ii)$ uses $
	\widetilde{W}_i^{[l_1]}= \widetilde{W}_i^{[l_1-1]} - \widehat{{\rho}}_i^{[l_1-1]} \widehat{\delta}^{[l_1-1]}
$
 and step $(iii)$ uses 
 the closed-form formula of $\widehat{\delta}^{[l_1-1]}$ given by Lemma \ref{proposition:closed_form_orthgonalization}.
	Therefore, we have
\begin{equation*}
\Theta^{[l_1] }
\mathbb{B}
...
\rangle_{\mathcal{H}}
-
\frac{1}{\| \widehat{\boldsymbol{\rho}}^{[l_1-1]} \|_2^2}
\widehat{\boldsymbol{\rho}}^{[l_1-1] \top}
\langle
\widetilde{W}^{[l_1-1]}  , 
\hat{\xi}^{[l_2]}
\rangle_{\mathcal{H}}
\widehat{\boldsymbol{\rho}}^{[l_1-1]}
\end{equation*}
	\begin{equation*}
	\hat{\boldsymbol{\pi}}_j^{[l]}
	=
...
\end{align*}
noting that $\int_{\mathcal{T}} \widetilde{W}^{[j]}[\mathbf{t}] \widehat{\xi}_j[\mathbf{t}] d\mathbf{t} = \widehat{\boldsymbol{\rho}}_j$. Now by the , we have that 
