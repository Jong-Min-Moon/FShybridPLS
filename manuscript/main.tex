...
\end{equation}
 An alternative notation for the hybrid object can be obtained by evaluating its functional part on $\mathbf{t}$ and expressing it as a $(K+p)$-dimensional vector: $\mathbf{W}[\mathbf{t}] = (X(\mathbf{t}), \mathbf{Z})^\top$, with $ X(\mathbf{t}) = (X^{(1)}(t_1), \ldots, X^{(K)}(t_K))^\top \in \mathbb{R}^K$ and $ \mathbf{Z} = (Z_1, \ldots, Z_p)^\top  \in \mathbb{R}^p$.  We define the inner product between any two hybrid objects, $\mathbf{h}_1 = (f_1, \mathbf{v}_1)$ and $\mathbf{h}_2 = (f_2, \mathbf{v}_2)$, as
\begin{equation}
    \langle \mathbf{h}_1, \mathbf{h}_2\rangle_{\mathcal{H}} 
    = 
    \langle f_1, f_2\rangle_\mathcal{F} 
    +
    \omega \langle  \mathbf{v_1}, \mathbf{v_2} \rangle = \sum \limits_{k=1}^K \int_{\mathcal{T}_k} f_1^{(k)}(t_k) f_2^{(k)}(t_k) dt_k
    +
...
\end{equation}
where $(\boldsymbol{\delta}_l)_{l \in \mathbb{N}} \in \mathcal{H}$ and $(\nu_l)_{l \in \mathbb{N}} \in \mathbb{R}$ are appropriate bases. The PLS scores are obtained as $\rho_l =  \langle \mathbf{W}^{[l]}, \boldsymbol{\xi}_l \rangle_{\mathcal{H}}$, where $\mathbf{W}^{[l]}$ denotes the residualized predictor sequentially derived as the residual of the regression of $\mathbf{W}^{[l-1]}$ on $\rho_{l-1}$ with $\mathbf{W}^{[1]} = \mathbf{W}$, and $\boldsymbol{\xi}_l=(\psi_l, \boldsymbol{\theta}_l) \in \mathcal{H}$ is the hybrid PLS component, with $\psi_l = (\psi^{(1)}_l, \ldots, \psi_l^{(K)}) \in \mathcal{F}$ and $\boldsymbol{\theta}_l=(\theta_{l1}, \ldots, \theta_{lp})^\top \in \mathbb{R}^p$, sequentially chosen to maximize the squared covariance between $\rho_l$ and the residualized outcome $Y^{[l]}$---i.e., $\textnormal{Cov}^2(\rho_l, Y^{[l]}) = \textnormal{Cov}^2(\langle \mathbf{W}^{[l]}, \boldsymbol{\xi}_l \rangle_\mathcal{H}, Y^{[l]})$. Here, $Y^{[l]}$ is also sequentially obtained as the residual of the regression of $Y^{[l-1]}$ on $\rho_{l-1}$ with $Y^{[1]} = Y$. The sequentially derived hybrid PLS components \{$\boldsymbol{\xi}_l\}_{l=1}^\infty$, are orthonormal in a sense that $\langle \boldsymbol{\xi}_l, \boldsymbol{\xi}_j \rangle_\mathcal{H} = \mathbb{1}(l=j)$.
...
\State 
 $
 \widehat{\boldsymbol{\xi}}_{l}[\mathbf{t}] 
 \leftarrow 
...
\bigl\{
\langle \mathbf{W}_{1}^{[l]}, \widehat{\boldsymbol{\xi}}_{l} \rangle_{\mathcal{H}},
\ldots,
\langle \mathbf{W}_{n}^{[l]}, \widehat{\boldsymbol{\xi}}_{l} \rangle_{\mathcal{H}}
\bigr\}
$
...
\begin{equation}\label{def:pls_score_naive}
    \widehat{\rho}_{il} = \langle \mathbf{W}_{i}^{[l]}, \widehat{\boldsymbol{\xi}}_{l} \rangle_{\mathcal{H}} = \langle X_i^{[l]}, \widehat{\psi}_{l} \rangle_{\mathcal{F}} + \langle \mathbf{Z}_i^{[l]}, \widehat{\boldsymbol{\theta}}_{l} \rangle, 
\end{equation}
where the $l$-th PLS loading $\widehat{\boldsymbol{\xi}}_{l} = (\widehat{\psi}_{l}, \widehat{\boldsymbol{\theta}}_{l}) \in \mathcal{H}$ maximizes $\widehat{\textnormal{Cov}}^2(\langle \mathbf{W}_{i}^{[l]}, \widehat{\boldsymbol{\xi}}_{l} \rangle_{\mathcal{H}}, Y_i^{[l]})=\widehat{\textnormal{Cov}}^2(\widehat{\rho_{il}}, Y_i^{[l]})$. Specifically, we define the $l$-th PLS loadings pointwise as follows:
\begin{equation}\label{def:pls_loading_ptws}
    \widehat{\boldsymbol{\xi}}_{l}[\mathbf{t}] = \frac{\sum_{i=1}^n Y_i^{[l]} \mathbf{W}_i^{[l]}[\mathbf{t}]}{\Vert \sum_{i=1}^n Y_i^{[l]} \mathbf{W}_i^{[l]} \Vert_{\mathcal{H}}} = \left[ \frac{\sum_{i=1}^n Y_i^{[l]} X_i^{[l]}(\mathbf{t})}{\Vert \sum_{i=1}^n Y_i^{[l]} \mathbf{W}_i^{[l]} \Vert_{\mathcal{H}}}, \frac{\sum_{i=1}^n Y_i^{[l]}\mathbf{Z}_i^{[l]}}{\Vert \sum_{i=1}^n Y_i^{[l]} \mathbf{W}_i^{[l]} \Vert_{\mathcal{H}}} \right]^\top,
\end{equation}
...
For $k = 1, \ldots, K$, let us denote the system of $M$ basis functions used to approximate the $k$-th functional predictor as 
\begin{equation*}
 \mathbf{b}^{(k)}(t_k)=(b^{(k)}_{1}(t_k), \ldots, b^{(k)}_{M}(t_k))^\top,~\text{where}~t_k \in \mathcal{T}_k.   
\end{equation*}
In our proposed hybrid PLS regression algorithm, this basis system is used to approximate both the predictors and the PLS components. It also serves as a regularization tool to borrow strength and prevent overfitting.

\paragraph{Basis approximation of the hybrid predictor.}
...
\end{equation*}
where $\mathbf{c}_i^{(k)} = (c_{i1}^{(k)}, \ldots, c_{iM}^{(k)})^\top$ is a $M$-dimensional vector of basis coefficients corresponding to the $k$th functional predictor. This basis approximation can be expressed collectively across $n$ observations as: $\widetilde{\mathbf{X}^{(k)}}(t_k) = \widetilde{C}^{(k)} \mathbf{b}^{(k)}(t_k)$, where $\widetilde{\mathbf{X}}^{(k)}(t_k)=(X_1^{(k)}(t_k), \ldots, X_n^{(k)}(t_k))^\top \in \mathbb{R}^n$, and $\widetilde{C}^{(k)} = (\mathbf{c}_1^{(k)} \cdots \mathbf{c}_n^{(k)})^\top \in \mathbb{R}^{n \times M}$. This notation can be further extended to simultaneously express the basis expansions of $K$ functional predictors as 
\begin{equation}
    \widetilde{X}(\mathbf{t}) = \widetilde{C}B(\mathbf{t}), \quad t \in \mathcal{T},
    \label{eq: basis approximation of X}
\end{equation}
where $\widetilde{X}(\mathbf{t}) = [\widetilde{\mathbf{X}}^{(1)}(t_1), \cdots, \widetilde{\mathbf{X}}^{(K)}(t_K)] \in \mathbb{R}^{n \times K}$, $\widetilde{C} = [\widetilde{C}^{(1)}, \cdots, \widetilde{C}^{(K)}] \in \mathbb{R}^{n \times MK}$, and 
    $B(\mathbf{t}) = \mathrm{blkdiag}[\mathbf{b}^{(1)}(t_1), \cdots, \mathbf{b}^{(K)}(t_K)] \in \mathbb{R}^{MK \times K}$.
 The number of basis functions $M$ for each functional predictor can be set to a sufficiently large value (e.g., 15 or 20) to incorporate a rich variety of patterns that functional predictors may exhibit. Note that there is no need to carefully choose different $M$ values for different functional predictors, as an appropriate penalization scheme will be introduced in the later section to regularize each functional predictor to achieve a desired smoothness (see Section \ref{sec: Regularization of PLS Components}).



Furthermore, let $\widetilde{Z} = [\mathbf{Z}_1, \cdots, \mathbf{Z}_n] \in \mathbb{R}^{n \times p}$ denote $n \times p$ matrix of the scalar predictors (observed version when $l=1$, or residualized version when $l \ge 2$), and let $\widetilde{W}[\mathbf{t}]=[\widetilde{X}(\mathbf{t}), \widetilde{Z}]$ denote $n \times (K+p)$ hybrid predictor matrix that stacks $\widetilde{X}(\mathbf{t})$ and $\widetilde{Z}$ as columns. Then we can also write the hybrid predictor, whose functional part is approximated using the basis functions, as a form of linear transformation similar to \eqref{eq: basis approximation of X}:
\begin{equation}\label{eq: basis approximation of W}
    \widetilde{W}[\mathbf{t}] =  \widetilde{C}^* B^*[\mathbf{t}], \quad t \in \mathcal{T}.
\end{equation}
where  $B^*[\mathbf{t}] = \mathrm{blkdiag}(B(\mathbf{t}), I_p) \in \mathbb{R}^{(MK+p) \times (K+p)}$ and $\widetilde{C}^*=[\widetilde{C}, \widetilde{Z}] \in \mathbb{R}^{n \times (MK+p)}$.

\paragraph{Basis approximation of the PLS component.}
We can also approximate the functional part of the PLS component in terms of the same basis functions: $\psi^{(k)}(t_k) = \sum_{m=1}^M a^{(k)}_{m} b^{(k)}_m(t_k) = \mathbf{a}^{(k)\top} \mathbf{b}^{(k)}(t_k)$ and $\psi(\mathbf{t}) = B(\mathbf{t})^\top \mathbf{a} \in \mathbb{R}^K$, where $\mathbf{a}^{(k)} = (a^{(k)}_{1}, \ldots, a^{(k)}_{M})^\top \in \mathbb{R}^M$, and $\mathbf{a}=(\mathbf{a}^{(1)\top}, \ldots, \mathbf{a}^{(K)\top})^\top \in \mathbb{R}^{MK}$, so that $\boldsymbol{\xi}[\mathbf{t}] = (\psi(\mathbf{t})^\top, \boldsymbol{\theta}^\top)^\top = (\mathbf{a}^\top B(\mathbf{t}), \boldsymbol{\theta}^\top)^\top \in \mathbb{R}^{K+p}$. We denote the outcome data as
\begin{equation}
\widetilde{\mathbf{y}}=(Y_1, \ldots, Y_n)^\top \in \mathbb{R}^n.
\end{equation}
%
For each $k = 1, \ldots, K$, let $J^{(k)} \in \mathbb{R}^{M \times M}$ denote the Gram matrix of the basis functions associated with the $k$-th functional predictor, defined by
\begin{equation}\label{def:gram_basis}
...
J = \mathrm{blkdiag}(J^{(1)}, \cdots, J^{(K)}) \in \mathbb{R}^{MK \times MK}~\text{and}~J^\ast=\mathrm{blkdiag}(J, I_p) \in \mathbb{R}^{(MK+p) \times (MK+p)}.
\end{equation}
...
\end{equation}
%
The finite-basis approximation allows us to represent the hybrid predictor observations using the coefficient matrix $\widetilde{C}^\ast$, enabling all associated computations to be carried out via matrix operations involving $\widetilde{C}^\ast$, $J^\ast$, and $\ddot{J}^\ast$.

\subsubsection{Smoothed estimation of PLS components}\label{sec: pls component computation intermediate}
Building on the basis function approximation from Section~\ref{sec: finite basis approximation}, we now introduce an  intermediate strategy for implementing the first step of the PLS algorithm. Specifically, the following proposition states a tractable generalized eigenvalue problem (i.e. maximization of a generalized Rayleigh quotient) that can be solved to directly obtain a hybrid PLS component $\widehat{\boldsymbol{\xi}}=(\widehat{\psi}, \widehat{\boldsymbol{\theta}})$, which has the same structure as the hybrid predictor $\mathbf{W}=(X,\mathbf{Z})$ of model \eqref{eq: Hybrid functional model}. 

At the $l$-th iteration, we denote, for simplicity, the residualized response and the hybrid predictors in basis approximation form as follows:
\begin{equation*}
    Y_i \equiv Y_i^{[l]} \quad \text{and}  \quad \mathbf{W}_i \equiv \mathbf{W}_i^{[l]},
...
\jongmin{in the (1,1)th block, should $ \mathbf{y}$ be replaced by $\widetilde{\mathbf{y}} $? }
...
\end{equation}
and  $\boldsymbol{\xi}^*=(\mathbf{a}^{\top}, \boldsymbol{\theta}^{\top})^\top \in \mathbb{R}^{MK + p}$. Then, the $l$-th PLS component, $\widehat{\boldsymbol{\xi}} \equiv \widehat{\boldsymbol{\xi}}_l$ can be obtained by solving
\begin{equation}\label{def:eigenproblem_smoothed}
       \argmax \limits_{\boldsymbol{\xi}^* \in \mathbb{R}^{MK+p}} \; \frac{\boldsymbol{\xi}^{*\top} V^\ast \boldsymbol{\xi}^*}{\boldsymbol{\xi}^{*\top} J^\ast \boldsymbol{\xi}^*}, \quad \textit{or equivalently}, \quad \argmax \limits_{\boldsymbol{\xi}^* \in \mathbb{R}^{MK+p}} \;  \boldsymbol{\xi}^{*\top} V^\ast \boldsymbol{\xi}^* \quad \textnormal{subject to}  \quad \boldsymbol{\xi}^{*\top} J^\ast \boldsymbol{\xi}^*=1,
\end{equation}
with respect to $\widehat{\boldsymbol{\xi}}^*=(\widehat{\mathbf{a}}^{\top}, \widehat{\boldsymbol{\theta}}^{\top})^\top$, and then setting $\widehat{\boldsymbol{\xi}}[\mathbf{t}] = [\widehat{\psi}(\mathbf{t})^\top, \widehat{\boldsymbol{\theta}}^\top]^\top =  B^*[\mathbf{t}]^\top \widehat{\boldsymbol{\xi}}^*$. The $l$-th PLS score $\widehat{\rho}_{i} \equiv \widehat{\rho}_{il}$ can be obtained as $\widehat{\rho}_i = \langle \mathbf{W}_i, \widehat{\boldsymbol{\xi}} \rangle_\mathcal{H}$.
\end{proposition}
...
Here, $\widehat{\textnormal{Cov}}^2(\langle \mathbf{W}, \boldsymbol{\xi} \rangle_{\mathcal{H}}, Y)$ is expressed as $\boldsymbol{\xi}^{*\top} V^\ast \boldsymbol{\xi}^*$ based on the aforementioned basis approximations. Thus, finding $\boldsymbol{\xi}^*$ that maximizes $\boldsymbol{\xi}^{*\top} V^\ast \boldsymbol{\xi}^*$ amounts to finding $\widehat{\boldsymbol{\xi}}$ that maximizes the sample squared covariance $\widehat{\textnormal{Cov}}^2(\langle \mathbf{W}, \boldsymbol{\xi} \rangle_{\mathcal{H}}, Y)$, which is the first step of the PLS algorithm. The constraint $\boldsymbol{\xi}^{*\top} J^\ast \boldsymbol{\xi}^*=1$ scales the obtained PLS component to have a unit norm, that is, $\langle \widehat{\boldsymbol{\xi}}, \widehat{\boldsymbol{\xi}} \rangle_\mathcal{H}=1$. 
...
\begin{align*}
    ...
    &= n^{-1} \widetilde{\mathbf{y}}^\top (\widetilde{C}J\mathbf{a} + \widetilde{Z} \boldsymbol{\theta}).
    \\&=
    \frac{1}{n}\tilde{\mathbf{y}}^\top
   \begin{bmatrix} \widetilde{C}J \; \widetilde{Z} \end{bmatrix} \;      \underbrace{
     \begin{bmatrix} \mathbf{a} \\ \mathbf{\theta} \end{bmatrix}
     }_{:= \boldsymbol{\beta}}.
\end{align*}
Let us 
denote $\mathbf{b} =  \widetilde{\mathbf{y}}^\top \begin{bmatrix} \widetilde{C}J \; \widetilde{Z} \end{bmatrix}$. This is the weighted row sum of $\begin{bmatrix} \widetilde{C}J \; \widetilde{Z} \end{bmatrix}$, weighted by $\widetilde{\mathbf{y}}$.
Then using the symmetricity of $J^\ast$, we solve a linear system $J^\ast \mathbf{d} =
    ...
    \widehat{\textnormal{Cov}} (\langle \mathbf{W}, \boldsymbol{\xi} \rangle_{\mathcal{H}}, Y ) 
    %%%%%%%%%%%%%%%%%%%%%
    &= 
    %%%%%%%%%%%%%%%%%%%%%%%%
     \max_{\sqrt{\boldsymbol{\beta} J^\ast  \boldsymbol{\beta}} = 1}
...
Although Proposition \ref{Proposition: generalized rayleigh quotient} provides an efficient and principled way to compute the PLS components $\boldsymbol{\xi}$, their functional part $\psi=(\psi^{(1)}, \ldots, \psi^{(K)})$ may not be smooth, which can lead to overfitting and unstable predictions. Thus, in this section, we provide a modification of Proposition  \ref{Proposition: generalized rayleigh quotient} that yields regularized (smoothed) PLS components.
...
Now, maximizing $\widehat{\textnormal{Cov}}^2 \left(\langle \mathbf{W}, \boldsymbol{\xi} \rangle_\mathcal{H}, Y \right)$ is not the only aim; we also want to prevent the roughness of each $\psi^{(k)}$---that is, $\textnormal{PEN}(\psi^{(k)})$---from being too large. One possible way to achieve this is to extend \citet{Rice1991}'s approach of obtaining smoothed functional PCA components, and find $\boldsymbol{\xi}^*$ that maximizes:
\begin{equation}
   \widehat{\textnormal{Cov}}^2 \left(\langle \mathbf{W}, \boldsymbol{\xi} \rangle_\mathcal{H}, Y \right) - \sum \limits_{k=1}^K \lambda_k \textnormal{PEN} \left(\psi^{(k)} \right),
    \label{eq: penalized squred variance}
\end{equation}
subject to $\boldsymbol{\xi}^{*\top} J^\ast \boldsymbol{\xi}^*=1$. The equation \eqref{eq: penalized squred variance} quantifies the trade-off between fidelity to the data (in this case the sample covariance between the $\rho=\langle \mathbf{W}, \boldsymbol{\xi} \rangle_\mathcal{H}$ and $Y$ in the direction of $\boldsymbol{\xi}$) and roughness as measured by the sum of the individual penalty terms in \eqref{eq: roughness penalty}. The smoothing parameters $\{\lambda_k\}_{k=1}^K$ control the relative importance between the two objectives. Note that the penalty term can be written concisely using matrices as:
\begin{equation*}
    \sum \limits_{k=1}^K \lambda_k \textnormal{PEN} \left(\psi^{(k)} \right) = \sum \limits_{k=1}^K \lambda_k \mathbf{a}^{(k)T} \ddot{J}^{(k)} \mathbf{a}^{(k)} = \boldsymbol{\xi}^{* \top} \Lambda \ddot{J}^\ast \boldsymbol{\xi}^*,
\end{equation*}
...
\begin{equation*}
    ...
    \in \mathbb{R}^{(MK+p) \times (MK+p)}.
\end{equation*}
and  $\boldsymbol{\xi}^*=(\mathbf{a}^{\top}, \boldsymbol{\theta}^{\top})^\top \in \mathbb{R}^{MK + p}$. Then, the $l$-th smoothed PLS component, $\widehat{\boldsymbol{\xi}} \equiv \widehat{\boldsymbol{\xi}}_l$ can be obtained in the following way.
We first compute the basis coefficients by solving the optimization problem
\begin{align}
\begin{split}
       &\argmax \limits_{\boldsymbol{\xi}^* \in \mathbb{R}^{MK+p}} \; \frac{\boldsymbol{\xi}^{*\top} V^\ast \boldsymbol{\xi}^*}{\boldsymbol{\xi}^{*\top}(J^\ast+\Lambda \ddot{J}^\ast) \boldsymbol{\xi}^*} , \quad \textit{or equivalently}, \\ 
       &\argmax \limits_{\boldsymbol{\xi}^* \in \mathbb{R}^{MK+p}} \;  \boldsymbol{\xi}^{*\top} V^\ast \boldsymbol{\xi}^* \quad \textnormal{subject to}  \quad \boldsymbol{\xi}^{*\top} (J^\ast+\Lambda \ddot{J}^\ast) \boldsymbol{\xi}^*=1.
...
\begin{equation*}
   \widehat{\boldsymbol{\xi}}[\mathbf{t}] = [\widehat{\psi}(\mathbf{t})^\top, \widehat{\boldsymbol{\theta}}^\top]^\top =  B^\ast[\mathbf{t}]^\top \widehat{\boldsymbol{\xi}}^\ast.   
\end{equation*}
 The $l$-th PLS score $\widehat{\rho}_{i} \equiv \widehat{\rho}_{il}$ can be obtained as 
 \begin{equation}\label{compute_pls_score}
 \widehat{\rho}_i = \langle \mathbf{W}_i, \widehat{\boldsymbol{\xi}} \rangle_\mathcal{H}
 \end{equation}
\end{proposition}
...
The constraint $\boldsymbol{\xi}^{*\top} (J^\ast+\Lambda \ddot{J}^\ast) \boldsymbol{\xi}^*=1$ enforces the following modified orthonormality of the estimated PLS components ($\widehat{\boldsymbol{\xi}}_1, \widehat{\boldsymbol{\xi}}_2, \ldots$)  based on the modified inner product $\langle \langle h_l, h_j \rangle \rangle = \langle h_l, h_j \rangle_{\mathcal{H}} + \sum_{k=1}^K \lambda_k \langle \ddot{f}_l^{(k)}, \ddot{f}_j^{(k)} \rangle_{L^2}$ which now takes into account the roughness of the functions:
\begin{align*}
  \langle \langle \widehat{\boldsymbol{\xi}}_l, \widehat{\boldsymbol{\xi}}_j \rangle \rangle 
   &= \langle \widehat{\boldsymbol{\xi}}_l, \widehat{\boldsymbol{\xi}}_j \rangle_{\mathcal{H}} + \sum_{k=1}^K \lambda_k \left\langle \ddot{\widehat{\psi}}_l^{(k)}, \ddot{\widehat{\psi}}_j^{(k)} \right \rangle_{L^2} \\
  &= \sum \limits_{k=1}^K \int_{\mathcal{T}_k} \widehat{\psi}_l^{(k)}(t_k) \widehat{\psi}_j^{(k)}(t_k) dt_k + \sum \limits_{r=1}^p \widehat{\theta}_{lr} \widehat{\theta}_{jr} + \sum_{k=1}^K \lambda_k \int_{\mathcal{T}_k} \ddot{\widehat{\psi}}_l^{(k)}(t_k) \ddot{\widehat{\psi}}_j^{(k)}(t_k) dt_k \\
& = \mathbb{1}(l=j). 
\end{align*}
...
\end{equation*}
through the transformations $\mathbf{e} = L^\top \boldsymbol{\xi}^*$ and $E = L^{-1} V^\ast L^{\top-1}$. Once $\mathbf{e}$ is obtained as the first eigenvector of $E$, we set $\boldsymbol{\xi}^* = L^{\top-1} \mathbf{e}$ and $\widehat{\boldsymbol{\xi}}[\mathbf{t}] = B^*[\mathbf{t}]^\top \boldsymbol{\xi}^*$.
...
The regression coefficient $\boldsymbol{\eta}$ in model \eqref{eq: Hybrid functional model} can be written in terms of the estimated PLS components and scores. First, note that we can show $\widehat{\rho}_{il} = \langle \mathbf{W}_i^{[l]}, \widehat{\boldsymbol{\xi}}_l \rangle_\mathcal{H} = \langle \mathbf{W}_i, \widehat{\boldsymbol{\zeta}}_l \rangle_\mathcal{H}$, where $\widehat{\boldsymbol{\zeta}}_l = \widehat{\boldsymbol{\xi}}_l - \sum_{u=1}^{l-1} \langle \widehat{\boldsymbol{\delta}}_u, \widehat{\boldsymbol{\xi}}_l \rangle_{\mathcal{H}} \widehat{\boldsymbol{\zeta}}_u$ for $l \ge 2$, with $\widehat{\boldsymbol{\zeta}}_1 = \widehat{\boldsymbol{\xi}}_1$. Then, the decomposition of $Y_i$ in \eqref{eq: decomposition} leads to
\begin{equation*}
    Y_i = \sum \limits_{l=1}^L \widehat{\nu}_l \langle \mathbf{W}_i^{[l]}, \widehat{\boldsymbol{\xi}}_l \rangle_\mathcal{H} +\epsilon_i = \left\langle \mathbf{W}_i, \sum \limits_{l=1}^L\widehat{\nu}_l \widehat{\boldsymbol{\zeta}}_l \right\rangle_\mathcal{H}+\epsilon_i,
\end{equation*}
...
\For{ l = 1, 2, \ldots  $T$}
\State $V^\ast \leftarrow n^{-2} J^\ast \widetilde{C}^{*\top} \widetilde{\mathbf{y}} \widetilde{\mathbf{y}}^\top \widetilde{C}^{*} J^\ast$
\Comment{ \eqref{def:V_star}}
\State 	$\widehat{\boldsymbol{\xi}}_l^\ast \leftarrow \argmax_{\boldsymbol{\xi}^* \in \mathbb{R}^{MK+p}} \;  \boldsymbol{\xi}^{*\top} V^\ast \boldsymbol{\xi}^* \quad \textnormal{subject to}  \quad \boldsymbol{\xi}^{*\top} (J^\ast+\Lambda \ddot{J}^\ast) \boldsymbol{\xi}^*=1
      $
    \Comment{ \eqref{eq: Regularized generalized rayleigh quotient equation}} 
\State $\widehat{\boldsymbol{\rho}}_l
\leftarrow 
\widetilde{C}^* J^\ast  \widehat{\boldsymbol{\xi}}_l^\ast
$
...
\begin{align*}
  & \mathcal{C}_{YW} \mathbf{h} = E\{\langle \mathbf{W}, \mathbf{h} \rangle_\mathcal{H} Y \} = \langle \Sigma_{YW}, \mathbf{h} \rangle_\mathcal{H} = \sum \limits_{k=1}^K \int_{\mathcal{T}_k} \sigma_{YX}^{(k)}(t_k) f^{(k)}(t_k) dt_k + E(Y \mathbf{Z}^\top) \mathbf{v} \\
...
\end{proposition}
By the Hilbert-Schmidt theorem (e.g., Theorem 4.2.4 in \citealp{Hsing2015}), Proposition \ref{Proposition: property of U} guarantees the existence of a complete orthonormal system of eigenfunctions $\{\boldsymbol{\xi}_{(u)}\}_{u \in \mathbb{N}}$ of $\mathcal{U}$ in $\mathcal{H}$ such that $\mathcal{U} \boldsymbol{\xi}_{(u)} = \kappa_{(u)}  \boldsymbol{\xi}_{(u)}$, where $\{\kappa_{(u)}\}_{u \in \mathbb{N}}$ are the corresponding sequence of eigenvalues that goes to zero as $u \rightarrow \infty$, that is, $\kappa_{(1)} \ge \kappa_{(2)} \ge \cdots \ge 0$.
...
\label{proposition: eigenanalysis}
\end{proposition}
...
\begin{proposition}
The estimated PLS components, $\widehat{\boldsymbol{\xi}}_1, \widehat{\boldsymbol{\xi}}_2, \ldots, \widehat{\boldsymbol{\xi}}_L$, from the proposed PLS algorithm are mutually orthonormal with respect to the modified inner product $\langle \langle h_l, h_j \rangle \rangle = \langle h_l, h_j \rangle_{\mathcal{H}} + \sum_{k=1}^K \lambda_k \langle \ddot{f}_l^{(k)}, \ddot{f}_j^{(k)} \rangle_{L^2}$ in $\mathcal{H}$ that incorporates the roughness of the functions; i.e.,
\begin{equation*}
  \langle \langle \widehat{\boldsymbol{\xi}}_l, \widehat{\boldsymbol{\xi}}_j \rangle \rangle 
   = \langle \widehat{\boldsymbol{\xi}}_l, \widehat{\boldsymbol{\xi}}_j \rangle_{\mathcal{H}} + \sum_{k=1}^K \lambda_k \left\langle \ddot{\widehat{\psi}}_l^{(k)}, \ddot{\widehat{\psi}}_j^{(k)} \right\rangle_{L^2}
 = \mathbb{1}(l=j). 
\end{equation*}
\label{proposition: modified orthnormality of PLS components}
\end{proposition}
...
\end{equation*}
\label{proposition: orthnormality of PLS scores}
\end{proposition}
...
The regression coefficients for the first functional predictor, $\beta_{F_1}$, are drawn from a multivariate normal distribution $N(0, 5 I_p)$, with a fixed random seed. The coefficients for the second functional predictor, $\beta_{F_2}$, are drawn independently from the same distribution and are also smoothed using a B-spline basis with 10 basis functions.
...
\begin{align*}
    ...
    &=  \frac{1}{n} \sum \limits_{i=1}^n  \left\{ \sum \limits_{k=1}^K \int_{\mathcal{T}_k} X_i(t_k) \psi^{(k)}(t_k) dt_k \right\} Y_i +  \frac{1}{n} \sum \limits_{i=1}^n \mathbf{Z}_i^\top \boldsymbol{\theta} Y_i \\
     &=  \frac{1}{n} \sum \limits_{i=1}^n \left[ \sum \limits_{k=1}^K \int_{\mathcal{T}_k} \left\{\sum_{m=1}^M c_{im}^{(k)} b^{(k)}_{m}(t) \right\} \left\{\sum_{m=1}^M a^{(k)}_{m} b^{(k)}_m(t) \right\} dt_k \right] Y_i + \frac{1}{n} \sum \limits_{i=1}^n \mathbf{Z}_i^\top \boldsymbol{\theta} Y_i \\
    &= n^{-1} \widetilde{\mathbf{y}}^\top \left[ \sum \limits_{k=1}^K \widetilde{C}^{(k)} \left\{ \int_{\mathcal{T}_k} \mathbf{b}^{(k)}(t_k) \mathbf{b}^{(k)}(t_k)^\top dt_k \right\} \mathbf{a}^{(k)} \right] + n^{-1}\widetilde{\mathbf{y}}^\top \widetilde{Z} \boldsymbol{\theta} \\
    &= n^{-1} \widetilde{\mathbf{y}}^\top (\widetilde{C} J \mathbf{a}) + n^{-1}\widetilde{\mathbf{y}}^\top \widetilde{Z} \boldsymbol{\theta} \\
    &= n^{-1} \widetilde{\mathbf{y}}^\top (\widetilde{C}J\mathbf{a} + \widetilde{Z} \boldsymbol{\theta}).
\end{align*}
This implies that
\begin{align*}
     \widehat{\textnormal{Cov}}^2(\langle \mathbf{W}, \boldsymbol{\xi} \rangle_{\mathcal{H}}, Y ) &= n^{-2} \widetilde{\mathbf{y}}^\top (\widetilde{C}J\mathbf{a} + \widetilde{Z} \boldsymbol{\theta}) (\widetilde{C}J\mathbf{a} + \widetilde{Z} \boldsymbol{\theta})^\top \widetilde{\mathbf{y}} \\
     &= n^{-2} \{\mathbf{a}^\top J \widetilde{C}^\top \widetilde{\mathbf{y}} \widetilde{\mathbf{y}}^\top \widetilde{C} J \mathbf{a} +  \mathbf{a}^\top J \widetilde{C}^\top \widetilde{\mathbf{y}} \widetilde{\mathbf{y}}^\top \widetilde{Z} \boldsymbol{\theta} +  \boldsymbol{\theta} \widetilde{Z}^\top \widetilde{\mathbf{y}} \widetilde{\mathbf{y}}^\top \widetilde{C} J \mathbf{a} + \boldsymbol{\theta}^\top \widetilde{Z}^\top \widetilde{\mathbf{y}} \widetilde{\mathbf{y}}^\top \widetilde{Z} \boldsymbol{\theta}\} \\
     &= \begin{bmatrix}
         \mathbf{a}^\top & \boldsymbol{\theta}^\top
     \end{bmatrix}
     \begin{bmatrix}
        n^{-2} J \widetilde{C}^\top \widetilde{\mathbf{y}} \widetilde{\mathbf{y}}^\top \widetilde{C} J & n^{-2} J \widetilde{C}^\top \widetilde{\mathbf{y}} \widetilde{\mathbf{y}}^\top \widetilde{Z}  \\
        n^{-2} \widetilde{Z}^\top \widetilde{\mathbf{y}} \widetilde{\mathbf{y}}^\top \widetilde{C} J & n^{-2} \widetilde{Z}^\top \widetilde{\mathbf{y}} \widetilde{\mathbf{y}}^\top \widetilde{Z}
     \end{bmatrix} 
     \begin{bmatrix}
         \mathbf{a} \\ \boldsymbol{\theta}
     \end{bmatrix} \\
     &\equiv \boldsymbol{\xi}^{*T} V^\ast \boldsymbol{\xi}^*.
\end{align*}
Also, the unit norm constraint of the PLS components in $\mathcal{H}$ can be translated into:
\begin{align*}
    \langle \boldsymbol{\xi}, \boldsymbol{\xi} \rangle_{\mathcal{H}} &= \sum \limits_{k=1}^K \int_{\mathcal{T}} \psi^{(k)}(t_k) \psi^{(k)}(t_k) dt_k + \boldsymbol{\theta}^{\top} \boldsymbol{\theta} \\
    ...
    &= \boldsymbol{\xi}^{*\top} J^\ast  \boldsymbol{\xi}^{*} \\ 
    &= 1 \\
\end{align*}
...
\begin{equation*}
    \max \limits_{\boldsymbol{\xi}^* \in \mathbb{R}^{MK+p}} \;  \boldsymbol{\xi}^{*\top} V^\ast \boldsymbol{\xi}^* \quad \textnormal{subject to}  \quad \boldsymbol{\xi}^{*\top} J^\ast \boldsymbol{\xi}^*=1,
\end{equation*}
or equivalently solving
\begin{equation*}
    \max \limits_{\boldsymbol{\xi}^* \in \mathbb{R}^{MK+p}} \; \frac{\boldsymbol{\xi}^{*\top} V^\ast \boldsymbol{\xi}^*}{\boldsymbol{\xi}^{*\top} J^\ast \boldsymbol{\xi}^*},
\end{equation*}
where $J^\ast = \mathrm{blkdiag}[J, I_p] \in \mathbb{R}^{MK+p}$. In other words, the PLS component is chosen to maximize the squared sample covariance  $\widehat{\textnormal{Cov}}^2(\langle \mathbf{W}, \boldsymbol{\xi} \rangle_{\mathcal{H}}, Y ) = \boldsymbol{\xi}^{*\top} V^\ast \boldsymbol{\xi}^*$ subject to the constraint $\boldsymbol{\xi}^{*\top} J^\ast \boldsymbol{\xi}^*=1$. 
...
% \begin{equation*}
%         \frac{\boldsymbol{\xi}^{*T} V^\ast \boldsymbol{\xi}^*}{\boldsymbol{\xi}^{*T} J^\ast \boldsymbol{\xi}^* + \kappa \textnormal{PEN}(\psi)},
% \end{equation*}
...
\noindent Now we prove Proposition 4. The singular value decomposition of $\mathcal{C}_{YW}$ is given by
\begin{equation*}
...
\vspace{1cm}
\noindent $\boldsymbol{\cdot}$  \underline{\textbf{Proposition 5}}

Firstly, it is obvious by the unit-norm constraint enforced in Proposition 4 that:
\begin{align*}
    \langle \langle \hat{\boldsymbol{\xi}}_l,  \hat{\boldsymbol{\xi}}_l \rangle \rangle &= \langle \widehat{\boldsymbol{\xi}}_l, \widehat{\boldsymbol{\xi}}_j \rangle_{\mathcal{H}} + \sum_{k=1}^K \lambda_k \left\langle \ddot{\widehat{\psi}}_l^{(k)}, \ddot{\widehat{\psi}}_j^{(k)} \right\rangle_{L^2}  \\
    &= \sum \limits_{k=1}^K \int_{\mathcal{T}_k} \widehat{\psi}_l^{(k)}(t_k) \widehat{\psi}_j^{(k)}(t_k) dt_k + \sum \limits_{r=1}^p \widehat{\theta}_{lr} \widehat{\theta}_{jr} + \sum_{k=1}^K \lambda_k \int_{\mathcal{T}_k} \ddot{\widehat{\psi}}_l^{(k)}(t_k) \ddot{\widehat{\psi}}_j^{(k)}(t_k) dt_k \\
    &= \sum \limits_{k=1}^K \int_{\mathcal{T}_k} \hat{\mathbf{a}}^{(k) \top}_l \mathbf{b}^{(k)}(t_k) \mathbf{b}^{(k) \top}(t_k) \hat{\mathbf{a}}^{(k)}_l  dt_k + \boldsymbol{\theta}_l^\top \boldsymbol{\theta}_l + \sum \limits_{k=1}^K \lambda_k \int_{\mathcal{T}_k} \hat{\mathbf{a}}^{(k) \top}_l \ddot{\mathbf{b}}^{(k)}(t_k) \ddot{\mathbf{b}}^{(k) \top}(t_k) \hat{\mathbf{a}}^{(k)}_l  dt_k \\
    ...
    & = \hat{\boldsymbol{\xi}}_l^{*\top} J^\ast \hat{\boldsymbol{\xi}}_l^* + \hat{\boldsymbol{\xi}}_l^{*\top} \Lambda \ddot{J}^\ast \hat{\boldsymbol{\xi}}_l^* = \hat{\boldsymbol{\xi}}_l^{*\top} (J^\ast +\Lambda \ddot{J}^\ast) \hat{\boldsymbol{\xi}}_l^* = 1
\end{align*}
...
Then for $j < l$,
\begin{align*}
\int_{\mathcal{T}} \widetilde{W}^{[l]}[\mathbf{t}] \widehat{\boldsymbol{\xi}}_j[\mathbf{t}] d\mathbf{t} &= \int_{\mathcal{T}} P   \left(I_n - \frac{\widehat{\boldsymbol{\rho}}_{j} \widehat{\boldsymbol{\rho}}_{j}^\top}{\widehat{\boldsymbol{\rho}}_{j}^\top \widehat{\boldsymbol{\rho}}_{j}} \right)\widetilde{W}^{[j]}[\mathbf{t}]  \widehat{\boldsymbol{\xi}}_k[\mathbf{t}] d\mathbf{t}  \\
&= P \left\{\int_{\mathcal{T}} \widetilde{W}^{[j]}[\mathbf{t}] \widehat{\boldsymbol{\xi}}_j[\mathbf{t}] d\mathbf{t} - \frac{\widehat{\boldsymbol{\rho}}_{j} \widehat{\boldsymbol{\rho}}_{j}^\top}{\widehat{\boldsymbol{\rho}}_{j}^\top \widehat{\boldsymbol{\rho}}_{j}} \int_{\mathcal{T}} \widetilde{W}^{[j]}[\mathbf{t}] \widehat{\boldsymbol{\xi}}_j[\mathbf{t}] d\mathbf{t} \right\} \\ 
...
\begin{align*}
    \langle \langle \widehat{\boldsymbol{\xi}}_l, \widehat{\boldsymbol{\xi}}_j \rangle \rangle &= \widehat{\boldsymbol{\xi}}_l^{*\top} (J^\ast + \Lambda \ddot{J}^\ast)\widehat{\boldsymbol{\xi}}_j^{*} \\
    & = \frac{1}{\kappa_l} (V^\ast_l \widehat{\boldsymbol{\xi}}_l^{*})^\top \widehat{\boldsymbol{\xi}}_j^{*} \\
    &=  \frac{1}{\kappa_l}\widehat{\boldsymbol{\xi}}_l^{* \top} V^\ast_l \widehat{\boldsymbol{\xi}}_j^{*} \\
    &= \frac{1}{\kappa_l}\widehat{\boldsymbol{\xi}}_l^{* \top} \left(n^{-2} J^\ast \widetilde{C}^{*[l]\top} \widetilde{\mathbf{y}}^{[l]}\widetilde{\mathbf{y}}^{[l]\top} \widetilde{C}^{*[l]} J^\ast \right) \widehat{\boldsymbol{\xi}}_j^{*} \\
    &= \frac{1}{n^2\kappa_l}\widehat{\boldsymbol{\xi}}_l^{* \top} \ J^\ast \widetilde{C}^{*[l]\top} \widetilde{\mathbf{y}}^{[l]}\widetilde{\mathbf{y}}^{[l]\top}  \widetilde{C}^{*[l]}\left(\int_{\mathcal{T}} B^*[\mathbf{t}] B^*[\mathbf{t}]^\top d\mathbf{t}  \right) \widehat{\boldsymbol{\xi}}_j^{*} \\
    &= \frac{1}{n^2\kappa_l}\widehat{\boldsymbol{\xi}}_l^{* \top}  J^\ast \widetilde{C}^{*[l]\top} \widetilde{\mathbf{y}}^{[l]}\widetilde{\mathbf{y}}^{[l]\top}  \left(\int_{\mathcal{T}} \widetilde{C}^{*[l]} B^*[\mathbf{t}] B^*[\mathbf{t}]^\top \widehat{\boldsymbol{\xi}}_j^{*} d\mathbf{t}  \right)   \\
    &= \frac{1}{n^2\kappa_l}\widehat{\boldsymbol{\xi}}_l^{* \top}  J^\ast \widetilde{C}^{*[l]\top} \widetilde{\mathbf{y}}^{[l]}\widetilde{\mathbf{y}}^{[l]\top}  \left(\int_{\mathcal{T}} \widetilde{W}^{[l]}[\mathbf{t}] \widehat{\boldsymbol{\xi}}_j[\mathbf{t}] d\mathbf{t}  \right)   \\
    &=0, 
\end{align*}
where the last equality follows from the above result that $\int_{\mathcal{T}} \widetilde{W}^{[l]}[\mathbf{t}] \widehat{\boldsymbol{\xi}}_j[\mathbf{t}] d\mathbf{t} = \mathbf{0}$.




\vspace{1cm}
\noindent $\boldsymbol{\cdot}$  \underline{\textbf{Proposition 6}}

First note that $\widehat{\boldsymbol{\rho}}_l = \int_{\mathcal{T}} \widetilde{W}^{[l]}[\mathbf{t}] \widehat{\boldsymbol{\xi}}_l[\mathbf{t}] d\mathbf{t}= \widetilde{C}^{*[l]} J^\ast \widehat{\boldsymbol{\xi}}^*_{l}$ for $l=1, \ldots, L$. Then, as above, we first express the $l$-th residualized hybrid predictor in terms of its previous $(l-1)$-th version:
\begin{equation*}
    \widetilde{W}^{[l]}[\mathbf{t}] = \widetilde{W}^{[l]}[\mathbf{t}] - \widehat{\boldsymbol{\rho}}_{l-1} \widehat{\boldsymbol{\delta}}_l[\mathbf{t}]^\top 
\end{equation*}
which, after applying the finite-dimensional form (9), implies that
...
\begin{aligned}
    ...
    & =\widetilde{C}^{*[l-1]}B^*[\mathbf{t}] -  \frac{\widetilde{C}^{*[l-1]} J^\ast \widehat{\boldsymbol{\xi}}^{*}_{l-1} \widehat{\boldsymbol{\rho}}_{l-1}^\top \widetilde{C}^{*[l-1]} B^*[\mathbf{t}]}{\widehat{\boldsymbol{\rho}}_{l-1}^\top \widehat{\boldsymbol{\rho}}_{l-1}},
\end{aligned}
\end{equation*}
...
\setlength{\jot}{10pt}
\begin{aligned}
     \widetilde{C}^{*[l]} &= \widetilde{C}^{*[l-1]} -  \frac{\widetilde{C}^{*[l-1]} J^\ast \widehat{\boldsymbol{\xi}}^{*}_{l-1} \widehat{\boldsymbol{\rho}}_{l-1}^\top \widetilde{C}^{*[l-1]}}{\widehat{\boldsymbol{\rho}}_{l-1}^\top \widehat{\boldsymbol{\rho}}_{l-1}} \\
     &= \widetilde{C}^{*[l-1]} \left(I_{MK+p} - \frac{J^\ast \widehat{\boldsymbol{\xi}}^{*}_{l-1} \widehat{\boldsymbol{\rho}}_{l-1}^\top \widetilde{C}^{*[l-1]}}{\widehat{\boldsymbol{\rho}}_{l-1}^\top \widehat{\boldsymbol{\rho}}_{l-1}} \right) \\
     &= \widetilde{C}^{*[j]} \left(I_{MK+p} - \frac{J^\ast \widehat{\boldsymbol{\xi}}^{*}_{j} \widehat{\boldsymbol{\rho}}_{j}^\top \widetilde{C}^{*[j]}}{\widehat{\boldsymbol{\rho}}_{j}^\top \widehat{\boldsymbol{\rho}}_{j}} \right) \widetilde{P},
\end{aligned}
\end{equation*}
where $\widetilde{P}$ is a $(MK+p) \times (MK+p)$ matrix defined as
\begin{equation*}
    \widetilde{P} = \prod \limits_{h=j+1}^{l-1} \left(I_{MK+p} - \frac{J^\ast \hat{\boldsymbol{\xi}}^*_h \widehat{\boldsymbol{\rho}}_{h}^\top \widetilde{C}^{*[h]}}{\widehat{\boldsymbol{\rho}}_{h}^\top \widehat{\boldsymbol{\rho}}_{h}} \right)
\end{equation*}
Then, for $j < l$,
\begin{equation*}
\setlength{\jot}{10pt}
\begin{aligned}
     \widehat{\boldsymbol{\rho}}_j^\top \widetilde{C}^{*[l]} &=  \widehat{\boldsymbol{\rho}}_j^\top \widetilde{C}^{*[j]} \left(I_{MK+p} - \frac{J^\ast \widehat{\boldsymbol{\xi}}^{*}_{j} \widehat{\boldsymbol{\rho}}_{j}^\top \widetilde{C}^{*[j]}}{\widehat{\boldsymbol{\rho}}_{j}^\top \widehat{\boldsymbol{\rho}}_{j}} \right) \widetilde{P} \\
     &=\left( \widehat{\boldsymbol{\rho}}_j^\top \widetilde{C}^{*[j]} - \frac{ \widehat{\boldsymbol{\rho}}_j^\top \widetilde{C}^{*[j]} J^\ast \widehat{\boldsymbol{\xi}}^{*}_{j} \widehat{\boldsymbol{\rho}}_{j}^\top \widetilde{C}^{*[j]}}{\widehat{\boldsymbol{\rho}}_{j}^\top \widehat{\boldsymbol{\rho}}_{j}} \right) \widetilde{P} \\
