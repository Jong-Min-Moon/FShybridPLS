...
\end{align*}
where $\boldsymbol{\xi}$ is the concatenated coefficient vector form of $\xi$ as defined in \eqref{def:maximizer_euclidean_form}, $\mathbb{B}^{\prime\prime}$ is defined in \eqref{def:gram_block}, and $\Lambda \in \mathbb{R}^{(MK+p) \times (MK+p)}$ is given by
...
However, the approach of \citet{Rice1991} assumes that the functional predictors admit an orthogonal expansion in the $L^2$ sense. As an alternative, we adopt the strategy of \citet{Silverman1996}, which replaces the standard orthonormality constraint with a weaker one based on a modified inner product that incorporates roughness.
Building on this idea, Proposition \ref{proposition:eigen_regul} modifies the optimization problem presented in Proposition \ref{proposition:eigen_noregul}, yielding a smoothed PLS direction that maximizes the squared covariance over a class of smooth functions.
...
\end{proposition}
	...
	\begin{proposition} 
		...
		\end{equation}
	\end{proposition}
	The proof of Proposition \ref{proposition:eigen_regul} is provided in Appendix \ref{section:proof:eigenproblem_derivation}.
...
Therefore, the optimization problem \eqref{optim:summation_form}, which is equivalent to the squared covariance maximization problem, reduces to the generalized eigenvalue problem \eqref{eq: Regularized generalized rayleigh quotient equation}. This completes the proof of Proposition~\ref{proposition:eigen_noregul}.
 ...
 \begin{proof}
	...
	Therefore, for any local maxima, gradients of $\mathcal{L}$ must be zero:
	\begin{align*}
	...
	\end{align*}
	By pre-multiplying equation \eqref{lag_grad_d} by $B^{-1}$,  and observing that  $2n^{-2}\sum_{\ell=1}^K \mathbf{y}^\top \Theta_\ell B \boldsymbol{\gamma}_\ell + \mathbf{y}^\top Z \boldsymbol{\zeta}$, which is a scalar quantity, appears in both \eqref{lag_grad_d} and \eqref{lag_grad_zeta}, the  conditions above together imply that any local maxima must be  scalar multiples of
	$
	\Theta_1^\top \mathbf{y}, \ldots, \Theta_K^\top \mathbf{y}, Z^\top \mathbf{y},
	$
	all scaled by the same factor. There exists only one such set of  vectors  that also satisfies the primal equality constraint, namely the normalized version of the vector set above. The global maximizer is therefore given by
	\begin{equation*}
