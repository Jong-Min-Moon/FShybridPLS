...
This computation along with \eqref{covariance_squares_as_quadratic_form} implies that the covariance maximization problem  
 ...
 \begin{proof}
 	...
 	\end{align*} 
 Let $\mathbf{u}_j := B \Theta_j^\top \mathbf{y} \in \mathbb{R}^M$ and $\mathbf{v} := Z^\top \mathbf{y} \in \mathbb{R}^p$. These are fixed problem data and not decision variables. Then the optimization problem becomes
 	 		\begin{align*}
 		\max_{\boldsymbol{\gamma}_1, \ldots, \boldsymbol{\gamma}_K \in \mathbb{R}^M,
 			\boldsymbol{\zeta} \in \mathbb{R}^p
 		}~&
...
The feasible set is compact, as it corresponds to the boundary of an ellipsoid in a finite-dimensional Euclidean space. Therefore, the  Weierstrass Extreme Value Theorem (see, for example, Theorem 2.3.1 in \citealt{bazaraaNonlinearProgrammingTheory2006}) ensures the existence of a global maximizer.
The constraint function is continuously differentiable, and its gradient vanishes only at the origin. Since the origin is not a feasible point, the gradient  is nonzero at all feasible points. Thus, the linear independence constraint qualification (LICQ) holds, ensuring that the KKT conditions are necessary for local maxima (see, for example, Theorem 5.3.1 in \citealt{bazaraaNonlinearProgrammingTheory2006}).
To use this fact,
...
\biggl[
\bigl\{
	\sum_{j=1}^K \boldsymbol{\gamma}_j^\top (B + \lambda_j B^{\prime\prime}) \boldsymbol{\gamma}_j + 	\boldsymbol{\zeta}^\top \boldsymbol{\zeta} 
	\bigr\}
	- 1
	\biggr]
.
\]
Then, if
$(\boldsymbol{\gamma}_1, \ldots, \boldsymbol{\gamma}_K, 
	\boldsymbol{\zeta}
)$
is a local maximizer, the gradients of $\mathcal{L}$ must vanish at that point:
\begin{align*}
	\nabla_{\boldsymbol{\gamma}_j} \mathcal{L} &= 
	s
	 \mathbf{u}_j
	- 2\mu (B + \lambda_j B^{\prime \prime}) \boldsymbol{\gamma}_j = 0, \quad \forall j, 
...
\begin{equation*}
	s := \frac{2}{n^2} 
	\left(
	\sum_{\ell=1}^K \mathbf{u}_\ell^\top B \boldsymbol{\gamma}_\ell + \mathbf{v}^\top  \boldsymbol{\zeta}
	\right).
\end{equation*}
Since we assumed in Section~\ref{sec: finite basis approximation} that the functions $\{b_m(t)\}$ and their second derivatives are linearly independent, both $B$ and $B^{\prime\prime}$ are positive definite (see, for example, Theorem 273 of \citealp{gockenbachFiniteDimensionalLinearAlgebra2010}). As positive definiteness is preserved under conic combinations, $B + \lambda_j B^{\prime\prime}$ is also positive definite. Therefore, \eqref{lag_grad_d} implies that 
\begin{equation*}
	\boldsymbol{\gamma}_j
	=
	\frac{s}{2\mu}	
	(B + \lambda_j B^{\prime \prime})^{-1}
	\mathbf{u}_j
	   , \quad \forall j.
\end{equation*}
If $\boldsymbol{\gamma}_j$ is part of a local optimum, then it must be a scalar multiple of 
$\tilde{\mathbf{u}}_j := (B + \lambda_j B^{\prime \prime})^{-1} \mathbf{u}_j$, 
i.e., $\boldsymbol{\gamma}_j = c_j \tilde{\mathbf{u}}_j$.
Similarly, \eqref{lag_grad_zeta} implies that if 
$\boldsymbol{\zeta}$ is a local optimum, then it must be a scalar multiple of $\mathbf{v}$,
i.e., $\boldsymbol{\zeta} = d \mathbf{v}$.
...
\end{equation*}
 	...
 	Observe that the objective is linear in each $\boldsymbol{\gamma}_j$ and in $\boldsymbol{\zeta}$, and the constraint is a strictly convex quadratic form. Hence, the optimal solution lies in the direction of the gradient of the linear objective.
 	
 	Define scalar weights $c_j$ and $c$ such that
 	\[
 	\boldsymbol{\gamma}_j = c_j \mathbf{u}_j, \quad \boldsymbol{\zeta} = c \mathbf{v}.
 	\]
 	...
 	\[
 	\boldsymbol{\gamma}_j = \frac{a_j}{d_j \sqrt{S}} \mathbf{u}_j, \quad \boldsymbol{\zeta} = \frac{1}{\sqrt{S}} \mathbf{v},
 	\]
 	where
 	\[
