...
\begin{document}
	...
	Additionally, let $\mathbf{Z}_i=(Z_{i1}, \ldots, Z_{ip})^\top$
...
Since $\widetilde{W}_i^{[l]}$ and $Y_i^{[l]}$ are assumed to have a sample mean of zero, their respective residuals, $\widetilde{W}_i^{[l+1]}$ and $Y_i^{[l+1]}$, also maintain a zero sample mean.
	...
	We derive that the PLS components obtained from the first step of the proposed algorithm satisfy the Tucker's Criterion \citep{Tucker1938} extended to our scalar-on-hybrid regression model setting \eqref{eq: Hybrid functional model}. We omit $l$ in the notations and first define the cross-covariance term between the response and the functional predictors (observed versions if $l=1$ and residualized versions if $l \ge 2$): 
...
\bigr) \in \mathcal{H}.
\end{equation*}
Based on these definitions, we introduce two cross-covariance operators.
\begin{lemma}\label{lemma:cross_cov_functional}
Define the operator $
\mathcal{C}_{YW}
=
\mathbb{E}(W_1 \otimes_\mathcal{H} Y_1)
: \mathcal{H} \rightarrow \mathbb{R}$ such that for any $h=(f_1, \ldots, f_K, \mathbf{v}) \in \mathcal{H}$, it maps $h$ to a real number as: \begin{equation*} \mathcal{C}_{YW} h := \mathbb{E} \left[ \langle W_1, h \rangle_\mathcal{H} Y_1 \right] = \langle \Sigma_{YW}, h \rangle_\mathcal{H} = \sum \limits_{k=1}^K \int_0^1 \sigma_{YX, k}(t) \, f_k(t) \, dt + \sigma_{YW}^\top \mathbf{v}. \end{equation*} 
...
Define $\mathcal{C}_{YW}= : \mathcal{H} \rightarrow \mathbb{R}$,
which	maps  
	$h=(f_1, \ldots, f_K, \mathbf{v}) \in \mathcal{H}$ to a real number:
	\begin{equation*}
		\mathcal{C}_{YW}
		h
		:=
		\mathbb{E} 
		\bigl[ 
		\langle W_1, h \rangle_\mathcal{H} Y_1 
		\bigr] 
		= 
		\langle \Sigma_{YW}, h \rangle_\mathcal{H}
		=
		\sum \limits_{k=1}^K \int_0^1 \sigma_{YX, k}(t) \, f_k(t) \, dt 
		+ 
		\sigma_{YW}^\top \mathbf{v}.
	\end{equation*}
	Next, define $\mathcal{C}_{WY} = \mathbb{E}[Y \otimes W]: \mathbb{R} \rightarrow \mathcal{H}$,
which	maps  $d \in \mathbb{R}$ to a hybrid object in $\mathcal{H}$ as follows:
	%%%%
	\begin{align*}		
		\mathcal{C}_{WY} d 
		:= 
...
Define
	$\mathcal{U} := \mathcal{C}_{WY} \circ \mathcal{C}_{YW}: \mathcal{H} \rightarrow \mathcal{H}$ as an operator which performs the following mapping:
	\begin{equation*}
		\mathcal{U} h 
		= 
		\mathcal{C}_{WY} ( \mathcal{C}_{YW} h) 
		=
		\mathcal{C}_{YW} (\langle \Sigma_{YW}, h \rangle_\mathcal{H}) 
		=  
		\Sigma_{YW} \, \langle \Sigma_{YW}, h \rangle_\mathcal{H} 
		= 
...
The fundamental property of classical scalar PLS is that the first eigenvector of the squared covariance matrix corresponds to the first PLS component direction. To derive an analogous result in the hybrid regression setting, we first demonstrate that $\mathcal{U}$ is a compact operator. This means that the image of any bounded sequence under $\mathcal{U}$ contains a convergent subsequence (for a formal definition, see Definition 4.1.1 of \citealt{hsingTheoreticalFoundationsFunctional2015}). The compactness of $\mathcal{U}$ is crucial as it guarantees the existence of a complete orthonormal system of eigenfunctions for $\mathcal{U}$ in $\mathcal{H}$.
To this end, we begin by defining the notion of uniform continuity for the cross-covariance function defined in \eqref{def:cross_covariance_yx}:
\begin{definition}[Uniform continuity]\label{def:uniform_continuity}
	For each $k \in \{1, \ldots, K\}$, the cross-covariance function $\sigma_{YX, k} \in \mathcal{F}$ is uniformly continuous if for any $\epsilon > 0$, there exists a $\delta_k > 0$ such that for all $t_1, t_2 \in [0,1]$ satisfying $|t_1 - t_2 | < \delta_k$, we have
$
		| \sigma_{YX, k} (t_1) - \sigma_{YX, k} (t_2) | < \epsilon.
$
\end{definition}
Using this definition, the following proposition states that $\mathcal{U}$ is a compact operator under a mild condition.
	\begin{proposition}[Compactness condition] \label{proposition:compact_operator}

		and if each $\sigma_{YX}^{(k)}$ ($k=1, \ldots, K)$ is uniformly continuous,
		then $\mathcal{U}$ is a compact operator.
\end{proposition}
The proof of Proposition \ref{proposition:compact_operator} is provided in Appendix \ref{section:proof:proposition:compact_operator}.
	By the Hilbert-Schmidt theorem (e.g., Theorem 4.2.4 in \citealp{hsingTheoreticalFoundationsFunctional2015}), Proposition \ref{proposition:compact_operator} guarantees the existence of a complete orthonormal system of eigenfunctions $\{\xi_{(u)}\}_{u \in \mathbb{N}}$ of $\mathcal{U}$ in $\mathcal{H}$ such that $\mathcal{U} \xi_{(u)} = \kappa_{(u)}  \xi_{(u)}$, where $\{\kappa_{(u)}\}_{u \in \mathbb{N}}$ are the corresponding sequence of eigenvalues that goes to zero as $u \rightarrow \infty$, that is, $\kappa_{(1)} \ge \kappa_{(2)} \ge \cdots \ge 0$.
...
\begin{theorem}[Population PLS]\label{theorem:population_PLS} Under the conditions of Proposition \ref{proposition:compact_operator}, the constrained maximum
	\begin{equation*} 
		\max \limits_{\substack{\xi \in \mathcal{H} \\ \Vert \xi \Vert_\mathcal{H}=1}} \operatorname{Cov}^2 \left(\langle W , \, \xi \rangle_\mathcal{H}, Y \right)
		\end{equation*} 
...
\noindent
\textit{Linearity.}~	  
 The operator $\mathcal{C}_{YW}: \mathcal{H} \rightarrow \mathbb{R}$ is defined as 
 $\mathcal{C}_{YW} h = \langle \Sigma_{YW}, h \rangle_\mathcal{H}$.
 For any $h_1, h_2 \in \mathcal{H}$ and scalar $c \in \mathbb{R}$, the linearity of the inner product implies: \begin{align*} \mathcal{C}_{YW}(h_1 + h_2) &= \langle \Sigma_{YW}, h_1 + h_2 \rangle_\mathcal{H} = \langle \Sigma_{YW}, h_1 \rangle_\mathcal{H} + \langle \Sigma_{YW}, h_2 \rangle_\mathcal{H} = \mathcal{C}_{YW} h_1 + \mathcal{C}_{YW} h_2,
 	 \\ 
 	 \mathcal{C}_{YW}(c h) &= \langle \Sigma_{YW}, c h \rangle_\mathcal{H} = c \langle \Sigma_{YW}, h \rangle_\mathcal{H} = c \, \mathcal{C}_{YW} h. \end{align*}
 Thus, $\mathcal{C}_{YW}$ is a linear functional. 
 
\medskip
\noindent
\textit{Boundedness.}~ 
To show that $\mathcal{C}_{YW}$ is bounded, we need to find a finite constant $M$ such that $|\mathcal{C}_{YW} h| \leq M \|h\|_\mathcal{H}$ for all $h \in \mathcal{H}$. By the Cauchy-Schwarz inequality, we have: \begin{equation*} 
	|\mathcal{C}_{YW} h| = |\langle \Sigma_{YW}, h \rangle_\mathcal{H}| \leq \|\Sigma_{YW}\|_\mathcal{H} \|h\|_\mathcal{H}. 
	\end{equation*}
Now, we must verify that $\|\Sigma_{YW}\|_\mathcal{H}$ is finite. 
Recall that 
...
The norm of $\Sigma_{YW}$ in $\mathcal{H}$ is given by: \begin{equation*} \|\Sigma_{YW}\|_\mathcal{H}^2 = \sum_{k=1}^K \int_0^1 \sigma_{YX,k}(t)^2 \, dt + \sum_{r=1}^p \sigma_{YZ,r}^2
	<
	K T Q_1 + p  Q_2 < \infty,
 \end{equation*} 
 where the last inequality uses the condition \eqref{condition_for_compact}.
 Let $M = \sqrt{K T Q_1 + p  Q_2 }$. Thus, we have shown that $|\mathcal{C}_{YW} h| \leq M \|h\|_\mathcal{H}$ for a finite constant $M$.
 ...
 \begin{proof}
For $h \in \mathcal{H}$ and $d \in \mathbb{R}$, we have:
\begin{equation*}
\langle
...
%%%%%%%%%%%%%%%%%%
=
\mathbb{E} \langle   Y_1 W_1 d , \, h\rangle_{\mathcal{H}} 
%%%%%%%%%%%%%%%%%%%%%%%%
 = \langle h, \, \mathbb{E}[Y_1 W_1 d   ]\rangle_{\mathcal{H}}
 =
 \langle h, \mathcal{C}_{WY} d \rangle_{\mathcal{H}}
\end{equation*}
Thus, we have $\langle \mathcal{C}_{YW} h, d \rangle =  \langle h, \mathcal{C}_{WY} d \rangle_{\mathcal{H}}$, which implies $\mathcal{C}_{WY} = \mathcal{C}_{YW}^\ast$.
...
\section{Proof of Lemma \ref{lemma:composite_cross_cov}}\label{section:proof:lemma:composite_cross_cov}
\begin{proof}
	The operator
$\mathcal{U}$ is \textit{self-adjoint} because for any $h_1, h_2 \in \mathcal{H}$, we have
\begin{align*}
\langle 
\mathcal{U} h_1, \, h_2 
\rangle_\mathcal{H}  
&= 
\bigl \langle 
	\langle h_1, \Sigma_{YW} \rangle_\mathcal{H} \Sigma_{YW},
	\,
	h_2
\bigr \rangle_\mathcal{H}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\&= \langle h_1, \Sigma_{YW} \rangle_\mathcal{H} \langle \Sigma_{YW}, h_2 \rangle_\mathcal{H}
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\&= 
\bigl \langle 
h_1,
\,
 \langle \Sigma_{YW}, h_2 \rangle_\mathcal{H} \Sigma_{YW} 
\bigr \rangle_\mathcal{H}
\\ 	&= \langle h_1, \mathcal{U} h_2 \rangle_\mathcal{H}.
\end{align*}
Next, 
	$\mathcal{U}$ is \textit{positive-semidefinite} because for every $h \in \mathcal{H}$, we have
\begin{equation*}
\langle 
	\mathcal{U} h,
	\,
...
\bigl \langle 
	\langle 
		\Sigma_{YW},\, h \rangle_{\mathcal{H}}  \Sigma_{YW}, \, h
\bigr \rangle_\mathcal{H}
	= \langle \Sigma_{YW}, \, h \rangle_\mathcal{H}^2 \ge 0.
\end{equation*}
...
\begin{proof}
	The brief strategy of the proof is as follows.
		We   show that an image of a bounded family in $\mathcal{H}$ under $\mathcal{U}$ is uniformly bounded and equicontinuous.
		Then we apply the Arzel\'{a}-Ascoli theorem to show that $\mathcal{U}$ is \textit{compact}.

Let $\mathcal{B}=\{h \in \mathcal{H}: \Vert h \Vert_\mathcal{H}^2 \le M\}$ denote a bounded family in $\mathcal{H}$ for some constant $0 < M < \infty$.
...
\medskip
\noindent
\textit{Uniform boundedness of $\mathcal{I}$.}~Given a hybrid object $h = (f_1, \ldots, f_K, \mathbf{v}) \in \mathcal{H}$ and an evaluation point $\mathbf{t}= (t_1, \ldots, t_k) \in [0,1]^K$, we denote the $\ell_2$ norm of the evaluated object $\bigl( f_1(t_1), \ldots, f_k(t_k), \mathbf{v} \bigr)$ as follows:
$\bigl(
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \overset{(i)}{=} 
\Vert \langle \Sigma_{YW}, h \rangle_\mathcal{H} \, \Sigma_{YW}(\mathbf{t}) \Vert^2_2
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  =
\langle \Sigma_{YW}, h \rangle_\mathcal{H}^2
\Vert   \Sigma_{YW}(\mathbf{t}) \Vert^2_2.
  \end{equation}
  We will show that $\Vert g (\mathbf{t}) \Vert^2_2 < \infty$ by  showing $\langle \Sigma_{YW}, h \rangle_\mathcal{H}^2 < \infty$ and $\Vert   \Sigma_{YW}(\mathbf{t}) \Vert^2_2 < \infty$ in turn. First, to bound 
   $\langle \Sigma_{YW}, h \rangle_\mathcal{H}^2$, it suffices to bound $\langle \Sigma_{YW}, h \rangle_\mathcal{H}$:
\begin{align*}
\langle \Sigma_{YW}, h \rangle_\mathcal{H}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&=
...
\medskip
\noindent
\textit{Equicontinuity of $\mathcal{I}$.}~Given a hybrid object $h = (f_1, \ldots, f_K, \mathbf{v}) \in \mathcal{H}$ and 
...
\begin{align*}
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&= %%%%%%%%%%%%%%%
	\Vert \langle \Sigma_{YW}, h \rangle_\mathcal{H} 
	\,
	\Sigma_{YW}(\mathbf{t}) - \langle \Sigma_{YW}, h \rangle_\mathcal{H} \Sigma_{YW}( \mathbf{t}^\ast ) \Vert^2_2
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\&=
\langle \Sigma_{YW}, h \rangle_\mathcal{H}^2 \Vert \Sigma_{YW}(\mathbf{t})- \Sigma_{YW}( \mathbf{t}^\ast ) \Vert^2.
\end{align*}
We have already shown in \eqref{squared_ip_bound} that  $\langle \Sigma_{YW}, h \rangle_\mathcal{H}^2 \leq  2K^2 M TQ_1   + 2MpQ_2$.
...
\section{Proof of Theorem \ref{theorem:population_PLS}}\label{section:proof:theorem:population_PLS}
 \begin{proof}
Since Proposition \ref{proposition:compact_operator} implies that $\mathcal{U}$ is compact,
by Theorem 4.3.1 of \citet{hsingTheoreticalFoundationsFunctional2015}, 
\end{proof}
  The singular value decomposition of $\mathcal{C}_{YW}$ is given by
	\begin{equation*}
		\mathcal{C}_{YW} = \sum \limits_{j=1}^\infty \iota_j (f_{1j} \otimes f_{2j}).
	\end{equation*}
	Let $\Vert \cdot \Vert_{op}$ denote an operator norm. Then, applying Theorem 4.3.4 in \citet{hsingTheoreticalFoundationsFunctional2015}, we can show that
	\begin{equation*}
