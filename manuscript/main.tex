...
\end{equation*}
We adopt the following notion for the coefficients:
\begin{equation}
...
The goal is to find a unit-norm direction $\boldsymbol{\xi} \in \mathcal{H}$, whose functional components lie in $\operatorname{span} \bigl( b_1(t), \ldots, b_M(t) \bigr)$, that maximizes the empirical covariance between the resulting PLS component scores $\langle \widetilde{W}_1, \boldsymbol{\xi} \rangle_{\mathcal{H}}, \ldots, \langle \widetilde{W}_n, \boldsymbol{\xi} \rangle_{\mathcal{H}}$ and the responses $y_1, \ldots, y_n$.
...
\begin{equation*}
	\hat{\xi}_j(t) = 
	\sum_{l=1}^M d_{jl} \, b_l(t), \quad j = 1, \ldots, K.
\end{equation*}

Let $\mathbf{d}_j := (d_{j1}, \ldots, d_{jM})^\top \in \mathbb{R}^M$ and $\mathbf{y} := (y_1, \ldots, y_n)^\top$. We compute $\mathbf{d}_1, \ldots, \mathbf{d}_K$ and $\hat{\boldsymbol{\zeta}}$ as follows.
...
\end{equation}
Next, we normalize all components with respect to the hybrid Hilbert norm (see Definition~\ref{def:hilbert_space}) to ensure $\hat{\boldsymbol{\xi}}$ has unit length. Specifically, we define
...
\begin{proposition} \label{Proposition: generalized rayleigh quotient}
	Let $\boldsymbol{\xi} \in \mathcal{H}$ be any unit-norm direction such that its $1$st through $K$th functional components lie in $\operatorname{span} \bigl( b_1(t), \ldots, b_M(t) \bigr)$, respectively.
