...
\jongmin{instead of showing $\mathcal{U}$ is compact, let's show $\mathcal{C}_{YW}$ is compact}If there exist finite constants $Q_1$ and $Q_2$ such that
...
Define $\mathcal{C}_{WY} = \mathbb{E}[Y \otimes W]: \mathbb{R} \rightarrow \mathcal{H}$,
...
Define
	...
	\end{equation*}
		...
		Then $\mathcal{U}$
	 is a self-adjoint, positive-semidefinite, and compact operator.
\end{lemma}
...
\noindent
\textit{Self-adjoint.}~
For any $h_1, h_2 \in \mathcal{H}$, we have
\begin{align*}
\langle 
\mathcal{U} h_1, \, h_2 
\rangle_\mathcal{H}  
