...
\begin{document}
	...
	Given a twice-differentiable basis $\{b_l(t)\}$ of $L^2([0,1])$, the functional predictors and the corresponding regression coefficients can be decomposed into
	\begin{equation*}
...
Since functional predictors are observed at finitely many discrete time points in practice,
we use finite-dimensional projections obtained by the expansion truncating at $M$ terms.
We choose a moderately large value of $M$ (e.g., 15 or 20) to capture rich functional variation without requiring fine-tuning, as our method promotes smoothness through penalization (see Section~\ref{sec: Regularization of PLS Components}). We denote the truncated space by
	\begin{equation}
		\mathcal{B} := \operatorname{span} \bigl( b_1(t), \ldots, b_M(t) \bigr).
	\end{equation}
	 We denote the  predictors projected onto $\mathcal{B}$ and the resulting coefficients as
	\begin{equation*}
	...
	\end{equation}
The observations in the hybrid PLS problem can be compactly represented by the tuple
\begin{equation}\label{def:problem_instance}
	(\mathbb{B}, \mathbb{B}^{\prime \prime}, \Theta, \mathbf{y})
	\in
	\mathbb{R}^{(MK+p) \times (MK+p)}
	\times 
	\mathbb{R}^{(MK+p) \times (MK+p)}
	\times
	\mathbb{R}^{n \times (MK+p)}
	\times
	\mathbb{R}^{n}
	,
\end{equation}
defined as follows.
First, let $B, B^{\prime\prime} \in \mathbb{R}^{M \times M}$ denote the Gram matrices of the basis functions and their second derivatives, respectively, with entries  
\begin{equation}
	B_{m, m'} := \int_0^1 b_m(t)\, b_{m'}(t)\, dt,
	\quad
	B^{\prime\prime}_{m, m'} := \int_0^1 b^{\prime\prime}_m(t)\, b^{\prime\prime}_{m'}(t)\, dt,
\end{equation}
for $m, m' = 1, \ldots, M$.
Leveraging these notations, 
we define the $(MK + p) \times (MK + p)$ block-diagonal matrices
\[
\mathbb{B} := \operatorname{blkdiag}(B, \ldots, B, I_p)
\quad \text{and} \quad
\mathbb{B}^{\prime\prime} := \operatorname{blkdiag}(B^{\prime\prime}, \ldots, B^{\prime\prime}, I_p),
\]
We denote $\Theta := (\Theta_1, \ldots, \Theta_K, Z)
\in
\mathbb{R}^{n \times (MK+p)}
$ , and let $\mathbf{y} := (y_1, \ldots, y_n)^\top$ denote the observed response vector.

	\begin{remark}
While it is possible to use a different basis for each functional predictor, we adopt a common basis for all predictors to simplify the notation. However, the definition of 
$\mathbb{B}$
and
$\mathbb{B}^{\prime \prime}$
 remains general enough to accommodate different bases across the $K$ predictors.
\end{remark}
	...
	The goal is to find a unit-norm direction $\xi \in 
	(\mathcal{B}^K \times \mathbb{R}^p) \subset \mathcal{H}$ that maximizes the squared empirical covariance between the resulting PLS component scores $\langle \widetilde{W}_1, \xi \rangle_{\mathcal{H}}, \ldots, \langle \widetilde{W}_n, \xi \rangle_{\mathcal{H}}$ and the responses $y_1, \ldots, y_n$,
	 defined as
\begin{equation}\label{def:squared_empirical_cov}
	 	\widehat{\textnormal{Cov}}^2 \left( \langle 
	 	\widetilde{W},
	 	\xi \rangle_{\mathcal{H}}, Y \right)
	 	:=
	 	\biggl(
	 	\frac{1}{n} 
	 	\sum_{i=1}^n
...
\bigr)
	\end{equation}
%
	where each functional part $\hat{\xi}_j $  is expressed as
	\begin{equation*}
		\hat{\xi}_j(t) := 
		\sum_{m=1}^M \hat{d}_{jm} \, b_m(t), \quad j = 1, \ldots, K.
	\end{equation*}
...
At the $l$-th iteration of the PLS algorithm, let
$( \hat{\mathbf{d}}_1^\top, \ldots, \hat{\mathbf{d}}_K^\top, \hat{\boldsymbol{\xi}}_{K+1}^\top )^\top$
denote the coefficients of $\hat{\xi}$, defined in \eqref{def:maximizer_squared_empirical_cov},
which corresponds to the projected unit-norm direction that maximizes the squared empirical covariance.
These coefficients are obtained by solving the following generalized eigenvalue problem:
\begin{equation}	\label{eq: Regularized generalized rayleigh quotient equation}
	\left(
	\hat{\mathbf{d}}_1^\top, \ldots, \hat{\mathbf{d}}_K^\top, \hat{\boldsymbol{\xi}}_{K+1}^\top
	\right)^\top
	=
...
\end{proposition}
Since $V$ is a rank 1 matrix, the solution of the generalized eigenvalue problem is easily obtained by matrix multiplication and normalization:
	\begin{proposition} \label{proposition:linear_noregul}
	We can compute  $\hat{\mathbf{d}}_1, \ldots, \hat{\mathbf{d}}_K$ and $\hat{\boldsymbol{\xi}}$ in Proposition \ref{proposition:eigen_noregul} as 
\begin{equation*}
	\hat{\mathbf{d}}_j  = \frac{1}{\sqrt{\sum_{j=1}^K 
			\mathbf{y}^\top
			\Theta_j
...
Proofs of Propositions \ref{proposition:eigen_noregul} and \ref{proposition:linear_noregul} are provided in Appendix~\ref{section:proof:proposition:linear_noregul_system_unregularized}.
...
Although Proposition \ref{proposition:eigen_noregul} efficiently estimates the PLS component direction $\hat{\xi} = (\hat{\xi}_1, \ldots, \hat{\xi}_K, \hat{\boldsymbol{\xi}}_{K+1})$, the functional components $\hat{\xi}_1, \ldots, \hat{\xi}_K$ may not be smooth, potentially leading to overfitting and unstable predictions. To address this, we propose a regularized extension that balances predictive performance with smoothness. Specifically, we penalize the roughness of each $\hat{\xi}_j$ using its integrated squared second derivative $\int_0^1 \bigl\{ \hat{\xi}_j^{\prime\prime}(t) \bigr\}^2 dt$.
...
One possible approach is to extend smoothed functional PCA \citep{Rice1991} to our optimization problem \eqref{def:maximizer_squared_empirical_cov}, leading to the following formulation:
	\begin{equation*}
	\hat{\xi} 
	= 
	(\hat{\xi}_1, \ldots, \hat{\xi}_K, \hat{\boldsymbol{\xi}}_{K+1})
	...
	,
\end{equation*}
where the smoothing parameters $\{\lambda_j\}_{j=1}^K$ control the trade-off between maximizing covariance and penalizing roughness. Here, the penalization term can be written in matrix form as:
		\begin{align*}
	\sum \limits_{k=1}^K 
	\lambda_k 
	\int_0^1 \bigl\{ \hat{\xi}_j^{\prime\prime}(t) \bigr\}^2 dt
...
\\&	 =
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	  \sum \limits_{k=1}^K \lambda_k \mathbf{a}^{(k)T} \ddot{J}^{(k)} \mathbf{a}^{(k)} = \xi^{* \top} \Lambda \ddot{J}^\ast \xi^*,
\end{align*}
However, the above approach assumes that the functional predictors admit an orthogonal expansion in the $L^2$ sense. As an alternative, we adopt the strategy of \citet{Silverman1996}, which works under weaker conditions than those required by \citet{Rice1991}. This is achieved by replacing the standard orthonormality constraint with one based on a modified inner product that accounts for roughness. Building on this idea, the following proposition reformulates the hybrid PLS problem as a generalized Rayleigh quotient, yielding a smoothed PLS direction that maximizes the squared covariance over a class of smooth functions.
...
\end{proposition}
	...
	\begin{proposition} \label{Proposition: Regularized generalized rayleigh quotient}
		...
		Note that the penalty term can be written concisely using matrices as:
		\begin{equation}
			\textnormal{PEN} \left( \hat{\xi}_j \right) 
			=  \int_0^1 
			\bigl\{ \hat{\xi}_j^{''}(t) \bigr\}^2 dt = \mathbf{a}^{(k)T} \ddot{J}^{(k)} \mathbf{a}^{(k)},
		...
		\begin{align}
			\begin{split}
				&\arg \max \limits_{\xi^* \in \mathbb{R}^{MK+p}} \; \frac{\xi^{*\top} \mathbf{V}^\ast \xi^*}{\xi^{*\top}(J^\ast+\Lambda \ddot{J}^\ast) \xi^*} , \quad \textit{or equivalently}, \\ 
				&\arg \max \limits_{\xi^* \in \mathbb{R}^{MK+p}} \;  \xi^{*\top} \mathbf{V}^\ast \xi^* \quad \textnormal{subject to}  \quad \xi^{*\top} (J^\ast+\Lambda \ddot{J}^\ast) \xi^*=1.
		...
		\begin{equation*}
			\widehat{\xi}[\mathbf{t}] = [\widehat{\psi}(\mathbf{t})^\top, \widehat{\boldsymbol{\theta}}^\top]^\top =  \mathbb{B}[\mathbf{t}]^\top \widehat{\xi}^\ast.   
		\end{equation*}
	...
	\section{Proof of Proposition \ref{proposition:eigen_noregul} }\label{section:proof:proposition:linear_noregul_system_unregularized}
	\begin{comment}
		We start by defining a  inner product in the product space $(\mathbb{R}^M)^K \times \mathbb{R}^p$:
		\begin{lemma}[Inner product of tuples]
			\label{lemma:valid_inner_product_tuple}
			Let $B \in \mathbb{R}^{M \times M}$ be a symmetric positive semi-definite matrix. 
			For any two tuples $u = (\mathbf{d}_1, \ldots, \mathbf{d}_K, \boldsymbol{\zeta})$ and $v = (\mathbf{g}_1, \ldots, \mathbf{g}_K, \boldsymbol{\omega})$ in the product space $(\mathbb{R}^M)^K \times \mathbb{R}^p$, define the bilinear form
			\[
			\langle u, v \rangle_{B} := \sum_{j=1}^K \mathbf{d}_j^\top B \mathbf{g}_j + \boldsymbol{\zeta}^\top \boldsymbol{\omega}.
			\]
			Then $\langle \cdot, \cdot \rangle_{B}$ is a valid inner product on $(\mathbb{R}^M)^K \times \mathbb{R}^p$.
		\end{lemma}
		The proof of Lemma \ref{lemma:valid_inner_product_tuple} is provided in Appendix \ref{section:proof:lemma:valid_inner_product_tuple}
	\end{comment}
	\begin{proof}
	...
	\end{equation*}
	with coefficient vectors $\mathbf{d}_j := (d_{j1}, \ldots, d_{jM})^\top \in \mathbb{R}^M$.
	Then, the squared norm of $\xi$ in the hybrid Hilbert space $\mathcal{H}$ 
	...
	\begin{align*}
		...
		\boldsymbol{\zeta}^\top \boldsymbol{\zeta},
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		%\\&=\bigl \langle(\mathbf{d}_1, \ldots, \mathbf{d}_K, \boldsymbol{\zeta}),(\mathbf{d}_1, \ldots, \mathbf{d}_K, \boldsymbol{\zeta})\bigr\rangle_B.
	\end{align*}
	In a similar manner, the empirical covariance is
	% expressed as the inner product on  defined in Lemma \ref{lemma:valid_inner_product_tuple} as follows:
	computed as follows:
	\begin{align*}
		...
		%	\frac{1}{n}
		%\bigl \langle
		%(\Theta_1^\top \mathbf{y} , \ldots, \Theta_K^\top %\mathbf{y}, Z^\top \mathbf{y}),
		%(\mathbf{d}_1, \ldots, \mathbf{d}_K, %\boldsymbol{\zeta})
		%\bigr\rangle_B.
	\end{align*}	
	Therefore, problem of maximizing the squared covariance  is equivalent to the following constrained optimization problem on the Euclidean space:
	\begin{align*}
		\max_{\mathbf{d}_1, \ldots, \mathbf{d}_K \in \mathbb{R}^M,
		...
		Z \boldsymbol{\zeta} 
		\biggr)^2,
		\numberthis \label{optim:summation_form}
		\\&~s.t.~	\sum \limits_{j=1}^K 
		\mathbf{d}_j^\top
		B
		\mathbf{d}_j
		+
	...
	\end{align*} 
The objective function matches that of \eqref{eq: Regularized generalized rayleigh quotient equation}, taking the form of a quadratic expression involving the matrix $V$ defined in \eqref{def:V_matrix}:
\begin{align*}
	\frac{1}{n^2}
\biggl(
\sum \limits_{k=1}^K 
...
\boldsymbol{\xi},
\end{align*}
and the constraint is identical to that in \eqref{eq: Regularized generalized rayleigh quotient equation}, because we have:
\begin{equation*}
		\sum \limits_{j=1}^K 
	\mathbf{d}_j^\top
	B
...
\end{equation*}
Therefore, the optimization problem \eqref{optim:summation_form}, which is equivalent to the squared covariance maximization problem, reduces to the generalized eigenvalue problem \eqref{eq: Regularized generalized rayleigh quotient equation}. This completes the proof of Proposition~\ref{proposition:eigen_noregul}.
 \end{proof}
 
 \section{Proof of Proposition  \ref{proposition:linear_noregul}}
 \begin{proof}
	From \eqref{optim:summation_form}, let us denote the objective function as
	\[
	f(\{\mathbf{d}_j\}, \boldsymbol{\zeta}) := 
	\frac{1}{n^2}
	\left(
	\sum_{j=1}^K \mathbf{y}^\top \Theta_j B \mathbf{d}_j + \mathbf{y}^\top Z \boldsymbol{\zeta}
	\right)^2,
	\]
	and the constraint function as
	\[
	g(\{\mathbf{d}_j\}, \boldsymbol{\zeta}) := \sum_{j=1}^K \mathbf{d}_j^\top B \mathbf{d}_j + \boldsymbol{\zeta}^\top \boldsymbol{\zeta} - 1 = 0.
	\]
	The feasible set is compact, as it corresponds to the boundary of an ellipsoid in a finite-dimensional Euclidean space. Since the objective function $f$ is continuous and the constraint defines a compact feasible set, the  Weierstrass Extreme Value Theorem (see, for example, Theorem 2.3.1 in \citealt{bazaraaNonlinearProgrammingTheory2006}) ensures the existence of a global maximizer.
	
	Define the Lagrangian:
	\[
	\mathcal{L}(\{\mathbf{d}_j\}, \boldsymbol{\zeta}, \lambda) 
	:= 
	f(\{\mathbf{d}_j\}, \boldsymbol{\zeta}) - \lambda g(\{\mathbf{d}_j\}, \boldsymbol{\zeta}).
	\]
	The constraint function $g$ is continuously differentiable, and its gradient vanishes only at the origin. Since the origin is not feasible (the constraint requires norm 1), the gradient of $g$ is nonzero at all feasible points. Thus, the linear independence constraint qualification (LICQ) holds, ensuring that the KKT conditions are necessary for local maxima (see, for example, Theorem 5.3.1 in \citealt{bazaraaNonlinearProgrammingTheory2006}).
	Taking gradients of $\mathcal{L}$ and setting them to zero yields:
	\begin{align*}
		\nabla_{\mathbf{d}_j} \mathcal{L} &= 
		\frac{2}{n^2} 
		\left(
		\sum_{\ell=1}^K \mathbf{y}^\top \Theta_\ell B \mathbf{d}_\ell + \mathbf{y}^\top Z \boldsymbol{\zeta}
		\right)
		B \Theta_j^\top \mathbf{y}
		- 2\lambda B \mathbf{d}_j = 0, \quad \forall j, 
		\numberthis \label{lag_grad_d}
		\\
		...
		\right)
		Z^\top \mathbf{y}
		- 2\lambda \boldsymbol{\zeta} = 0.
		\numberthis \label{lag_grad_zeta}
	\end{align*}
	By pre-multiplying equation \eqref{lag_grad_d} by $B^{-1}$ and observing that the scalar quantity $n^{-2}\sum_{\ell=1}^K \mathbf{y}^\top \Theta_\ell B \mathbf{d}_\ell + \mathbf{y}^\top Z \boldsymbol{\zeta}$ appears in both \eqref{lag_grad_d} and \eqref{lag_grad_zeta}, the gradient conditions together imply that any local optimum must be  scalar multiples of
	$
	\Theta_1^\top \mathbf{y}, \ldots, \Theta_K^\top \mathbf{y}, Z^\top \mathbf{y},
	$
	all scaled by the same factor. There exists only one set of vectors  that also satisfies the primal equality constraint, namely the normalized version of the vector set above. The global maximizer is therefore given by
	\begin{equation*}
		\mathbf{d}_j^\ast = \frac{1}{\sqrt{\sum_{j=1}^K 
				\mathbf{y}^\top
				\Theta_j
				B \Theta_j^\top \mathbf{y} 
				+ 
		...
		\quad
		%%%%%%%%%%%%%%%%
		\boldsymbol{\zeta}^\ast = \frac{1}{\sqrt{\sum_{j=1}^K 
				\mathbf{y}^\top
				\Theta_j
				B \Theta_j^\top \mathbf{y} 
				+ 
		...
		Z^\top \mathbf{y},
	\end{equation*}
	which are the same as  $\hat{\mathbf{d}}_1, \ldots, \hat{\mathbf{d}}_K$ and $\hat{\boldsymbol{\xi}}$. This completes the proof of  Proposition \ref{proposition:linear_noregul}.
	\end{proof}
