...
%
	...
	\begin{proposition}\label{proposition:generalized_rayleigh_quotient}
...
of $	\hat{\xi}$, 
defined in \eqref{def:maximizer_squared_empirical_cov},
which is
the projected unit-norm direction that maximizes the squared empirical covariance.

 These coefficients can be obtained by solving the following generalized eigenvalue problem
\begin{align}
	\left(
...
Since $V$ is a rank 1 matrix, the solution of the generalized eigenvalue problem is easily obtained by matrix multiplication and normalization:
	\begin{proposition} \label{proposition:generalized_rayleigh_quotient_linear}
	We can compute  $\hat{\mathbf{d}}_1, \ldots, \hat{\mathbf{d}}_K$ and $\hat{\boldsymbol{\zeta}}$ in Proposition \ref{proposition:generalized_rayleigh_quotient} as 
\begin{equation*}
...
Proofs of Propositions \ref{proposition:generalized_rayleigh_quotient} and \ref{proposition:generalized_rayleigh_quotient_linear} are provided in Appendix~\ref{section:proof:proposition:generalized_rayleigh_quotient_linear_system_unregularized}.
Although the unnormalized coefficients are computed separately for the functional and scalar part of the PLS component direction, the normalization step couples them, allowing the components to influence each other. This enables the procedure to account for the correlation between functional and scalar components.
	...
	Although Proposition \ref{proposition:generalized_rayleigh_quotient} provides an efficient and principled way to compute the PLS components $\xi$, their functional part $\psi=(\psi^{(1)}, \ldots, \psi^{(K)})$ may not be smooth, which can lead to overfitting and unstable predictions. Thus, in this section, we provide a modification of Proposition  \ref{proposition:generalized_rayleigh_quotient} that yields regularized (smoothed) PLS components.
	...
	\subsection{Proof of Proposition \ref{proposition:generalized_rayleigh_quotient_linear_system_unregularized}}\label{section:proof:proposition:generalized_rayleigh_quotient_linear_system_unregularized}
	\begin{comment}
	...
	\begin{align*}
		...
		%\bigr\rangle_B.
	\end{align*}	
	Therefore, problem of maximizing the squared covariance can be written as
	\begin{comment}
		\begin{equation*}
	...
	presented in Proposition \ref{proposition:generalized_rayleigh_quotient_linear_system_unregularized} is equivalent to the following constrained optimization problem on the Euclidean space:
	\begin{align*}
	...
	\begin{equation*}
		...
		Z^\top \mathbf{y},
	\end{equation*}
	which are the same as  $\hat{\mathbf{d}}_1, \ldots, \hat{\mathbf{d}}_K$ and $\hat{\boldsymbol{\zeta}}$. This completes the proof of  Proposition \ref{proposition:generalized_rayleigh_quotient_linear_system_unregularized}.
	...
	\subsection{Proof of Propositions \ref{proposition:generalized_rayleigh_quotient} and \ref{Proposition: Regularized generalized rayleigh quotient} }\label{section:proof:eigenproblem_derivation}
	%
	%
	\begin{align*}
		\widehat{\textnormal{Cov}} (
