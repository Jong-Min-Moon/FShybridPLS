...
\begin{document}
	...
	\end{equation}
where $Y_i$ is a scalar response, $\mathbf{Z}_i=(Z_{i1}, \ldots, Z_{ip})^\top$ is a Euclidean vector covariate, $X_{i1}(t), \ldots, X_{iK}(t)$ are functional predictors that belong to $L^2[0,1]$, and $\epsilon_i$ is observational noise.
In other words, this is a scalar-on-hybrid regression where the hybrid covariates belong to $L^2[0,1]^{\otimes K} \times \mathbb{R}^p$.
...
%	and possess second  derivatives.
	...
	\section{Background on partial least squares and its extension to hybrid predictors} \jongmin{still revising}
...
\end{equation*}
 where the two $\widehat{\operatorname{Cov}}^2$ denote sample cross-covariance and sample covariance, respectively. A standard algorithm for solving this problem, called nonlinear iterative partial least squares (NIPALS) is presented in Algorithm \ref{scalar_pls}.
 \begin{algorithm} 
 	\caption{Scalar partial least squares }\label{scalar_pls}
 	\begin{algorithmic}[1]
 		...
 		\State $\widehat{\boldsymbol{\xi}}_l \leftarrow
 		...
 		$
 		\Comment{PLS direction}\label{possible?}
 		\State 
 		$
 		\widehat{\rho}_i^{[l]} \leftarrow \langle \hat{\boldsymbol{\xi}}_l,   \mathbf{Z}_i \rangle, 
 		~i=1,  \ldots, n
 		$ \Comment{PLS score}
  		\\
 \textbf{Residualization:}
 		...
 		%
 		\State $ \widehat{\boldsymbol{\delta}}^{[l]}  \leftarrow \frac{1}{\sum_{i=1}^n \widehat{\rho}_{i}^{[l]2}}\sum_{i=1}^n  \widehat{\rho}_{i}^{[l]}\mathbf{Z}_i^{[l]}.
 	$
...
We can express the predictor
 $W_i$ 
 and the regression coefficient $\beta$ using an orthonormal basis 
 $\psi_1, \psi_2, \ldots$ of $\mathcal{H}$, 
so that
$W_i = \sum_j \langle W_i, \psi_j \rangle_{\mathcal{H}}\, \psi_j $	
 and
  $\beta = \sum_j \langle \beta, \psi_j \rangle_{\mathcal{H}}\, \psi_j $.
To make the problem tractable, we truncate these expansions to the first  $L$ terms. 
However, if the basis is chosen without considering the data, the most important regression information may not be captured within these first $L$ terms.
The  core idea of partial least squares (PLS) is simultaneously learn a data-driven, regression-aware orthonormal basis while also performing the regression.
This results in a  simultaneous  decomposition of the predictor and outcome in terms of uncorrelated PLS scores 
\begin{equation}	\label{eq: decomposition}
	W_i = \sum \limits_{l=1}^{\infty} \rho_{l}  \delta_l  \quad \textnormal{and} \quad Y_i = \sum \limits_{l=1}^{\infty}\rho_{l} \nu_l + \epsilon_i,
	\end{equation}
	where $(\boldsymbol{\delta}_l)_{l \in \mathbb{N}} \in \mathcal{H}$ and $(\nu_l)_{l \in \mathbb{N}} \in \mathbb{R}$ are appropriate bases.
	
 
		
	\section{Population PLS and Tucker's Criterion in hybrid regression setting}\label{section:main:tucker}\jongmin{Previously, was in theoretical property section, now procedes the method section, to motivate the estimation procedure}
...
Under a mild assumption,
	...
	\begin{definition}[Hybrid predictor and inner product]\label{def:hilbert_space}
		...
		\begin{equation}		\label{eq: hybrid inner product}
			...
			~\text{where}~\int_0^1 X_{1k}(t) X_{2k}(t) \, dt
		\end{equation}
		for $\omega > 0$.
	...
	\end{remark}  
	Then the joint regression model \eqref{eq: functional model}  can be concisely written as
	\begin{equation}		\label{eq: Hybrid functional model}
		Y_i = \langle \beta, W_i \rangle_{\mathcal{H}} + \epsilon_i,~\text{where}~\beta := \bigl( \beta_1(t), \ldots, \beta_K(t), \boldsymbol{\beta} \bigr)\in \mathcal{H}.
	\end{equation}
...
\begin{equation*}
	\Vert \mathcal{C}_{YW} \Vert_{\mathrm{op}} = \sup_{\substack{h \in \mathcal{H} \\ \Vert h \Vert_\mathcal{H}=1}} | \mathcal{C}_{YW} h |^2 
	= \sup_{\substack{h \in \mathcal{H} \\ \Vert h \Vert_\mathcal{H}=1}} | E(\langle W, h \rangle_\mathcal{H} Y)|^2 
	= \sup_{\substack{h \in \mathcal{H} \\ \Vert h \Vert_\mathcal{H}=1}} \textnormal{Cov}^2(\langle W, h \rangle_\mathcal{H}, Y) 
	= \kappa_1^2,
\end{equation*}
