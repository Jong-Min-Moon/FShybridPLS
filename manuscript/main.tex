...
\begin{equation}\label{def:truncated_hilbert_space}
	\widetilde{ \mathcal{H} } := \mathcal{B}^K \times \mathbb{R}^p \in \mathcal{H}.
\end{equation}
...
\end{remark}
	...
	\subsubsection{Core strategy for PLS component direction estimation}\label{sec: pls component computation intermediate}
Building on the basis function approximation in Section~\ref{sec: finite basis approximation}, we now present the core strategy for directly estimating the hybrid PLS component direction. This method fully leverages both the continuous nature of the functional components and the function-scalar hybrid structure, and can be implemented via simple matrix-vector multiplications.
	
We describe the strategy at the $l$-th iteration. For simplicity, we omit the iteration index and assume the observations have been residualized from previous steps (see Section~\ref{subsec: Step 2: Computing Predictors via Hybrid-on-Scalar Regression} for details). The  PLS component direction is estimated by the unit-norm direction $\xi \in \tilde{\mathcal{H}}$ that maximizes the squared empirical covariance, which quantifies the linear dependence between the PLS scores $\langle \widetilde{W}_1, \xi \rangle_{\mathcal{H}}, \ldots, \langle \widetilde{W}_n, \xi \rangle_{\mathcal{H}}$ and the responses $y_1, \ldots, y_n$, defined as
