...
\begin{equation}\label{def:squared_empirical_cov}
	 	\widehat{\textnormal{Cov}}^2 \left( \langle \widetilde{\mathbf{W}}, \xi \rangle_{\mathcal{H}}, Y \right)
	 	:=
	 	\biggl(
	 	\frac{1}{n} 
	 	\sum_{i=1}^n
...
:= \arg \max_{\xi \in (\mathcal{B}^K \times \mathbb{R}^p) \subset \mathcal{H}} \widehat{\textnormal{Cov}}^2 \left( \langle \widetilde{\mathbf{W}}, \xi \rangle_{\mathcal{H}}, Y \right)
	\end{equation}
%
...
which is
the projected unit-norm direction that maximizes the squared empirical covariance. These coefficients can be obtained by solving the following generalized eigenvalue problem
\begin{align}
	\left(
...
$$
where
$B^\ast := \operatorname{blkdiag}(B, \ldots, B, I_p)  \in  \mathbb{R}^{(MK+p) \times (MK+p)}$,
 $\Theta := (\Theta_1, \ldots, \Theta_K, Z) \in \mathbb{R}^{n \times (MK + p)}$,
$B \in \mathbb{R}^{M \times M}$ is the gram matrix  of the basis functions  with entries
$
...
Although the unnormalized coefficients are computed separately for the functional and scalar part of the PLS component direction, the normalization step couples them, allowing the components to influence each other. This enables the procedure to account for the correlation between functional and scalar components.
	...
	\[
	\mathcal{L}(\{\mathbf{d}_j\}, \boldsymbol{\zeta}, \lambda) = f(\{\mathbf{d}_j\}, \boldsymbol{\zeta}) - \lambda g(\{\mathbf{d}_j\}, \boldsymbol{\zeta}).
	\]
	...
	\end{align*}
	By pre-multiplying equation \eqref{lag_grad_d} by $B^{-1}$ and observing that the scalar quantity $\sum_{\ell=1}^K \mathbf{y}^\top \Theta_\ell B \mathbf{d}_\ell + \mathbf{y}^\top Z \boldsymbol{\zeta}$ appears in both \eqref{lag_grad_d} and \eqref{lag_grad_zeta}, the gradient conditions together imply that any local optimum must be  scalar multiples of
	$$
	\Theta_1^\top \mathbf{y}, \ldots, \Theta_K^\top \mathbf{y}, Z^\top \mathbf{y},
	$$
