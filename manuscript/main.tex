...
by proposing a hybrid partial least square (PLS) regression framework that integrates functional and vector predictors in a principled and coherent manner. Before we summarize the key contributions of this new framework, we will first discuss previous approaches and their limitations.
	...
	A widespread approach in this line is the principal component analysis (PCA) regression.
	It is a two-step approach, applying (multivariate) functional PCA  to the functional predictors, possibly with standard PCA to the scalar predictors,
	followed by a classical multivariate regression on the resulting scores.
	PCA regression is well-studied for functional predictors alone \citep{hallMethodologyConvergenceRates2007, reissFunctionalPrincipalComponent2007, febrero-bandeFunctionalPrincipalComponent2017}.
...
In terms of predictive power, partial least squares (PLS) regression is a powerful alternative. This two-step approach first iteratively constructs a set of orthogonal latent components from the predictors that have the maximal covariance with the response variable. This low-dimensional representation is then used to fit a linear regression model.
PLS for linear regression in the context of functional data was initiated by \citet{predaPLSRegressionStochastic2005} for signle predictor.
  PLS has been extensively studied , both for single and multiple functional predictors \citep{, reissFunctionalPrincipalComponent2007, aguileraUsingBasisExpansions2010, delaigleMethodologyTheoryPartial2012, aguileraPenalizedVersionsFunctional2016, febrero-bandeFunctionalPrincipalComponent2017, beyaztasRobustFunctionalPartial2022, saricamPartialLeastsquaresEstimation2022, mutisFunctiononfunctionLinearQuantile2025}.
