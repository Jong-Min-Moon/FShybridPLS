...
\end{equation}
defined as follows.
Let $B, B^{\prime\prime} \in \mathbb{R}^{M \times M}$ denote the Gram matrices of the basis functions and their second derivatives, respectively, with entries  
\begin{equation}
...
Define the $(MK + p) \times (MK + p)$ block-diagonal matrices
\[
...
\]
and let $\Theta := (\Theta_1, \ldots, \Theta_K, Z)$ denote the combined design matrix. Finally, let $\mathbf{y} := (y_1, \ldots, y_n)^\top$ denote the observed response vector.

	\begin{remark}
While it is possible to use a different basis for each functional predictor, we adopt a common basis for all predictors to simplify the notation. However, the definition of $\mathbb{B}$ remains general enough to accommodate different bases across the $K$ predictors.
\end{remark}
