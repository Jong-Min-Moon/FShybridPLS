...
Given a twice-differentiable basis $\{b_l(t)\}$ of $L^2([0,1])$,
the functional predictors,
the corresponding regression coefficients,
and PLS component direction
 can be decomposed into
	\begin{equation*}
		X_{ij}(t) = \sum_{l=1}^\infty \theta_{ijl} \, b_l(t),~
		\beta_j(t) = \sum_{l=1}^\infty \eta_{0,jl} \, b_l(t), j=1, \ldots, K,~
		%%%%%%%%%%%%%%%%%%%%%%
		\xi 
		=
		\sum_{l=1}^\infty 
...
Since $V$ is a rank 1 matrix, the solution of the generalized eigenvalue problem is easily obtained by matrix multiplication and normalization:
	...
	We can compute  $\hat{\boldsymbol{\gamma}}_1, \ldots, \hat{\boldsymbol{\gamma}}_K$ and $\hat{\boldsymbol{\xi}}$ in Proposition \ref{proposition:eigen_noregul} as 
\begin{equation*}
...
\end{align*}
where we recall that $\mathbb{B}^{\prime \prime} $ is defined in \eqref{def:gram_block}, 
...
\end{proposition}
	...
	\begin{comment}
		...
		\begin{lemma}[Inner product of tuples]
			...
			For any two tuples $u = (\boldsymbol{\gamma}_1, \ldots, \boldsymbol{\gamma}_K, \boldsymbol{\zeta})$ and $v = (\mathbf{g}_1, \ldots, \mathbf{g}_K, \boldsymbol{\omega})$ in the product space $(\mathbb{R}^M)^K \times \mathbb{R}^p$, define the bilinear form
			\[
			\langle u, v \rangle_{B} := \sum_{j=1}^K \boldsymbol{\gamma}_j^\top B \mathbf{g}_j + \boldsymbol{\zeta}^\top \boldsymbol{\omega}.
			\]
	...
	\begin{align*}
		...
		\boldsymbol{\zeta}^\top \boldsymbol{\zeta},
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		%\\&=\bigl \langle(\boldsymbol{\gamma}_1, \ldots, \boldsymbol{\gamma}_K, \boldsymbol{\zeta}),(\boldsymbol{\gamma}_1, \ldots, \boldsymbol{\gamma}_K, \boldsymbol{\zeta})\bigr\rangle_B.
	\end{align*}
	...
	\begin{align*}
		...
		%(\boldsymbol{\gamma}_1, \ldots, \boldsymbol{\gamma}_K, %\boldsymbol{\zeta})
		%\bigr\rangle_B.
	\end{align*}	
...
Therefore, the optimization problem \eqref{optim:summation_form}, which is equivalent to the squared covariance maximization problem, reduces to the generalized eigenvalue problem \eqref{eq: Regularized generalized rayleigh quotient equation}. This completes the proof of Proposition~\ref{proposition:eigen_noregul}.
 ...
 \begin{proof}
	...
	\[
	f(\{\boldsymbol{\gamma}_j\}, \boldsymbol{\zeta}) := 
	\frac{1}{n^2}
	\left(
	\sum_{j=1}^K \mathbf{y}^\top \Theta_j B \boldsymbol{\gamma}_j + \mathbf{y}^\top Z \boldsymbol{\zeta}
	\right)^2,
	\]
	and the constraint function as
	\[
	g(\{\boldsymbol{\gamma}_j\}, \boldsymbol{\zeta}) := \sum_{j=1}^K \boldsymbol{\gamma}_j^\top B \boldsymbol{\gamma}_j + \boldsymbol{\zeta}^\top \boldsymbol{\zeta} - 1 = 0.
	\]
	...
	\[
	\mathcal{L}(\{\boldsymbol{\gamma}_j\}, \boldsymbol{\zeta}, \lambda) 
	:= 
	f(\{\boldsymbol{\gamma}_j\}, \boldsymbol{\zeta}) - \lambda g(\{\boldsymbol{\gamma}_j\}, \boldsymbol{\zeta}).
	\]
	...
	\begin{align*}
		...
		\frac{2}{n^2} 
		\left(
		\sum_{\ell=1}^K \mathbf{y}^\top \Theta_\ell B \boldsymbol{\gamma}_\ell + \mathbf{y}^\top Z \boldsymbol{\zeta}
		\right)
		B \Theta_j^\top \mathbf{y}
		- 2\lambda B \boldsymbol{\gamma}_j = 0, \quad \forall j, 
	...
	\end{align*}
	By pre-multiplying equation \eqref{lag_grad_d} by $B^{-1}$ and observing that the scalar quantity $n^{-2}\sum_{\ell=1}^K \mathbf{y}^\top \Theta_\ell B \boldsymbol{\gamma}_\ell + \mathbf{y}^\top Z \boldsymbol{\zeta}$ appears in both \eqref{lag_grad_d} and \eqref{lag_grad_zeta}, the gradient conditions together imply that any local optimum must be  scalar multiples of
	$
	\Theta_1^\top \mathbf{y}, \ldots, \Theta_K^\top \mathbf{y}, Z^\top \mathbf{y},
	$
	...
	\begin{equation*}
		\boldsymbol{\gamma}_j^\ast = \frac{1}{\sqrt{\sum_{j=1}^K 
				\mathbf{y}^\top
				\Theta_j
				B \Theta_j^\top \mathbf{y} 
				+ 
		...
		Z^\top \mathbf{y},
	\end{equation*}
	which are the same as  $\hat{\boldsymbol{\gamma}}_1, \ldots, \hat{\boldsymbol{\gamma}}_K$ and $\hat{\boldsymbol{\xi}}$. This completes the proof of  Proposition \ref{proposition:linear_noregul}.
	\end{proof}
