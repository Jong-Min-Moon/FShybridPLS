...
Define $\mathcal{C}_{YW}= \mathbb{E}(W_1 \otimes_\mathcal{H} Y_1): \mathcal{H} \rightarrow \mathbb{R}$,
which	maps  
...
To guarantee the existence of a complete orthonormal system of eigenfunctions for $\mathcal{U}$ in $\mathcal{H}$, we  first demonstrate that $\mathcal{U}$ is  a compact operator,
that is,
images of a bounded sequence contains a convergent subsequence (for a formal definition, see Definition 4.1.1 of \citealt{hsingTheoreticalFoundationsFunctional2015}).
...
\begin{theorem}[Population PLS]\label{proposition: eigenanalysis} Under the conditions of Proposition \ref{proposition:compact_operator}, the constrained maximum
	\begin{equation*} 
