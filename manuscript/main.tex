...
\end{equation*}
Then using \eqref{algorithm_step:residualization_predictor:recall}, we have, with some abuse of notation,
\begin{equation*}
\widetilde{\boldsymbol{W}}^{[l_1]}  = 
...
\biggl(
	...
	\biggr)
		\widetilde{\boldsymbol{W}}^{[l_1-1]},
\end{equation*}
Clearly, the right-hand side represents a linear operator from $\tilde{\mathcal{H}}^{\otimes n}$ to $\tilde{\mathcal{H}}^{\otimes n}$, which we denote as $P^{[l_1-1]}$.
	Thus, 
	we have $\widetilde{\boldsymbol{W}}^{[l_1]}  = 
	P^{[l_1-1]} \widetilde{\boldsymbol{W}}^{[l_1-1]}$.
Using this relationship, we have
	\begin{align*}
		\widetilde{\boldsymbol{W}}^{[l_1]} &=P^{[l_1-1]} \widetilde{\boldsymbol{W}}^{[l_1-1]} \\
		&=
		P^{[l_1-1]} P^{[l_1-2]} \widetilde{\boldsymbol{W}}^{[l_1-2]}
		  \\
		&= P^{[l_1-1]} P^{[l_1-2]} \ldots P^{[l_2 + 1]}  P^{[l_2]} \widetilde{\boldsymbol{W}}^{[l_2]}
		  \\
&=P P^{[l_2]} \widetilde{\boldsymbol{W}}^{[l_2]},		
	\end{align*}
	where $P = P^{[l_1-1]} P^{[l_1-2]} \ldots P^{[l_2 + 1]} $ is a linear operator from  $\tilde{\mathcal{H}}^{\otimes n}$.
	Then for $j < l$,
	\begin{align*}
