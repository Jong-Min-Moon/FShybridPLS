...
\begin{proposition} \label{Proposition: generalized rayleigh quotient}
	Let $\boldsymbol{\xi} \in \mathcal{H}$ be any unit-norm direction such that its $1$st through $K$th functional components lie in $\operatorname{span}(\Phi^{(1)}), \ldots, \operatorname{span}(\Phi^{(K)})$, respectively.
	Then, the empirical covariance between the projected component $\langle \widetilde{\mathbf{W}}, \boldsymbol{\xi} \rangle_{\mathcal{H}}$ and the response $Y$, approximated in terms of the basis, is given by
	\begin{equation*}
		\widehat{\textnormal{Cov}} \left( \langle \widetilde{\mathbf{W}}, \boldsymbol{\xi} \rangle_{\mathcal{H}}, Y \right)
		:=
		\frac{1}{n} 
		\mathbf{y}^\top 
		\langle \widetilde{\mathbf{W}}, \boldsymbol{\xi} \rangle_{\mathcal{H}},
	\end{equation*}
where right-hand side notation comes from Definition \ref{def:n_one_inner_product}.
Then the PLS direction 
$	\hat{\boldsymbol{\xi}}$ 
 computed in \eqref{def:computed_PLS_direction} maximizes $\widehat{\textnormal{Cov}} (\langle \widetilde{\mathbf{W}}, \boldsymbol{\xi} \rangle_{\mathcal{H}}, Y )$.
	\end{proposition}
