...
%where $\boldsymbol{\alpha}=(\alpha_1, \ldots, \alpha_p)^\top \in \mathbb{R}^p$ and $\beta=(\beta^{(1)}, \ldots, \beta^{(K)}) \in \mathcal{F}$ are respectively the regression coefficient vector and function that characterize the effect of scalar and functional predictors on the outcome.
	...
	Let ${b_m(t)}$ be a twice-differentiable basis of $L^2([0,1])$ whose second derivatives are also linearly independent, for example, cubic B-splines, the Fourier basis, or an orthonormal polynomial basis of degree greater than three. Using this basis, the $j$th functional predictor, regression coefficient,  PLS direction (suppressing iteration indices) are represented as follows:
	\begin{equation*}
...
The $l$-th iteration's second step involves orthogonalization. This is achieved by fitting a hybrid-on-scalar regression model, $$\widetilde{W}^{[l]}_i = \hat{\rho}^{[l]}\, \delta^{[l]} + E_i^{[l]},$$ and then taking the residuals. Here, $\delta^{[l]}$ and $E_i^{[l]}$ are the regression coefficient and error terms, respectively, both elements of $\tilde{\mathcal{H}}$ defined in \eqref{def:truncated_hilbert_space}. In the same spirit as in Section \ref{sec: Regularization of PLS Components}, we want the estimated regression coefficieint $\hat{\delta}^{[l]} 
= 
(\hat{\delta}^{[l]}_1, \ldots, \hat{\delta}^{[l]}_K, \hat{\boldsymbol{\chi}}^{[l]} )$ 
have  smooth functional parts, for the whole PLS estimation proecedure to be stable.
Therefore, we obtain $\delta^{[l]}$ by minimizing the followign penaluized least square criteria, where the functional part is penalized by 
 integeraed squared second derivative defined in \eqref{def:roughness_penalty}:
 \begin{align*}
