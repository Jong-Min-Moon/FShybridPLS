...
\end{equation*}
	...
	%
	For each $k = 1, \ldots, K$, let $\mathbf{J}^{(k)} \in \mathbb{R}^{M \times M}$ denote the Gram matrix of the basis functions associated with the $k$-th functional predictor, defined by
	\begin{equation}\label{def:gram_basis}
		\mathbf{J}^{(k)} = \left[ \int_{\mathcal{T}k} b_m^{(k)}(t) \, b_n^{(k)}(t) , dt \right]_{m,n=1}^M.
	\end{equation}
	Using this definition, we construct the following block-diagonal matrices:
	\begin{equation}\label{def:J_and_J_star}
		\mathbf{J} = \mathrm{blkdiag}(\mathbf{J}^{(1)}, \cdots, \mathbf{J}^{(K)}) \in \mathbb{R}^{MK \times MK}~\text{and}~\mathbf{J}^\ast=\mathrm{blkdiag}(\mathbf{J}, \mathbf{I}_p) \in \mathbb{R}^{(MK+p) \times (MK+p)}.
	\end{equation}
...
We now describe the strategy for computing the PLS direction at the $l$-th iteration. For simplicity, we omit the iteration index and denote the residualized response by $Y$, with finite-sample observations given by $\mathbf{y} = (y_1, \ldots, y_n) \in \mathbb{R}^n$. Let $\widetilde{W} \in \mathcal{H}$ denote the residualized, basis-approximated hybrid predictor, and let $\widetilde{\mathbf{C}} \in \mathbb{R}^{n \times (MK)}$ denote the basis coefficients of its functional part in the finite-sample observations. Our strategy seeks the direction $\boldsymbol{\xi} \in \mathcal{H}$, lying in the span of the basis $B^*[\mathbf{t}]$, that maximizes the empirical covariance between the resulting PLS component $\langle \widetilde{W}, \boldsymbol{\xi} \rangle_{\mathcal{H}}$ and the response.

For practical computation, 
let us first denote
\begin{equation*}
\mathbf{b} := \mathbf{y}^\top
[ \widetilde{\mathbf{C}} \, \mathbf{J} \, \mathbf{Z} ].
\end{equation*}
Then we obtain $\mathbf{d}$ by solving a linear system
\begin{equation*}
	\mathbf{J}^\ast \mathbf{d} =
	\mathbf{b}^\top.
\end{equation*}  
Then we obtain the coefficient for the PLS direction by normalize it with respect to the basis $B^*[\mathbf{t}]$:
\begin{equation}\label{compute:pls_direction_coeff}
	\begin{bmatrix} \mathbf{a} \\ \mathbf{\theta} \end{bmatrix}
	= \frac{1}{\sqrt{\mathbf{d}^\top \mathbf{J}^\ast \mathbf{d}}} \mathbf{d}.
\end{equation}
The $l$-th  PLS score for the 
$i$th observation is then computed as the inner product  with respect to the basis $B^*[\mathbf{t}]$:
\begin{equation*}
	\widehat{\rho}_{i}
	= 
	[ \widetilde{\mathbf{C}}  \, \mathbf{Z} ] \, \mathbf{J}^\ast \begin{bmatrix} \mathbf{a} \\ \mathbf{\theta} \end{bmatrix}
\end{equation*}
$ \equiv \widehat{\rho}_{il}$ can be obtained as $\widehat{\rho}_i = \langle \mathbf{W}_i, \widehat{\boldsymbol{\xi}} \rangle_\mathcal{H}$.
...
%	consider the basis function expansions of the functional predictors $X_i^{(k)}(t_k) = \sum_{m=1}^M c_{im}^{(k)} b^{(k)}_{m}(t_k)$, $i=1, \ldots, n$, and those of the functional parts of the PLS component $\psi^{(k)}(t_k) = \sum_{m=1}^M a^{(k)}_{m} b^{(k)}_m(t_k)$, $k=1, \ldots, K$. Let 
	\begin{proposition} \label{Proposition: generalized rayleigh quotient}
		Let $\operatorname{span}(B^\ast[\mathbf{t}])$ 
		be a linear subspace of $\mathcal{H}$ spanned by the basis functions.
		  For any unit-length direction  $\boldsymbol{\xi} \in \operatorname{span}(B^\ast[\mathbf{t}])$,
expressed as $
\boldsymbol{\xi} 
= 
\boldsymbol{\alpha}
B^\ast [\mathbf{t}] $ with $\boldsymbol{\alpha} \in \mathbb{R}^{MK + p}$,
		Let us define
		the empirical and basis-approximated sample covariance as
\begin{equation*}
	\widehat{\textnormal{Cov}} (\langle \widetilde{\mathbf{W}}, \boldsymbol{\xi} \rangle_{\mathcal{H}}, Y )
	:=
	\frac{1}{n} \sum \limits_{i=1}^n \langle \widetilde{W}_i, \boldsymbol{\xi} \rangle_\mathcal{H} y_i
	=
		\frac{1}{n} \sum \limits_{i=1}^n 
	(	\tilde{\mathbf{C}}^\ast_i \mathbf{J}^\ast \boldsymbol{\alpha} ) y_i.
\end{equation*}
Then the coefficients computed in \eqref{compute:pls_direction_coeff}  $\widehat{\textnormal{Cov}} (\langle \widetilde{\mathbf{W}}, \boldsymbol{\xi} \rangle_{\mathcal{H}}, Y )$
over all unit vectors in $\operatorname{span}(B^\ast[\mathbf{t}])$.
In a more formal way:
	\begin{equation}
	\begin{bmatrix} \mathbf{a} \\ \mathbf{\theta} \end{bmatrix}
	B^*[\mathbf{t}] \in \arg \max_{
 \|\boldsymbol{\xi}  \| = 1, 
	 \boldsymbol{\xi} \in \mathrm{span}(B^*[\mathbf{t}])
	}
		\widehat{\textnormal{Cov}} (\langle \widetilde{\mathbf{W}}, \boldsymbol{\xi} \rangle_{\mathcal{H}}, Y ).
\end{equation}
	...
	\begin{proposition} \label{Proposition: Regularized generalized rayleigh quotient}
		...
		\begin{align}
			\begin{split}
				&\argmax \limits_{\boldsymbol{\xi}^* \in \mathbb{R}^{MK+p}} \; \frac{\boldsymbol{\xi}^{*\top} \mathbf{V}^\ast \boldsymbol{\xi}^*}{\boldsymbol{\xi}^{*\top}(J^\ast+\Lambda \ddot{J}^\ast) \boldsymbol{\xi}^*} , \quad \textit{or equivalently}, \\ 
				&\argmax \limits_{\boldsymbol{\xi}^* \in \mathbb{R}^{MK+p}} \;  \boldsymbol{\xi}^{*\top} \mathbf{V}^\ast \boldsymbol{\xi}^* \quad \textnormal{subject to}  \quad \boldsymbol{\xi}^{*\top} (J^\ast+\Lambda \ddot{J}^\ast) \boldsymbol{\xi}^*=1.
	...
	\end{equation*}
	through the transformations $\mathbf{e} = L^\top \boldsymbol{\xi}^*$ and $E = L^{-1} \mathbf{V}^\ast L^{\top-1}$. Once $\mathbf{e}$ is obtained as the first eigenvector of $E$, we set $\boldsymbol{\xi}^* = L^{\top-1} \mathbf{e}$ and $\widehat{\boldsymbol{\xi}}[\mathbf{t}] = B^*[\mathbf{t}]^\top \boldsymbol{\xi}^*$.
	...
	\begin{algorithm}[b!]
		...
		\begin{algorithmic}[1]
			...
			\State 	$\widehat{\boldsymbol{\xi}}_l^\ast \leftarrow \argmax_{\boldsymbol{\xi}^* \in \mathbb{R}^{MK+p}} \;  \boldsymbol{\xi}^{*\top} \mathbf{V}^\ast \boldsymbol{\xi}^* \quad \textnormal{subject to}  \quad \boldsymbol{\xi}^{*\top} (J^\ast+\Lambda \ddot{J}^\ast) \boldsymbol{\xi}^*=1
			$
	...
	\begin{equation}\label{def:eigenproblem_smoothed}
		\argmax \limits_{\boldsymbol{\xi}^* \in \mathbb{R}^{MK+p}} \; \frac{\boldsymbol{\xi}^{*\top} \mathbf{V}^\ast \boldsymbol{\xi}^*}{\boldsymbol{\xi}^{*\top} J^\ast \boldsymbol{\xi}^*}, \quad \textit{or equivalently}, \quad \argmax \limits_{\boldsymbol{\xi}^* \in \mathbb{R}^{MK+p}} \;  \boldsymbol{\xi}^{*\top} \mathbf{V}^\ast \boldsymbol{\xi}^* \quad \textnormal{subject to}  \quad \boldsymbol{\xi}^{*\top} J^\ast \boldsymbol{\xi}^*=1,
	\end{equation}
	with respect to $\widehat{\boldsymbol{\xi}}^*=(\widehat{\mathbf{a}}^{\top}, \widehat{\boldsymbol{\theta}}^{\top})^\top$
	...
	\begin{align*}
		...
		&= n^{-1} \widetilde{\mathbf{y}}^\top \left[ \sum \limits_{k=1}^K \widetilde{C}^{(k)} \left\{ \int_{\mathcal{T}_k} \mathbf{b}^{(k)}(t_k) \mathbf{b}^{(k)}(t_k)^\top dt_k \right\} \mathbf{a}^{(k)} \right] + n^{-1}\widetilde{\mathbf{y}}^\top \mathbf{Z} \boldsymbol{\theta} \\
	...
	This implies that
	\begin{align*}
		\widehat{\textnormal{Cov}}^2(\langle \mathbf{W}, \boldsymbol{\xi} \rangle_{\mathcal{H}}, Y ) &= n^{-2} \widetilde{\mathbf{y}}^\top (\widetilde{C}J\mathbf{a} + \mathbf{Z} \boldsymbol{\theta}) (\widetilde{C}J\mathbf{a} + \mathbf{Z} \boldsymbol{\theta})^\top \widetilde{\mathbf{y}} \\
		...
		\end{bmatrix} \\
		&\equiv \boldsymbol{\xi}^{*T} \mathbf{V}^\ast \boldsymbol{\xi}^*.
	\end{align*}
	...
	\begin{equation*}
		\max \limits_{\boldsymbol{\xi}^* \in \mathbb{R}^{MK+p}} \;  \boldsymbol{\xi}^{*\top} \mathbf{V}^\ast \boldsymbol{\xi}^* \quad \textnormal{subject to}  \quad \boldsymbol{\xi}^{*\top} J^\ast \boldsymbol{\xi}^*=1,
	\end{equation*}
	or equivalently solving
	\begin{equation*}
		\max \limits_{\boldsymbol{\xi}^* \in \mathbb{R}^{MK+p}} \; \frac{\boldsymbol{\xi}^{*\top} \mathbf{V}^\ast \boldsymbol{\xi}^*}{\boldsymbol{\xi}^{*\top} J^\ast \boldsymbol{\xi}^*},
	\end{equation*}
	where $J^\ast = \mathrm{blkdiag}[J, \mathbf{I}_p] \in \mathbb{R}^{MK+p}$. In other words, the PLS component is chosen to maximize the squared sample covariance  $\widehat{\textnormal{Cov}}^2(\langle \mathbf{W}, \boldsymbol{\xi} \rangle_{\mathcal{H}}, Y ) = \boldsymbol{\xi}^{*\top} \mathbf{V}^\ast \boldsymbol{\xi}^*$ subject to the constraint $\boldsymbol{\xi}^{*\top} J^\ast \boldsymbol{\xi}^*=1$. 
	...
	% \begin{equation*}
		%         \frac{\boldsymbol{\xi}^{*T} \mathbf{V}^\ast \boldsymbol{\xi}^*}{\boldsymbol{\xi}^{*T} J^\ast \boldsymbol{\xi}^* + \kappa \textnormal{PEN}(\psi)},
		% \end{equation*}
