...
\begin{equation*}
	...
	\in \mathbb{R}^n.
\end{equation*}
Recalling \eqref{thetaBzetal2i} our goal of showing $
\Theta^{[l_1] }
\mathbb{B}
\hat{\boldsymbol{\xi}}_{l_2} = \mathbf{0} \in \mathbb{R}^n.
$
is equivalent to showing  $	\langle 	\widetilde{\boldsymbol{W}}^{[l_1]}  ,
\hat{\xi}^{[l_2]}
\rangle_{\mathcal{H}} = \mathbf{0}$.
Using this notation and  \eqref{algorithm_step:residualization_predictor:recall}, we have, with some abuse of notation,
\begin{equation*}
\widetilde{\boldsymbol{W}}^{[l_1]}  = 
