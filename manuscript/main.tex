...
Since functional predictors are observed at finitely many discrete time points, we approximate them by truncating the expansion at $M$ terms, yielding a finite-dimensional projection. A moderately large $M$ (e.g., 15 or 20) suffices to capture rich functional variation, and no fine-tuning is needed, as our method controlls smoothness  via penalization (Section~\ref{sec: Regularization of PLS Components}). We denote the truncated space by
	\begin{equation}
...
\begin{equation}\label{def:problem_instance}
	(B^\ast, \Theta, \mathbf{y}),
\end{equation}
defined as follows.
Let 
$B, B^{''} \in \mathbb{R}^{M \times M}$
denote the Gram matrices of the basis functions and their second derivatives, respectively, with entries  
\begin{equation}
\end{equation}
\[
B_{m, m'} := \langle b_m, b_{m'} \rangle = \int_0^1 b_m(t)\, b_{m'}(t)\, dt,
\]
for $m, m' = 1, \ldots, M$.
Then define $B^\ast := \operatorname{blkdiag}(B, \ldots, B, I_p) \in \mathbb{R}^{(MK + p) \times (MK + p)}$,  
$\Theta := (\Theta_1, \ldots, \Theta_K, Z) \in \mathbb{R}^{n \times (MK + p)}$,  
and $\mathbf{y} := (y_1, \ldots, y_n)^\top$.
	\begin{remark}
While it is possible to use a different basis for each functional predictor, we adopt a common basis for all predictors to simplify the notation. However, the definition of $B^\ast$ remains general enough to accommodate different bases across the $K$ predictors.
\end{remark}
...
\begin{equation}	\label{eq: Regularized generalized rayleigh quotient equation}
	...
	\quad \text{subject to} \quad \boldsymbol{\xi}^\top B^\ast \boldsymbol{\xi} = 1,
\end{equation}
where
\begin{equation}\label{def:V_matrix}
V := \frac{1}{n^2} (B^\ast \Theta^\top \mathbf{y})(B^\ast \Theta^\top \mathbf{y})^\top \in \mathbb{R}^{(MK+p) \times (MK+p)},
\end{equation}
...
\frac{1}{n^2} 
\boldsymbol{\xi}^\top
(B^\ast \Theta^\top \mathbf{y})(B^\ast \Theta^\top \mathbf{y})^\top 
\boldsymbol{\xi}
\\&=
	\boldsymbol{\xi}^\top
V 
...
\begin{equation*}
	...
	+ \boldsymbol{\zeta}^\top \boldsymbol{\zeta}
	=
	\boldsymbol{\xi}^\top B^\ast \boldsymbol{\xi}.
\end{equation*}
