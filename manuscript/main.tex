...
and structure-aware regularization, such as roughness penalties \jongmin{let's add citations here}.
For the high-dimensionality challenge, a major solution is regression with derived inputs,
such as Principal Component Analysis (PCA) regression.
This involves applying (multivariate) functional PCA (FPCA)  to the functional predictors, possibly with standard PCA to the scalar predictors,
followed by a classical multivariate regression on the resulting scores.
This PCA regression strategy is well-studied for functional predictors alone \citep{hallMethodologyConvergenceRates2007, reissFunctionalPrincipalComponent2007, febrero-bandeFunctionalPrincipalComponent2017}.
However, this approach has a major drawback: the derived inputs are not informed by the response variable. Consequently, they are not guaranteed to capture the core regression relationship with a small number of derived inputs.
Partial Least Squares (PLS) regression offers a powerful alternative by explicitly seeking low-dimensional projections of the predictors that maximize their covariance with the outcome. PLS has been extensively studied in the context of functional data, both for single and multiple functional predictors \citep{predaPLSRegressionStochastic2005, reissFunctionalPrincipalComponent2007, aguileraUsingBasisExpansions2010, delaigleMethodologyTheoryPartial2012, aguileraPenalizedVersionsFunctional2016, febrero-bandeFunctionalPrincipalComponent2017, beyaztasRobustFunctionalPartial2022, saricamPartialLeastsquaresEstimation2022, mutisFunctiononfunctionLinearQuantile2025}.
However, all of these existing PCA or PLS approaches overlook the potentially strong correlations between functional and scalar components. This can lead to multicollinearity and suboptimal predictive performance.
To address the third challenge, \citet{jangPrincipalComponentAnalysis2021}  proposed a joint PCA method that accounts for correlations between functional and scalar data. However, the resulting components are still not informed by the response, so they may fail to capture the directions of maximal correlation with the outcome.

To bridge this gap, we unify the remedies proposed for these three issues: basis expansion, partial least squares, and a hybrid approach that accounts for the correlation between functional and scalar parts.
...
Therefore, our method is computationally efficient, and applicable for dense or irregular functional data.
	...
	least squares criterion: \jongmin{I removed the penalization because it makes proving the orthonormality of the PLS components in Proposition \ref{proposition: modified orthnormality of PLS components} impossible.}
...
The hybrid regression coefficient $\beta$ in model \eqref{eq: Hybrid functional model} can be written in terms of the estimated PLS directions and scores. First, note that we can show $
\widehat{\rho}_i^{[l]}
=
...
$, where 
$\widehat{ \iota}^{[l]} = \widehat{\xi}^{[l]} - \sum_{u=1}^{l-1} \langle \widehat{ \delta}^{[u]}, \widehat{\xi}^{[l]} \rangle_{\mathbb{H}} \widehat{ \iota}^{[u]}$ for $l \ge 2$, with $
...
This completes the proof of Lemma \ref{proposition:closed_form_orthgonalization}.
	\section{Proof of Section \ref{section:main:properties_of_our_method}}
