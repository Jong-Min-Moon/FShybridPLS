...
For notational convenience, we assume throguth this paper that the responsd sand preedictoras ahve been centered so the intercept can be ignored. 
...
~
	...
	We describe the strategy at the $l$-th iteration. %For simplicity, we omit the iteration index and assume the observations have been residualized from previous steps (see Section~\ref{subsec: Step 2: Computing Predictors via Hybrid-on-Scalar Regression} for details). 
	...
	$
	and $\hat{\boldsymbol{\zeta}}$ is the scalar part. This maximizer can also be represented as a Euclidean vector
...
The $l$-th iteration's second step involves orthogonalization. This is achieved by fitting a hybrid-on-scalar regression model, where the predictor is the PLS score for each observation. The individual PLS score is computed as  $\hat{\rho}^{[l]}_i := \langle \widetilde{W}_i^{[l]}, \, \hat{\xi}^{[l]} \rangle_{\mathcal{H}}$, utilizing the estimated PLS component direction $\widehat{\xi}^{[l]}$ obtained from Propositions \ref{proposition:eigen_regul} and \ref{proposition:linear_regul}. Subsequently, using the basis expansion coefficient notation from \eqref{def:truncated_zeta_and_delta}, \eqref{def:coef_vector_form}, \eqref{def:theta}, and \eqref{def:B_matrix}, the full vector of PLS scores $\hat{\boldsymbol{\rho}}^{[l]} := (\hat{\rho}^{[l]}_1, \ldots, \hat{\rho}^{[l]}_n)^\top$ is computed through the following matrix multiplication:
\begin{equation*}
( \hat{\boldsymbol{\rho}}^{[l]} )^\top = \sum_{k=1}^K ( \hat{\boldsymbol{\gamma}}_j^{[l]})^\top B \, (\Theta_j^{[l]})^\top + \boldsymbol{\zeta}^\top (\mathbf{Z}^{[l]}).
\end{equation*}
Using this as a scalar predictor vector, we fit the regression model $\widetilde{W}^{[l]}_i = \hat{\rho}^{[l]}_i\, \delta^{[l]} + E_i^{[l]}$, and then take the residuals as the predictor for the next iteration, namely $\widetilde{W}^{[l+1]}_i$. Here, $\delta^{[l]}$ and $E_i^{[l]}$ are the regression coefficient and error terms, respectively, both elements of $\tilde{\mathcal{H}}$ as defined in \eqref{def:truncated_hilbert_space}. To ensure the stability of the entire PLS estimation procedure, the estimated regression coefficient $\hat{\delta}^{[l]} = (\hat{\delta}^{[l]}_1, \ldots, \hat{\delta}^{[l]}_K, \hat{\boldsymbol{\chi}}^{[l]} )$ should have smooth functional parts, consistent with Section \ref{sec: Regularization of PLS Components}. Consequently, $\delta^{[l]}$ is obtained by minimizing a penalized least squares criterion, where the functional part is penalized by the integrated squared second derivative defined in \eqref{def:roughness_penalty}:
...
The following proposition shows that the solution of the problem \ref{def:argmin_pensse} can be obtained by solving linear equations.
 ...
 \begin{proposition}[Closed-form  orthogonalization]\label{proposition:closed_form_orthgonalization}
 	asdasdasd
 \end{proposition}
 Proof of Proposition \ref{proposition:closed_form_orthgonalization} is provided in Appendix \ref{section:proof:proposition:closed_form_orthgonalization}.
%Using the basis expansion coefficient notation presented in \eqref{def:truncated_zeta_and_delta} and \eqref{def:coef_vector_form}, \eqref{def:theta}, \eqref{def:B_matrix}
\jongmin{Old writing starts here. I temporarily mixed the order of the paragraphs.}
%As in Section \ref{subsec: Step 1: Computing Smoothed PLS components}, the hybrid response can be expanded using basis functions as $\widetilde{W}[\mathbf{t}] = \widetilde{C}^* B^*[\mathbf{t}]$. Similarly, we expand each functional regression coefficient using the basis functions as $\pi^{(k)}(t_k) =\sum_{m=1}^M d_m^{(k)} b^{(k)}_m(t_k) = \boldsymbol{\gamma}^{(k)\top} \mathbf{b}^{(k)}(t_k)$, where $\boldsymbol{\gamma}^{(k)} = (d_1^{(k)}, \ldots, d_M^{(k)})\top \in \mathbb{R}^M$ are basis coefficients. Here, a different basis system can be flexibly chosen to expand the regression coefficient, but without loss of generality we use the same system as in Section \ref{subsec: Step 1: Computing Smoothed PLS components}. With the $(MK+p)$-dimensional row vector $\boldsymbol{\gamma}^* = (\boldsymbol{\gamma}^{(1)\top}, \ldots, \boldsymbol{\gamma}^{(K)\top}, \gamma_1, \ldots, \gamma_p)$ that stacks the basis coefficients and scalar regression coefficients, we can express the linear predictor of the regression as: 
$\widehat{\boldsymbol{\rho}} \, \boldsymbol{\delta}[\mathbf{t}]^\top = \widehat{\boldsymbol{\rho}} \, \boldsymbol{\gamma}^* B^*[\mathbf{t}]$. Finally, these basis expansions of the hybrid response and the regression coefficient enable us to re-write the hybrid-on-scalar regression model \eqref{eq: hybrid on scalar regression model} in a finite-dimensional form:
\begin{equation}
...
\end{equation}
$\mathbf{e}[\mathbf{t}]$ is an $n \times (K+p)$ matrix of errors. 
\\
...
Then, we can use $\widehat{\boldsymbol{\delta}}[\mathbf{t}]$ to obtain the subsequent residualized hybrid predictor as $\mathbf{W}_i^{[l+1]}[\mathbf{t}] = \mathbf{W}_i^{[l]}[\mathbf{t}] - \widehat{\boldsymbol{\delta}}_{l}[\mathbf{t}] \widehat{\rho}_{il}$. 
	...
	\section{Proof of Proposition \ref{proposition:closed_form_orthgonalization} \ref{}}\label{section:proof:proposition:closed_form_orthgonalization}
	 \begin{align*}
		\operatorname{PENSSE}( \delta)
		&=
		\sum_{i=1}^n \langle \widetilde{W}_i^{[l]} - \hat{\rho}_i^{[l]}\, \delta,
		...
		\\&=
		\sum_{i=1}^n 
		\langle \widetilde{W}_i^{[l]},  \, \widetilde{W}_i^{[l]} \rangle_{\mathcal{H}}
		-
		\sum_{i=1}^n 
		\hat{\rho}_i^{[l]} 
		\langle \delta,  \, \widetilde{W}_i^{[l]} \rangle_{\mathcal{H}}
		+
		\bigl(
		\sum_{i=1}^n 
		( \hat{\rho}_i^{[l]} )^2
		...
		\sum_{j=1}^K\operatorname{PEN}(\delta_j) 
		\\&=
		\operatorname{tr}(C^{*T} C^{*}J^{*}\Lambda) 
		- 
		2 \operatorname{tr}(d^{*}J^{*}C^{*T} \rho\Lambda) 
		+
		\rho^T \rho
		\operatorname{tr}( d^{*}J^{*}d^{*T} \Lambda) + \tau \operatorname{tr}(\ddot{d}^{*}J^{*}\ddot{d}^{*T}) \jongmin{need to be fixed}
	\end{align*}
