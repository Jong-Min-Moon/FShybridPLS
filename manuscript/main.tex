...
For notational simplicity, we omit the index $l$. At the population level, where the random objects are fully observed, it suffices to consider $Y_1$ and $W_1$ rather than their sampled realizations.
We assume 
$\mathbb{E}[Y_1]=0 \in \mathbb{R}$ and $\mathbb{E}[W_1] = 0 \in \mathbb{H}$.
 We first define the cross-covariance between the response and the functional predictors, where the predictors correspond to the observed versions for $l=1$ and to the residualized versions for $l \ge 2$:
	...
	\begin{theorem}[Tucker's criterion]\label{theorem:population_PLS} Under the conditions of Lemma \ref{lemma:cross_cov_functional}, the constrained maximum
		\begin{equation*} 
			\max \limits_{\substack{\xi \in \mathbb{H} \\ \Vert \xi \Vert_\mathbb{H}=1}} \operatorname{Cov}^2 \left(\langle W_1 , \, \xi \rangle_\mathbb{H}, Y_1 \right)
		\end{equation*} 
		is attained by the eigenfunction associated with the largest eigenvalue of the operator $\mathcal{U}$.
	\end{theorem}
	Note that the maximizer is a deterministic element of $\mathbb{H}$.
	The proof of Theorem \ref{theorem:population_PLS} is provided in Appendix \ref{section:proof:theorem:population_PLS}.

 
Let 
 $\mathcal{U}^{[l]} := \Sigma_{YW}^{[l]} \otimes \Sigma_{YW}^{[l]}$,
 where
 $\Sigma_{YW}^{[l]} = E(Y_1^{[l]}  W_1^{[l]})$
 is formulated   using the $l$-th residualized response and predictors.
 Assuming that $\mathcal{U}^{[l]}$
   satisfies the conditions of Lemma \ref{lemma:cross_cov_functional},
	the $l$-th PLS direction $\xi^{[l]}$ can be obtained as the \textit{first} eigenfunction of  $\mathcal{U}^{[l]}$. 
	
\jongmin{comment: I added the following properties, motivated by Proposition 4 of \citet{predaPLSRegressionStochastic2005} and section 2 of \citet{aguileraPenalizedVersionsFunctional2016}. }


 
\begin{lemma} \label{lem:regression_W_rho} The population-level least square regression coefficient of regressing $W_1^{[l]}$ on its score $\rho_1^{[l]} = \langle W_1^{[l]}, \xi^{[l]} \rangle_{\mathbb{H}}$ is $\mathbf{\xi^{[l]}} \in \mathbb{H}$. This coefficient is used in the residualization process as follows: \begin{align*} W_1^{[l]} &= W_1^{[l-1]} - \langle W_1^{[l-1]}, \xi^{[l-1]} \rangle_{\mathbb{H}} \xi^{[l-1]},\end{align*} where  $W_1^{[1]} = W_1$. \end{lemma} 
Proof of Lemma \ref{lem:regression_W_rho} is provided in Appendix \ref{section:proof:lem:regression_W_rho}.
 

  
  	
 We define an operator $\mathcal{T}$ that maps an element $g \in \mathbb{H}$   (representing a potential regression coefficient) to the predicted response.
\begin{lemma}\label{lemma:tau_ast}
	Define the linear operator $\mathcal{T}: \mathbb{H} \to \mathbb{R}$ that maps  $g \in \mathbb{H}$ to 
	$
	\mathcal{T}g = \langle W, g \rangle_{\mathbb{H}}
	$.
 Let $\mathcal{T}^\ast: \mathbb{R} \to \mathbb{H}$ denote its adjoint operator. Then,
	\[
	\Sigma_{YW}^{[1]} = \mathcal{T}^\ast Y.
	\]
\end{lemma}
Proof of Lemma \ref{lemma:tau_ast} is provided in Appendix \ref{section:rpoof:lemma:tau_ast}.



\begin{proposition}\label{proposition:pls_direction_in_range_T_ast}
	All PLS Directions Reside in $R(\mathcal{T}^\ast  )$
\end{proposition}
 Proof of Proposition \ref{proposition:pls_direction_in_range_T_ast} is provided in Appendix \ref{section:proof:proposition:pls_direction_in_range_T_ast}
 
\begin{proposition}
	\label{proposition:CONS}
	The set of PLS directions $\{\xi^{[l]}\}_{l=1}^\infty$ forms a complete orthonormal system (CONS) for $R(\mathcal{T}^*)$.
\end{proposition}
Proof of Proposition \ref{proposition:CONS}
is provided in Appendix \ref{section:proof:proposition:CONS}.

\begin{corollary}
	\label{cor:orthogonal_scores}
	Given that $\{\xi^{[l]}\}_{l=1}^\infty$ is a complete orthonormal system (CONS) for $R(\mathcal{T}^*)$, the corresponding PLS scores $\rho^{[l]}$ can be equivalently expressed as $\rho^{[l]} = \langle W^{[l]}, \xi^{[l]} \rangle_{\mathbb{H}} = \langle W, \xi^{[l]} \rangle_{\mathbb{H}}$. Furthermore, the set of PLS scores $\{\rho^{[l]}\}_{l=1}^\infty$ forms an orthogonal system, that is, for any $j \neq l$, $\mathbb{E}[\rho^{[j]} \rho^{[l]}] = 0$.
\end{corollary}
 
\begin{proposition} \label{prop:ols_uniqueness} Let $\beta_{\mathrm{OLS}}$ denote a minimizer of the mean squared error $\mathbb{E}[\|Y - \langle \beta, W \rangle_{\mathbb{H}}\|_2^2]$. This minimizer satisfies the normal equation: \begin{equation*} \mathbb{E}[(Y - \langle \beta_{\mathrm{OLS}}, W \rangle_{\mathbb{H}})W] = \mathbf{0} \in {\mathbb{H}}. \end{equation*}  If $\mathcal{T}^\ast \mathcal{T}$ is not invertible, $\beta_{\mathrm{OLS}}$ may not be unique, but all solutions for $\beta_{\mathrm{OLS}}$ reside in $R(\mathcal{T}^*)$, and the corresponding fitted values $Y_{\mathrm{OLS}} = \langle \beta_{\mathrm{OLS}}, W \rangle_{\mathbb{H}}$ are always unique. \end{proposition}
Proof of Proposition \ref{prop:ols_uniqueness} is provided in Appendix \ref{section:proof:prop:ols_uniqueness}.
\begin{proposition}\label{proposition:nu_l}
	At the $l$-th iteration, the 
	population level least square regression coefficient of the residual response $Y^{[l]}$ on the score $\rho^{[l]}$ is defined as:
	$$\nu^{[l]} = \langle \beta_{\mathrm{OLS}}, \xi^{[l]} \rangle_{\mathbb{H}}.$$
\end{proposition}
Proof of Proposition \ref{proposition:nu_l} is provided in Appendix \ref{section:proof:proposition:nu_l}.
...
\begin{theorem}[$L^2$ Convergence of Hybrid PLS at Population Level]\label{thm:hybrid_pls_convergence}
	 Define $Y_{\mathrm{OLS}} = \langle \beta_{\mathrm{OLS}}, W \rangle_{\mathbb{H}}$ as the population-level ordinary linear regression fitted value. Let $Y_{\mathrm{PLS}, L} = \sum_{l=1}^L \nu^{[l]} \rho^{[l]}$ be the Hybrid PLS fitted value with $L$ components. Then, the Hybrid PLS fitted value converges to the ordinary linear regression fitted value in the mean-squared sense:
	\begin{equation*}
		\lim_{L \to \infty} \mathbb{E}\left[ \| Y_{\mathrm{PLS},L } - Y_{\mathrm{OLS}} \|_2^2 \right] = 0
	\end{equation*}
\end{theorem}
 The proof of Theorem \ref{thm:hybrid_pls_convergence} is provided in Appendix \ref{section:proof:thm:hybrid_pls_convergence}.
	...
	A fundamental property of partial least squares   is that between iterations, its derived directions are orthonormal and PLS scores are orthogonal. Our regularized estimates preserve this property, with respect to a modified inner product that incorporates the roughness penalty, defines as follows:
...
\subsection{
		Proof of Lemma \ref{lem:regression_W_rho}
}\label{section:proof:lem:regression_W_rho}
\begin{proof} We are seeking the population-level regression coefficient, denoted $\gamma \in \mathbb{H}$, for the linear model: $$W_1^{[\ell]} = \gamma \rho_1^{[\ell]} + \text{Error}$$ where $W_1^{[\ell]}$ is the dependent variable (a random element in $\mathbb{H}$), and $\rho_1^{[\ell]} = \langle W_1^{[\ell]}, \xi^{[\ell]} \rangle_{\mathbb{H}}$ is the independent variable (a scalar random variable). The optimal regression coefficient $\gamma$ (in the sense of minimizing the mean squared error $E[\|W_1^{[\ell]} - \gamma \rho_1^{[\ell]}\|^2_{\mathbb{H}}]$) is given by the formula: 
	$$\gamma = \frac{E[W_1^{[\ell]} \rho_1^{[\ell]}]}{E[(\rho_1^{[\ell]})^2]}$$ 
	This formula holds assuming $E[W_1^{[\ell]}] = \mathbf{0}_{\mathbb{H}}$ and $E[\rho_1^{[\ell]}] = 0$, which is true due to the residualization process and the property of zero-mean predictors in the population. Let's compute the components of this formula:
 \begin{enumerate} \item \textbf{Denominator:} The denominator is $E[(\rho_1^{[\ell]})^2] = E[(\langle W_1^{[\ell]}, \xi^{[\ell]} \rangle_{\mathbb{H}})^2]$. This is the population variance of the score $\rho_1^{[\ell]}$. \item \textbf{Numerator:} The numerator is $E[W_1^{[\ell]} \rho_1^{[\ell]}] = E[W_1^{[\ell]} \langle W_1^{[\ell]}, \xi^{[\ell]} \rangle_{\mathbb{H}}]$. This is the expectation of the product of a random element $W_1^{[\ell]}$ and a scalar random variable $\langle W_1^{[\ell]}, \xi^{[\ell]} \rangle_{\mathbb{H}}$. A fundamental identity in Hilbert spaces states that for any random element $X \in \mathbb{H}$ and any non-random unit vector $u \in \mathbb{H}$ (like $\xi^{[\ell]}$ which has unit norm), if $E[X] = \mathbf{0}_{\mathbb{H}}$, then: $$E[X\langle X,u\rangle_{\mathbb{H}}]=E[\langle X,u\rangle_{\mathbb{H}}^2]u$$ Applying this identity by setting $X=W_1^{[\ell]}$ and $u=\xi^{[\ell]}$: $$E[W_1^{[\ell]}\langle W_1^{[\ell]},\xi^{[\ell]}\rangle_{\mathbb{H}}]=E[(\langle W_1^{[\ell]},\xi^{[\ell]}\rangle_{\mathbb{H}})^2]\xi^{[\ell]}$$ \end{enumerate} Substituting these into the formula for $\gamma$: $$\gamma = \frac{E[(\langle W_1^{[\ell]}, \xi^{[\ell]} \rangle_{\mathbb{H}})^2] \xi^{[\ell]}}{E[(\langle W_1^{[\ell]}, \xi^{[\ell]} \rangle_{\mathbb{H}})^2]}$$ Assuming that $E[(\langle W_1^{[\ell]}, \xi^{[\ell]} \rangle_{\mathbb{H}})^2] \neq 0$ (i.e., the score has non-zero variance, implying $\xi^{[\ell]}$ captures some variation), we can cancel the terms, resulting in: $$\gamma = \xi^{[\ell]}$$ This result is intuitively consistent with the orthogonal decomposition of $W_1^{[\ell]}$: $$W_1^{[\ell]}=\langle W_1^{[\ell]},\xi^{[\ell]}\rangle_{\mathbb{H}}\xi^{[\ell]}+(W_1^{[\ell]}-\langle W_1^{[\ell]},\xi^{[\ell]}\rangle_{\mathbb{H}}\xi^{[\ell]})$$ The second term, $(W_1^{[\ell]}-\langle W_1^{[\ell]},\xi^{[\ell]}\rangle_{\mathbb{H}}\xi^{[\ell]})$, is the part of $W_1^{[\ell]}$ that is orthogonal to $\xi^{[\ell]}$ (and thus uncorrelated with $\rho_1^{[\ell]}$). When $W_1^{[\ell]}$ is regressed on $\rho_1^{[\ell]}$, the regression model isolates the component of $W_1^{[\ell]}$ that is linearly related to $\rho_1^{[\ell]}$, which is precisely $\langle W_1^{[\ell]},\xi^{[\ell]}\rangle_{\mathbb{H}}\xi^{[\ell]}$. The coefficient for $\rho_1^{[\ell]}$ in this linear relationship is simply $\xi^{[\ell]}$. \end{proof}
	
 

		
\subsection{Proof of Lemma \ref{lemma:tau_ast} }\label{section:rpoof:lemma:tau_ast}
	\begin{proof}
	The adjoint operator  is uniquely defined by the property relating the inner products in $L^2(\Omega)$ and $\mathbb{H}$: $$ \langle \mathcal{T}g, Y \rangle_{L^2(\Omega)} = \langle g, \mathcal{T}^\ast Y \rangle_{\mathbb{H}}, \quad \text{for all } g \in \mathbb{H}, Y \in L^2(\Omega) $$ Here, $\langle \cdot, \cdot \rangle_{L^2(\Omega)}$ denotes the inner product in $L^2(\Omega)$, which is the expectation $E[\cdot]$. Let's explicitly derive the expression for $\mathcal{T}^\ast Y$: $$ \langle g, \mathcal{T}^\ast Y \rangle_{\mathbb{H}} = E[\langle W, g \rangle_{\mathbb{H}} Y] $$ By the properties of expectation and the linearity of the inner product (assuming appropriate boundedness and measurability for interchange of expectation and inner product): $$ E[\langle W, g \rangle_{\mathbb{H}} Y] = E[\langle g, W \rangle_{\mathbb{H}} Y] = \langle g, E[YW] \rangle_{\mathbb{H}} $$ Comparing the leftmost and rightmost parts of the definition of the adjoint: $$ \mathcal{T}^\ast Y = E[YW] $$
	\end{proof}

\subsection{Proof of}
\begin{proof}
The first PLS direction $\xi^{[1]}$ is the principal eigenfunction of $\mathcal{U}^{[1]} = \Sigma_{YW}^{[1]} \otimes \Sigma_{YW}^{[1]}$. Let $\lambda^{[1]}$ be the largest eigenvalue associated with $\xi^{[1]}$. By the definition of an eigenfunction: $$ \mathcal{U}^{[1]} \xi^{[1]} = \lambda^{[1]} \xi^{[1]} $$ Substituting the definition of $\mathcal{U}^{[1]}$: $$ (\Sigma_{YW}^{[1]} \otimes \Sigma_{YW}^{[1]}) \xi^{[1]} = \lambda^{[1]} \xi^{[1]} $$ This means: $$ \Sigma_{YW}^{[1]} \langle \Sigma_{YW}^{[1]}, \xi^{[1]} \rangle_{\mathbb{H}} = \lambda^{[1]} \xi^{[1]} $$ If there is any linear correlation between $Y$ and $W$, then $\Sigma_{YW}^{[1]} \neq 0$, which implies $\lambda^{[1]}$ must be non-zero. From the equation above, if $\lambda^{[1]} \neq 0$: $$ \xi^{[1]} = \frac{\langle \Sigma_{YW}^{[1]}, \xi^{[1]} \rangle_{\mathbb{H}}}{\lambda^{[1]}} \Sigma_{YW}^{[1]} $$ This equation shows that $\xi^{[1]}$ is a scalar multiple of $\Sigma_{YW}^{[1]}$. Since $\Sigma_{YW}^{[1]} \in R(\mathcal{T}^\ast  )$ (as shown in Step 2), and $R(\mathcal{T}^\ast  )$ is a subspace of $\mathbb{H}$ (and thus closed under scalar multiplication), any scalar multiple of $\Sigma_{YW}^{[1]}$ must also be in $R(\mathcal{T}^\ast  )$. Therefore, the first PLS direction $\xi^{[1]}$ formally resides in the range of the adjoint operator $R(\mathcal{T}^\ast  )$.
\end{proof}
  The first PLS direction $\xi^{[1]}$ is the principal eigenfunction of the operator $\mathcal{U}^{[1]} = \Sigma_{YW}^{[1]} \otimes \Sigma_{YW}^{[1]}$. The term $\Sigma_{YW}^{[1]}$ is the population cross-covariance between the (initial, unresidualized) response $Y$ and hybrid predictor $W$: $$ \Sigma_{YW}^{[1]} = E[YW] $$ From our derivation in Step 1, we see that $\Sigma_{YW}^{[1]}$ is precisely equal to $\mathcal{T}^\ast Y$. Since $\Sigma_{YW}^{[1]} = \mathcal{T}^\ast Y$, it directly follows that $\Sigma_{YW}^{[1]}$ belongs to the **range of the adjoint operator $R(\mathcal{T}^\ast  )$**. By definition, $R(\mathcal{T}^\ast  )$ comprises all elements in $\mathbb{H}$ that can be expressed as $\mathcal{T}^\ast Y$ for some $y \in L^2(\Omega)$. 

\subsection{Proof of Proposition \ref{proposition:pls_direction_in_range_T_ast}}\label{section:proof:proposition:pls_direction_in_range_T_ast}
\begin{proof}
	The First Eigenfunction $\xi^{[1]}$ Resides in $R(\mathcal{T}^\ast  )$
  We aim to formally prove that for any $\ell \ge 1$, the $\ell$-th PLS direction $\xi^{[\ell]}$ belongs to the range of the adjoint operator $\mathcal{T}^\ast  $, denoted $R(\mathcal{T}^\ast  )$, at the population level. This proof uses mathematical induction. \paragraph*{1. Recall Definitions and Base Case} \begin{itemize} \item \textbf{Hybrid Hilbert Space ($\mathbb{H}$):} The space where predictors $W$ reside. \item \textbf{Response Space ($L^2(\Omega)$):} The space of scalar random variables (responses $Y$) with finite second moments. \item \textbf{Operator $\mathcal{T}: \mathbb{H} \rightarrow L^2(\Omega)$:} Defined by $\mathcal{T}g = \langle W, g \rangle_{\mathbb{H}}$. This maps a potential regression coefficient $g$ to a predicted response. \item \textbf{Adjoint Operator $\mathcal{T}^\ast  : L^2(\Omega) \rightarrow \mathbb{H}$:} Uniquely defined by $\langle \mathcal{T}g, Y \rangle_{L^2(\Omega)} = \langle g, \mathcal{T}^\ast Y \rangle_{\mathbb{H}}$. As derived, $\mathcal{T}^\ast Y = E[YW]$. The range $R(\mathcal{T}^\ast  )$ is the subspace of $\mathbb{H}$ containing all elements that are linearly correlated with $Y$. \item \textbf{$\ell$-th PLS Direction ($\xi^{[\ell]}$):} The principal eigenfunction of $\mathcal{U}^{[\ell]} = \Sigma_{YW}^{[\ell]} \otimes \Sigma_{YW}^{[\ell]}$, where $\Sigma_{YW}^{[\ell]} = E[Y^{[\ell]} W^{[\ell]}]$. \end{itemize} \paragraph{Base Case ($\ell=1$):} For the first PLS direction, $\xi^{[1]}$, we consider $\Sigma_{YW}^{[1]} = E[YW]$. From the definition of the adjoint operator $\mathcal{T}^\ast  $, we have $\mathcal{T}^\ast Y = E[YW]$. Therefore, $\Sigma_{YW}^{[1]} = \mathcal{T}^\ast Y$. This explicitly shows that $\Sigma_{YW}^{[1]} \in R(\mathcal{T}^\ast  )$. Since $\xi^{[1]}$ is the principal eigenfunction of $\mathcal{U}^{[1]} = \Sigma_{YW}^{[1]} \otimes \Sigma_{YW}^{[1]}$, it satisfies $\mathcal{U}^{[1]} \xi^{[1]} = \lambda^{[1]} \xi^{[1]}$. This implies $\Sigma_{YW}^{[1]} \langle \Sigma_{YW}^{[1]}, \xi^{[1]} \rangle_{\mathbb{H}} = \lambda^{[1]} \xi^{[1]}$. If $\Sigma_{YW}^{[1]} \neq 0$ (meaning there's linear correlation between $Y$ and $W$), then $\lambda^{[1]} \neq 0$, and $\xi^{[1]}$ must be a scalar multiple of $\Sigma_{YW}^{[1]}$: $$ \xi^{[1]} = \frac{\langle \Sigma_{YW}^{[1]}, \xi^{[1]} \rangle_{\mathbb{H}}}{\lambda^{[1]}} \Sigma_{YW}^{[1]} $$ Since $\Sigma_{YW}^{[1]} \in R(\mathcal{T}^\ast  )$ and $R(\mathcal{T}^\ast  )$ is a subspace (closed under scalar multiplication), it directly follows that $\xi^{[1]} \in R(\mathcal{T}^\ast  )$. The base case is proven. \paragraph*{2. Inductive Step: Assume $\xi^{[1]}, \ldots, \xi^{[\ell-1]} \in R(\mathcal{T}^\ast  )$} Assume that the first $\ell-1$ PLS directions, $\xi^{[1]}, \ldots, \xi^{[\ell-1]}$, all reside in $R(\mathcal{T}^\ast  )$. We need to prove that $\xi^{[\ell]}$ also resides in $R(\mathcal{T}^\ast  )$. The $\ell$-th PLS direction $\xi^{[\ell]}$ is a scalar multiple of $\Sigma_{YW}^{[\ell]} = E[Y^{[\ell]} W^{[\ell]}]$. Thus, to prove $\xi^{[\ell]} \in R(\mathcal{T}^\ast  )$, it suffices to prove that $\Sigma_{YW}^{[\ell]} \in R(\mathcal{T}^\ast  )$. Recall the residualization process at the population level: $$W^{[\ell]} = W^{[\ell-1]} - \langle W^{[\ell-1]}, \xi^{[\ell-1]} \rangle_{\mathbb{H}} \xi^{[\ell-1]}$$ $$Y^{[\ell]} = Y^{[\ell-1]} - \nu^{[\ell-1]} \rho^{[\ell-1]}$$ where $\rho^{[\ell-1]} = \langle W^{[\ell-1]}, \xi^{[\ell-1]} \rangle_{\mathbb{H}}$. By iterating this process, we can express $W^{[\ell]}$ and $Y^{[\ell]}$ in terms of the original $W$, $Y$, and the previously extracted PLS components: $$W^{[\ell]} = W - \sum_{j=1}^{\ell-1} \langle W, \xi^{[j]} \rangle_{\mathbb{H}} \xi^{[j]}$$ $$Y^{[\ell]} = Y - \sum_{j=1}^{\ell-1} \nu^{[j]} \rho^{[j]}$$ From the inductive hypothesis and the base case, all $\xi^{[j]} \in R(\mathcal{T}^\ast  )$ for $j=1, \ldots, \ell-1$. Since $R(\mathcal{T}^\ast  )$ is a subspace, any linear combination of these $\xi^{[j]}$ also belongs to $R(\mathcal{T}^\ast  )$. Thus, the sum $\sum_{j=1}^{\ell-1} \langle W, \xi^{[j]} \rangle_{\mathbb{H}} \xi^{[j]}$ is an element of $R(\mathcal{T}^\ast  )$. Now, we need to show that $\Sigma_{YW}^{[\ell]} = E[Y^{[\ell]} W^{[\ell]}]$ resides in $R(\mathcal{T}^\ast  )$. To do this, we demonstrate that for any $v \in R(\mathcal{T}^\ast  )^\perp$ (the orthogonal complement of $R(\mathcal{T}^\ast  )$ in $\mathbb{H}$), $\langle \Sigma_{YW}^{[\ell]}, v \rangle_{\mathbb{H}} = 0$. By definition, $R(\mathcal{T}^\ast  )^\perp = \{v \in \mathbb{H} \mid E[\langle W, v \rangle_{\mathbb{H}} Y] = 0\}$. Consider $E[\langle W^{[\ell]}, v \rangle_{\mathbb{H}} Y^{[\ell]}]$ for any $v \in R(\mathcal{T}^\ast  )^\perp$: $$E[\langle W^{[\ell]}, v \rangle_{\mathbb{H}} Y^{[\ell]}] = E\left[ \left\langle W - \sum_{j=1}^{\ell-1} \langle W, \xi^{[j]} \rangle_{\mathbb{H}} \xi^{[j]}, v \right\rangle_{\mathbb{H}} \left( Y - \sum_{k=1}^{\ell-1} \nu^{[k]} \rho^{[k]} \right) \right]$$ Due to the orthogonality of $v$ to all $\xi^{[j]}$ (since $v \in R(\mathcal{T}^\ast  )^\perp$ and $\xi^{[j]} \in R(\mathcal{T}^\ast  )$): $$ \left\langle W - \sum_{j=1}^{\ell-1} \langle W, \xi^{[j]} \rangle_{\mathbb{H}} \xi^{[j]}, v \right\rangle_{\mathbb{H}} = \langle W, v \rangle_{\mathbb{H}} - \sum_{j=1}^{\ell-1} \langle W, \xi^{[j]} \rangle_{\mathbb{H}} \langle \xi^{[j]}, v \rangle_{\mathbb{H}} = \langle W, v \rangle_{\mathbb{H}} - 0 = \langle W, v \rangle_{\mathbb{H}} $$ Thus, the expression simplifies to: $$ E[\langle W^{[\ell]}, v \rangle_{\mathbb{H}} Y^{[\ell]}] = E\left[ \langle W, v \rangle_{\mathbb{H}} \left( Y - \sum_{k=1}^{\ell-1} \nu^{[k]} \rho^{[k]} \right) \right] $$ $$ = E[\langle W, v \rangle_{\mathbb{H}} Y] - \sum_{k=1}^{\ell-1} \nu^{[k]} E[\langle W, v \rangle_{\mathbb{H}} \rho^{[k]}] $$ The first term $E[\langle W, v \rangle_{\mathbb{H}} Y] = 0$ because $v \in R(\mathcal{T}^\ast  )^\perp$. For the second term, $E[\langle W, v \rangle_{\mathbb{H}} \rho^{[k]}] = E[\langle W, v \rangle_{\mathbb{H}} \langle W, \xi^{[k]} \rangle_{\mathbb{H}}]$. Since $v \in R(\mathcal{T}^\ast  )^\perp$ and $\xi^{[k]} \in R(\mathcal{T}^\ast  )$, and $E[\langle U, X \rangle \langle V, X \rangle]$ represents the covariance between inner products, and components in $R(\mathcal{T}^\ast  )$ and $R(\mathcal{T}^\ast  )^\perp$ are uncorrelated: $E[\langle W, v \rangle_{\mathbb{H}} \langle W, \xi^{[k]} \rangle_{\mathbb{H}}] = \text{Cov}(\langle W, v \rangle_{\mathbb{H}}, \langle W, \xi^{[k]} \rangle_{\mathbb{H}}) + E[\langle W, v \rangle_{\mathbb{H}}]E[\langle W, \xi^{[k]} \rangle_{\mathbb{H}}]$. Assuming zero-mean predictors ($E[W]=0$), the second term is zero. The covariance $\text{Cov}(\langle W, v \rangle_{\mathbb{H}}, \langle W, \xi^{[k]} \rangle_{\mathbb{H}})$ is also zero since $v$ and $\xi^{[k]}$ are in orthogonal subspaces. Therefore, $E[\langle W^{[\ell]}, v \rangle_{\mathbb{H}} Y^{[\ell]}] = 0$ for all $v \in R(\mathcal{T}^\ast  )^\perp$. This implies that $\Sigma_{YW}^{[\ell]} = E[Y^{[\ell]} W^{[\ell]}]$ is orthogonal to $R(\mathcal{T}^\ast  )^\perp$. By the property of orthogonal decomposition in Hilbert spaces ($R(\mathcal{T}^\ast  ) \oplus R(\mathcal{T}^\ast  )^\perp = \mathbb{H}$), if an element is orthogonal to the orthogonal complement of a closed subspace, it must belong to that subspace. Thus, $\Sigma_{YW}^{[\ell]}$ must reside in $R(\mathcal{T}^\ast  )$. Since $\xi^{[\ell]}$ is a scalar multiple of $\Sigma_{YW}^{[\ell]}$, we conclude that $\xi^{[\ell]} \in R(\mathcal{T}^\ast  )$. By mathematical induction, since the base case holds and the inductive step is proven, all PLS directions $\{\xi^{[\ell]}\}_{\ell=1}^\infty$ reside in $R(\mathcal{T}^\ast  )$. 
\end{proof}

\subsection{Proof of Proposition \ref{proposition:CONS}}\label{section:proof:proposition:CONS}

\paragraph{1.1 Orthonormality through Residualization} The iterative nature of the PLS algorithm guarantees the orthonormality of the directions. The \textbf{residualization (or orthogonalization)} step is crucial. After the $(l-1)$-th PLS direction $\xi^{[l-1]}$ is determined, the predictors and response are adjusted to remove the linear effect of this component. Specifically, at the population level: $$W^{[l]} = W^{[l-1]} - \langle W^{[l-1]}, \xi^{[l-1]} \rangle_{\mathbb{H}} \xi^{[l-1]}$$ $$Y^{[l]} = Y^{[l-1]} - \nu^{[l-1]} \rho^{[l-1]}$$ where $W^{[1]} = W$ and $Y^{[1]} = Y$. This procedure ensures that the residualized predictors $W^{[l]}$ are orthogonal to all previous directions $\xi^{[j]}$ for $j < l$. Consequently, the newly found direction $\xi^{[l]}$, being derived from $W^{[l]}$, will be orthogonal to $\{\xi^{[1]}, \ldots, \xi^{[l-1]}\}$. By convention, PLS directions are normalized to have unit norm ($\Vert \xi^{[l]} \Vert_{\mathbb{H}} = 1$). Thus, the set $\{\xi^{[l]}\}_{l=1}^\infty$ is an orthonormal set. \paragraph{1.2 Completeness} The relevant predictor subspace is precisely the \textbf{range of the adjoint operator $\mathcal{T}^\ast  $}, denoted $R(\mathcal{T}^\ast  )$. To show that $\{\xi^{[l]}\}_{l=1}^\infty$ forms a CONS for $R(\mathcal{T}^\ast  )$, given their orthonormality, we must prove completeness. According to Theorem 2.4.12, this requires demonstrating that if an element $v \in R(\mathcal{T}^\ast  )$ is orthogonal to all PLS directions $\{\xi^{[l]}\}_{l=1}^\infty$, then $v$ must be the zero vector. Let $\mathcal{S} = \text{span}\{\xi^{[l]}\}_{l=1}^\infty$ be the subspace spanned by the PLS directions. Assume, for contradiction, that there exists a non-zero $v \in R(\mathcal{T}^\ast  )$ such that $v \perp \mathcal{S}$. This means $\langle v, \xi^{[l]} \rangle_{\mathbb{H}} = 0$ for all $l=1,2,\ldots$. Since $v \in R(\mathcal{T}^\ast  )$ and $v \neq 0$, it implies that $v$ contains non-zero linear predictive information regarding $Y$. Formally, $R(\mathcal{T}^\ast  )$ is the space of elements in $\mathbb{H}$ that are correlated with $Y$, so $E[\langle W, v \rangle_{\mathbb{H}} Y] \neq 0$. Now, consider the implications of $v \perp \mathcal{S}$ on the iterative PLS algorithm. At any step $\ell$, the residualized predictor $W^{[\ell]}$ is constructed such that its components orthogonal to $\{\xi^{[1]}, \ldots, \xi^{[\ell-1]}\}$ remain. Since $v$ is assumed orthogonal to all $\xi^{[j]}$, the projection of $W$ onto $v$ (i.e., $\langle W, v \rangle_{\mathbb{H}}$) is not removed by any residualization step. Similarly, $Y^{[\ell]}$ is the residual response. The cross-covariance relevant at step $\ell$ is $\Sigma_{YW}^{[\ell]} = E[Y^{[\ell]} W^{[\ell]}]$. If $v \in R(\mathcal{T}^\ast  )$ and $v \neq 0$, it implies that $E[\langle W, v \rangle_{\mathbb{H}} Y] \neq 0$. Let's examine the inner product of $\Sigma_{YW}^{[\ell]}$ with $v$: $\langle \Sigma_{YW}^{[\ell]}, v \rangle_{\mathbb{H}} = \langle E[Y^{[\ell]} W^{[\ell]}] , v \rangle_{\mathbb{H}} = E[Y^{[\ell]} \langle W^{[\ell]}, v \rangle_{\mathbb{H}}]$. Since $v \perp \xi^{[j]}$ for all $j$, the residualization process on $W$ does not affect $\langle W, v \rangle_{\mathbb{H}}$: $\langle W^{[\ell]}, v \rangle_{\mathbb{H}} = \langle W - \sum_{j=1}^{\ell-1} \langle W, \xi^{[j]} \rangle_{\mathbb{H}} \xi^{[j]}, v \rangle_{\mathbb{H}} = \langle W, v \rangle_{\mathbb{H}} - \sum_{j=1}^{\ell-1} \langle W, \xi^{[j]} \rangle_{\mathbb{H}} \langle \xi^{[j]}, v \rangle_{\mathbb{H}} = \langle W, v \rangle_{\mathbb{H}} - 0 = \langle W, v \rangle_{\mathbb{H}}$. So, $\langle \Sigma_{YW}^{[\ell]}, v \rangle_{\mathbb{H}} = E[Y^{[\ell]} \langle W, v \rangle_{\mathbb{H}}]$. By the construction of PLS, the sequence $Y^{[\ell]}$ is generated such that $Y^{[\ell]} = Y - \sum_{j=1}^{\ell-1} \nu^{[j]} \rho^{[j]}$. Substituting this: $\langle \Sigma_{YW}^{[\ell]}, v \rangle_{\mathbb{H}} = E\left[ \left( Y - \sum_{j=1}^{\ell-1} \nu^{[j]} \rho^{[j]} \right) \langle W, v \rangle_{\mathbb{H}} \right]$ $= E[Y \langle W, v \rangle_{\mathbb{H}}] - \sum_{j=1}^{\ell-1} \nu^{[j]} E[\rho^{[j]} \langle W, v \rangle_{\mathbb{H}}]$ The first term, $E[Y \langle W, v \rangle_{\mathbb{H}}]$, is non-zero because $v \in R(\mathcal{T}^\ast  )$ and $v \neq 0$. The second term, $\sum_{j=1}^{\ell-1} \nu^{[j]} E[\rho^{[j]} \langle W, v \rangle_{\mathbb{H}}]$, simplifies. Recall $\rho^{[j]} = \langle W, \xi^{[j]} \rangle_{\mathbb{H}}$. So $E[\rho^{[j]} \langle W, v \rangle_{\mathbb{H}}] = E[\langle W, \xi^{[j]} \rangle_{\mathbb{H}} \langle W, v \rangle_{\mathbb{H}}]$. This represents the covariance between the component of $W$ in direction $\xi^{[j]}$ and the component of $W$ in direction $v$. Since $\xi^{[j]} \in R(\mathcal{T}^\ast  )$ and $v \perp R(\mathcal{T}^\ast  )$, these components are uncorrelated (assuming zero-mean $W$). Thus, $E[\rho^{[j]} \langle W, v \rangle_{\mathbb{H}}] = 0$. Therefore, $\langle \Sigma_{YW}^{[\ell]}, v \rangle_{\mathbb{H}} = E[Y \langle W, v \rangle_{\mathbb{H}}] \neq 0$ for all $\ell$. This implies that $\Sigma_{YW}^{[\ell]}$ is never the zero vector (or operator) as long as $v \neq 0$. The PLS algorithm, at each step $\ell$, extracts $\xi^{[\ell]}$ as the principal eigenfunction of $\mathcal{U}^{[\ell]}$. If $\Sigma_{YW}^{[\ell]}$ is non-zero, then $\mathcal{U}^{[\ell]}$ has a non-zero largest eigenvalue $\lambda^{[\ell]}$, and $\xi^{[\ell]} = (\langle \Sigma_{YW}^{[\ell]}, \xi^{[\ell]} \rangle_{\mathbb{H}} / \lambda^{[\ell]}) \Sigma_{YW}^{[\ell]}$. This means $\xi^{[\ell]}$ is directly related to $\Sigma_{YW}^{[\ell]}$. Since $\Sigma_{YW}^{[\ell]}$ has a non-zero component orthogonal to all previously extracted $\xi^{[j]}$ (namely, the component in direction $v$), the algorithm *must* continue to extract components that capture this remaining predictive information. This contradicts the assumption that $v$ is orthogonal to *all* $\xi^{[\ell]}$, as the algorithm would continue to extract components along the direction of $v$ or its correlates. The process of PLS is designed to exhaust all such linearly predictive information in $R(\mathcal{T}^\ast  )$. Therefore, the initial assumption that a non-zero $v \in R(\mathcal{T}^\ast  )$ exists orthogonal to all $\xi^{[\ell]}$ leads to a contradiction. Thus, if $v \in R(\mathcal{T}^\ast  )$ and $v \perp \mathcal{S}$, it must imply $v=0$. This establishes that $\mathcal{S}$ is complete in $R(\mathcal{T}^\ast  )$. Combining orthonormality and completeness, $\{\xi^{[l]}\}_{l=1}^\infty$ forms a CONS for $R(\mathcal{T}^\ast  )$.


\subsection{Proof of Proposition \ref{prop:ols_uniqueness}}\label{section:proof:prop:ols_uniqueness}
 \begin{proof} 
 	1. Location of Solutions for $\beta_{\mathrm{OLS}}$
 	The normal equation can be rewritten using the operator $\mathcal{T}$ and its adjoint $\mathcal{T}^*$. Recall that $E[YW] = \mathcal{T}^*Y$ and $E[W \langle \beta, W \rangle_{\mathbb{H}}] = \mathcal{T}^* \mathcal{T} \beta$. Thus, the normal equation becomes: $$ \mathcal{T}^* Y - \mathcal{T}^* \mathcal{T} \beta_{\mathrm{OLS}} = \mathbf{0}_{\mathbb{H}} $$ $$ \mathcal{T}^* \mathcal{T} \beta_{\mathrm{OLS}} = \mathcal{T}^* Y $$ This equation directly shows that $\mathcal{T}^* Y$ must be in the range of $\mathcal{T}^* \mathcal{T}$, i.e., $\mathcal{T}^* Y \in R(\mathcal{T}^* \mathcal{T})$. A fundamental property of operators in Hilbert spaces is that the range of $\mathcal{T}^* \mathcal{T}$ is equal to the range of $\mathcal{T}^*$, i.e., $R(\mathcal{T}^* \mathcal{T}) = R(\mathcal{T}^*)$. Therefore, any solution $\beta_{\mathrm{OLS}}$ to the normal equation implies that $\mathcal{T}^* Y$ must reside in $R(\mathcal{T}^*)$. If $\mathcal{T}^* \mathcal{T}$ is invertible, the unique solution is $\beta_{\mathrm{OLS}} = (\mathcal{T}^* \mathcal{T})^{-1} \mathcal{T}^* Y$. As shown in the proof "The OLS Solution $(\mathcal{T}^* \mathcal{T})^{-1} \mathcal{T}^* Y$ Resides in $R(\mathcal{T}^*)$," this unique solution itself resides in $R(\mathcal{T}^*)$. If $\mathcal{T}^* \mathcal{T}$ is not invertible, its null space (kernel) is non-trivial. In this case, there are infinitely many solutions to the normal equation. However, any solution $\beta_{\mathrm{OLS}}$ still satisfies $\mathcal{T}^* \mathcal{T} \beta_{\mathrm{OLS}} = \mathcal{T}^* Y$. Since the right-hand side $\mathcal{T}^* Y$ is an element of $R(\mathcal{T}^*)$, and $R(\mathcal{T}^* \mathcal{T}) = R(\mathcal{T}^*)$, any solution $\beta_{\mathrm{OLS}}$ must have its "predictive part" (its projection onto $R(\mathcal{T}^*)$) uniquely determined. More precisely, the minimum-norm solution (obtained via the Moore-Penrose pseudoinverse) always lies in $R(\mathcal{T}^*)$, and any other solution differs from it by an element in the null space of $\mathcal{T}^* \mathcal{T}$. 
 	
 	2. Uniqueness of Fitted Values $Y_{\mathrm{OLS}}$
 	 Despite the potential non-uniqueness of $\beta_{\mathrm{OLS}}$, the fitted values $Y_{\mathrm{OLS}}$ are always unique. Let $\beta_1$ and $\beta_2$ be two different solutions to the normal equation, i.e., $\mathcal{T}^* \mathcal{T} \beta_1 = \mathcal{T}^* Y$ and $\mathcal{T}^* \mathcal{T} \beta_2 = \mathcal{T}^* Y$. Subtracting these equations gives: $$ \mathcal{T}^* \mathcal{T} (\beta_1 - \beta_2) = \mathbf{0}_{\mathbb{H}} $$ This means that the difference $(\beta_1 - \beta_2)$ belongs to the null space of $\mathcal{T}^* \mathcal{T}$, i.e., $(\beta_1 - \beta_2) \in \text{Null}(\mathcal{T}^* \mathcal{T})$. A key property in Hilbert spaces is that $\text{Null}(\mathcal{T}^* \mathcal{T}) = \text{Null}(\mathcal{T})$. Therefore, if we let $\delta = \beta_1 - \beta_2$, then $\mathcal{T}\delta = \mathbf{0}_{L^2(\Omega)}$. Now, let's examine the fitted values corresponding to $\beta_1$ and $\beta_2$: $$ Y_{\mathrm{OLS}}(\beta_1) = \langle \beta_1, W \rangle_{\mathbb{H}} = \mathcal{T}\beta_1 $$ $$ Y_{\mathrm{OLS}}(\beta_2) = \langle \beta_2, W \rangle_{\mathbb{H}} = \mathcal{T}\beta_2 $$ Consider their difference: $$ Y_{\mathrm{OLS}}(\beta_1) - Y_{\mathrm{OLS}}(\beta_2) = \mathcal{T}\beta_1 - \mathcal{T}\beta_2 = \mathcal{T}(\beta_1 - \beta_2) = \mathcal{T}\delta $$ Since $\mathcal{T}\delta = \mathbf{0}_{L^2(\Omega)}$, it follows that: $$ Y_{\mathrm{OLS}}(\beta_1) - Y_{\mathrm{OLS}}(\beta_2) = \mathbf{0}_{L^2(\Omega)} $$ Thus, $Y_{\mathrm{OLS}}(\beta_1) = Y_{\mathrm{OLS}}(\beta_2)$, demonstrating that the fitted values are unique even when the regression coefficient $\beta_{\mathrm{OLS}}$ itself is not unique. \end{proof}
		\subsection{Proof of Proposition \ref{proposition:nu_l}}\label{section:proof:proposition:nu_l}



\begin{proof}Since population level least square regression coefficient is precisely
	\begin{equation*}
		\nu^{[l]} = \frac{\mathbb{E}[Y^{[l]} \rho^{[l]}]}{\mathbb{E}[(\rho^{[l]})^2]},
	\end{equation*}
	it suffices to show that
	\begin{equation*}
		\mathbb{E}[Y^{[l]} \rho^{[l]}]
		= 
		\langle \beta_{\mathrm{OLS}}, \xi^{[\ell]} \rangle_{\mathbb{H}} \mathbb{E}[(\rho^{[\ell]})^2].
	\end{equation*} 
	We proceed in two intermediate steps:
	\begin{enumerate}
		\item Showing  
		$
		\beta_{\mathrm{OLS}} = (\mathcal{T}^\ast \mathcal{T})^{-1} \mathcal{T}^\ast Y \in R(\mathcal{T}^\ast)$,
		%
		\item Expanding and transforming $E[Y^{[l]} \rho^{[l]}]  $.
	\end{enumerate}		
	...
	\textit{1. Showing  
		$
		\beta_{\mathrm{OLS}} = (\mathcal{T}^\ast \mathcal{T})^{-1} \mathcal{T}^\ast Y \in R(\mathcal{T}^\ast)$.}~
	The term $\mathcal{T}^\ast Y$ is, by definition, an element of $R(\mathcal{T}^\ast  )$.
	Next, let us consider The operator $\mathcal{T}^\ast   \mathcal{T}: \mathbb{H} \mapsto \mathbb{H}$ is a self-adjoint, positive semi-definite operator.  The inverse operator $(\mathcal{T}^\ast   \mathcal{T})^{-1}$ exists and is well-defined when $\mathcal{T}^\ast   \mathcal{T}$ is invertible.
	Now, since $R(\mathcal{T}^\ast   \mathcal{T}) = R(\mathcal{T}^\ast  )$,
	the operator $\mathcal{T}^\ast   \mathcal{T}$ maps elements from $\mathbb{H}$ specifically into the subspace $R(\mathcal{T}^\ast  )$.
	Consequently, when the inverse operator $(\mathcal{T}^\ast   \mathcal{T})^{-1}$ is applied to an element that is already within $R(\mathcal{T}^\ast  )$ (such as $\mathcal{T}^\ast   Y$), the result will also remain within this same subspace $R(\mathcal{T}^\ast  )$.
	Therefore, the entire expression $(\mathcal{T}^\ast   \mathcal{T})^{-1} \mathcal{T}^\ast   Y$ must   reside in $R(\mathcal{T}^\ast  )$.
	
	\medskip
	\noindent
	\textit{2. Expanding and transforming $E[Y^{[l]} \rho^{[l]}]$.
	}
	We can expand $\mathbb{E}[Y^{[l]} \rho^{[l]}] $ as:
	\begin{equation*}
		\mathbb{E}[Y^{[l]} \rho^{[l]}] 
		\overset{(i)}{=} 
		E
		\bigl[ \bigl\{ ( Y - \sum_{j=1}^{\ell-1} \nu^{[j]} \rho^{[j]} \bigr\} \rho^{[l]} 
		\bigr]
		= \mathbb{E}[Y \rho^{[l]}] - \sum_{j=1}^{\ell-1} \nu^{[j]} \mathbb{E}[\rho^{[j]} \rho^{[l]}]
		\overset{(ii)}{=}
		\mathbb{E}[Y \rho^{[l]}],
	\end{equation*}
	where 
	step $(i)$ repeatedly uses the residualization process for the response and
	step $(ii)$ uses the orthogonality of the PLS scores presented in 
	Corollary \ref{cor:orthogonal_scores}. 
	We further expand:
	\begin{align*}
		\mathbb{E}[Y^{[l]} \rho^{[l]}] 
		=
		\mathbb{E}[Y \rho^{[l]}]
		=
		\mathbb{E}[Y  \langle W, \, \xi^{[l]} \rangle_{\mathbb{H}} ]
		=
		\langle \mathbb{E}[ Y W ], \, \xi^{[l]} \rangle_{\mathbb{H}} 
		%%%%%%%%%%%%%%
		&\overset{(i)}{=}
		\langle 
		\mathbb{E}[W \langle \beta_{\mathrm{OLS}}, W \rangle_{\mathbb{H}}], \, \xi^{[l]} \rangle_{\mathbb{H}} 
		\\&=
		\mathbb{E}[\langle W, \xi^{[\ell]} \rangle_{\mathbb{H}} \langle \beta_{\mathrm{OLS}}, W \rangle_{\mathbb{H}}].
	\end{align*}		
	where step $(i)$ uses the normal equation $	\mathbb{E}[(Y - \langle \beta_{\mathrm{OLS}}, W \rangle_{\mathbb{H}})W] = 0$ presented in  \eqref{normal_equation}.
	
	Since $\beta_{\mathrm{OLS}} \in R(\mathcal{T}^*)$ and 		$\{\xi^{[k]}\}_{k=1}^\infty$ is a  CONS for $R(\mathcal{T}^\ast)$ by Proposition \ref{proposition:CONS}, we can further expand:
	\begin{align*}
		\mathbb{E}[Y^{[l]} \rho^{[l]}] 
		=
		\mathbb{E}[\langle W, \xi^{[\ell]} \rangle_{\mathbb{H}} \langle \beta_{\mathrm{OLS}}, W \rangle_{\mathbb{H}}]
		&=
		\mathbb{E}[\langle W, \xi^{[\ell]} \rangle_{\mathbb{H}} \langle \sum_{k=1}^{\infty} \langle \beta_{\mathrm{OLS}}, \xi^{[k]} \rangle_{\mathbb{H}} \xi^{[k]}, W \rangle_{\mathbb{H}}]
		\\&=
		%%%%%%%%%%%%%%%%%5
		\mathbb{E}
		\bigl[ \rho^{[\ell]} \sum_{k=1}^{\infty} \langle \beta_{\mathrm{OLS}}, \xi^{[k]} \rangle_{\mathbb{H}} \rho^{[k]} \bigr]
		\\& = \langle \beta_{\mathrm{OLS}}, \xi^{[\ell]} \rangle_{\mathbb{H}} \mathbb{E}[(\rho^{[\ell]})^2],
	\end{align*}
	where the last step 
	uses the orthogonality of scores presented in Corollary \ref{cor:orthogonal_scores}.
	This completes the proof of Proof of Proposition \ref{proposition:nu_l}.
\end{proof}

\subsection{Proof of Theorem \ref{thm:hybrid_pls_convergence}}\label{section:proof:thm:hybrid_pls_convergence}
\begin{proof}
By Proposition \ref{proposition:CONS},
	 $\{\xi^{[l]}\}_{l=1}^\infty$ is a CONS for $R(\mathcal{T}^\ast  )$.
	 By Appendix \ref{section:proof:proposition:nu_l}, 
	 we have
	 $\beta_{\mathrm{OLS}}   \in R(\mathcal{T}^\ast)$.
Therefore, we can expand $Y_{\mathrm{OLS}}$ as:
\begin{align*}
 Y_{\mathrm{OLS}}
 = 
 \langle \beta_{\mathrm{OLS}}, W \rangle_{\mathbb{H}}
 =
 \biggl \langle 
 \sum_{l=1}^{\infty} \langle \beta_{\mathrm{OLS}}, \xi^{[l]} \rangle_{\mathbb{H}} \xi^{[l]}, W 
 \biggr \rangle_{\mathbb{H}}
 \hspace{-.5em}
 =
 \sum_{l=1}^{\infty} \langle \beta_{\mathrm{OLS}}, \xi^{[l]} \rangle_{\mathbb{H}} \langle \xi^{[l]}, W \rangle_{\mathbb{H}}
 =
\sum_{l=1}^{\infty} \nu^{[l]} \rho^{[l]},
\end{align*}
where the last equality uses
Corollary \ref{cor:orthogonal_scores} and
Proposition \ref{proposition:nu_l}.

Then  the mean-squared error between the PLS approximation and the ordinary linear regression solution is written as
\begin{align*}
 \mathbb{E}\left[ \| Y_{\mathrm{PLS},L } - Y_{\mathrm{OLS}} \|_2^2 \right] 
 %%%%%%%%%%%%%%%
 =
 \mathbb{E}\left[ 
 \bigl\|
  \sum_{l=1}^{L} \nu^{[l]} \rho^{[l]} - \sum_{l=1}^{\infty} \nu^{[l]} \rho^{[l]} 
 \bigr\|_2^2 \right]
 %%%%%%%%%%%%%%%%%%
 &=
 \mathbb{E}\left[ 
 ...
 %%%%%%%%%%%%%%%%%%%
 \\&=
 \sum_{l=L+1}^{\infty} (\nu^{[l]})^2 \mathbb{E}[(\rho^{[l]})^2],
\end{align*}
where the last equality uses Corollary \ref{cor:orthogonal_scores}.

Now, it suffices to show that $\sum_{l=L+1}^{\infty} (\nu^{[l]})^2 \mathbb{E}[(\rho^{[l]})^2] \to 0$ as $L \to \infty$.
Now, since we assume $\mathbb{E}[Y] = 0$, and $Y_{\mathrm{OLS}}$ is linear in $Y$, we have
\begin{align*}
	\text{Var}(Y_{\mathrm{OLS}}) = \mathbb{E}[(Y_{\mathrm{OLS}})^2] = \mathbb{E}\left[\left(\sum_{l=1}^{\infty} \nu^{[l]} \rho^{[l]}\right)^2\right] 
	= \sum_{l=1}^{\infty} (\nu^{[l]})^2 \mathbb{E}[(\rho^{[l]})^2],
\end{align*}
where the last equality uses Corollary \ref{cor:orthogonal_scores}.

  By Cauchy–Schwarz in $\mathbb{H}$,
\begin{equation*}
\mathbb{E}|Y_{\mathrm{OLS}}|^2 
	=
\mathbb{E}	|\langle \beta_{\text{OLS}}, W\rangle_{\mathbb{H}}|^2 \le \mathbb{E}\|\beta_{\text{OLS}}\|_{\mathbb{H}}^{2}\,\mathbb{E}\|W\|_{\mathbb{H}}^{2} < \infty,
\end{equation*}
$(\nu^{[l]})^2 \mathbb{E}[(\rho^{[l]})^2]$ has a convergent series.
For any convergent series, the sum of its tail (i.e., from $L+1$ to infinity) must approach zero as $L$ tends to infinity This concludes the proof of Theorem \ref{thm:hybrid_pls_convergence}. \end{proof} 
