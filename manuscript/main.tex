...
\end{proposition}
	The proof of Proposition \ref{proposition:eigen_regul} is provided in Appendix  \ref{section:proof:proposition:linear_noregul_system_unregularized}.
	...
	The regression coefficient $\boldsymbol{\eta}$ in model \eqref{eq: Hybrid functional model} can be written in terms of the estimated PLS components and scores. First, note that we can show $\widehat{\rho}_{il} = \langle \mathbf{W}_i^{[l]}, \widehat{\xi}_l \rangle_\mathcal{H} = \langle \mathbf{W}_i, \widehat{\boldsymbol{\xi}_{K+1}}_l \rangle_\mathcal{H}$, where $\widehat{\boldsymbol{\xi}_{K+1}}_l = \widehat{\xi}_l - \sum_{u=1}^{l-1} \langle \widehat{\boldsymbol{\delta}}_u, \widehat{\xi}_l \rangle_{\mathcal{H}} \widehat{\boldsymbol{\xi}_{K+1}}_u$ for $l \ge 2$, with $\widehat{\boldsymbol{\xi}_{K+1}}_1 = \widehat{\xi}_1$. Then, the decomposition of $Y_i$ in \eqref{eq: decomposition} leads to
	\begin{equation*}
		Y_i = \sum \limits_{l=1}^L \widehat{\nu}_l \langle \mathbf{W}_i^{[l]}, \widehat{\xi}_l \rangle_\mathcal{H} +\epsilon_i = \left\langle \mathbf{W}_i, \sum \limits_{l=1}^L\widehat{\nu}_l \widehat{\boldsymbol{\xi}_{K+1}}_l \right\rangle_\mathcal{H}+\epsilon_i,
	\end{equation*}
	which, given the uniqueness of $\boldsymbol{\eta}$, leads to
	\begin{equation*}
		\widehat{\boldsymbol{\eta}} = \sum \limits_{l=1}^L\widehat{\nu}_l \widehat{\boldsymbol{\xi}_{K+1}}_l.
	\end{equation*}
	...
	\section{Proof of Propositions \ref{proposition:eigen_noregul} and \ref{proposition:eigen_regul} }\label{section:proof:proposition:linear_noregul_system_unregularized}
	\begin{proof}
	Let $\xi = (\xi_1, \ldots, \xi_K, \boldsymbol{\xi}_{K+1}) \in \mathcal{H}$. Assume that each functional component $\xi_j \in L^2([0,1])$ lies in the span of the basis functions ${b_1(t), \ldots, b_M(t)}$ and can be written as
	\begin{equation*}
		\xi_j(t) := 
	...
	\begin{align*}
		...
		\limits_{i=1}^n
		y_i
		( \mathbf{Z}_i^\top \boldsymbol{\xi}_{K+1} )
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		\\&=  
...
%	The objective function matches that of \eqref{eq: Regularized generalized rayleigh quotient equation}, takin
	...
	\begin{align*}
		...
		\rangle
		+
		\boldsymbol{\xi}_{K+1}^\top \boldsymbol{\xi}_{K+1}
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		\\&=
...
This computation along with \eqref{covariance_squares_as_quadratic_form} implies that the covariance maximization problem  
 ...
 \begin{proof}
 	...
 	\ref{eq: Regularized generalized rayleigh quotient equation} can be written as
 		\begin{align*}
 		\max_{\boldsymbol{\gamma}_1, \ldots, \boldsymbol{\gamma}_K \in \mathbb{R}^M,
 			\boldsymbol{\xi}_{K+1} \in \mathbb{R}^p
 		}~&
 		\frac{1}{n^2}
 		\biggl(
 		\sum \limits_{k=1}^K 
 		...
 		Z \boldsymbol{\xi}_{K+1} 
 		\biggr)^2,
 		\\\text{subject to}\quad&	\sum \limits_{j=1}^K 
 		\boldsymbol{\gamma}_j^\top
 	(	B + \lambda_k B^{\prime \prime})
 		\boldsymbol{\gamma}_j
 		+
 		\boldsymbol{\xi}_{K+1}^\top \boldsymbol{\xi}_{K+1} = 1.
 	\end{align*} 
 Let $\mathbf{u}_j := \Theta_j^\top \mathbf{y} \in \mathbb{R}^M$ and $\mathbf{v} := Z^\top \mathbf{y} \in \mathbb{R}^p$. These are fixed problem data and not decision variables. Then the optimization problem becomes
 	 		\begin{align*}
 		\max_{\boldsymbol{\gamma}_1, \ldots, \boldsymbol{\gamma}_K \in \mathbb{R}^M,
 			\boldsymbol{\xi}_{K+1} \in \mathbb{R}^p
 		}~&
 	...
 	\mathbf{u}_j^\top
 		...
 		\boldsymbol{\gamma}_j
 		+
 		\mathbf{v}^\top \boldsymbol{\xi}_{K+1} 
 		\biggr)^2,
 		\\\text{subject to}\quad&	\sum \limits_{j=1}^K 
 		\boldsymbol{\gamma}_j^\top
 		(	B + \lambda_k B^{\prime \prime})
 		...
 		\boldsymbol{\xi}_{K+1}^\top \boldsymbol{\xi}_{K+1} = 1.
 	\end{align*}  	
The feasible set is compact, as it corresponds to the boundary of an ellipsoid in a finite-dimensional Euclidean space. Since the objective function is continuous and the constraint defines a compact feasible set, the  Weierstrass Extreme Value Theorem (see, for example, Theorem 2.3.1 in \citealt{bazaraaNonlinearProgrammingTheory2006}) ensures the existence of a global maximizer.

Define the Lagrangian:
\[
\mathcal{L}(\{\boldsymbol{\gamma}_j\}, \boldsymbol{\xi}_{K+1}, \mu) 
:= 
	\left( \sum_{j=1}^K \mathbf{u}_j^\top B \boldsymbol{\gamma}_j + \mathbf{v}^\top \boldsymbol{\xi}_{K+1} \right)^2
- 
\mu 
\biggl[
\bigl\{
	\sum_{j=1}^K \boldsymbol{\gamma}_j^\top (B + \lambda_j B^{\prime\prime}) \boldsymbol{\gamma}_j + 	\boldsymbol{\xi}_{K+1}^\top \boldsymbol{\xi}_{K+1} 
	\bigr\}
	- 1
	\biggr]
.
\]
The constraint function   is continuously differentiable, and its gradient vanishes only at the origin. Since the origin is not feasible, the gradient  is nonzero at all feasible points. Thus, the linear independence constraint qualification (LICQ) holds, ensuring that the KKT conditions are necessary for local maxima (see, for example, Theorem 5.3.1 in \citealt{bazaraaNonlinearProgrammingTheory2006}).
Therefore, for any local maxima, gradients of $\mathcal{L}$ must be zero:
\begin{align*}
	\nabla_{\boldsymbol{\gamma}_j} \mathcal{L} &= 
	\frac{2}{n^2} 
	\left(
	\sum_{\ell=1}^K \mathbf{y}^\top \Theta_\ell B \boldsymbol{\gamma}_\ell + \mathbf{y}^\top Z \boldsymbol{\xi}_{K+1}
	\right)
	B \Theta_j^\top \mathbf{y}
	- 2\lambda (B + \lambda_j B^{\prime \prime}) \boldsymbol{\gamma}_j = 0, \quad \forall j, 
	\numberthis \label{lag_grad_d}
	\\
	\nabla_{\boldsymbol{\xi}_{K+1}} \mathcal{L} &= 
	\frac{2}{n^2} 
	\left(
	\sum_{\ell=1}^K \mathbf{y}^\top \Theta_\ell B \boldsymbol{\gamma}_\ell + \mathbf{y}^\top Z \boldsymbol{\xi}_{K+1}
	\right)
	Z^\top \mathbf{y}
	- 2\lambda \boldsymbol{\xi}_{K+1} = 0.
	\numberthis \label{lag_grad_zeta}
\end{align*}
By pre-multiplying equation \eqref{lag_grad_d} by $B^{-1}$,  and observing that  $2n^{-2}\sum_{\ell=1}^K \mathbf{y}^\top \Theta_\ell B \boldsymbol{\gamma}_\ell + \mathbf{y}^\top Z \boldsymbol{\xi}_{K+1}$, which is a scalar quantity, appears in both \eqref{lag_grad_d} and \eqref{lag_grad_zeta}, the  conditions above together imply that any local maxima must be  scalar multiples of
$
...
\begin{equation*}
	...
	\quad
	%%%%%%%%%%%%%%%%
	\boldsymbol{\xi}_{K+1}^\ast = \frac{1}{\sqrt{\sum_{j=1}^K 
			\mathbf{y}^\top
			\Theta_j
			B \Theta_j^\top \mathbf{y} 
			+ 
...
\end{equation*}
 	...
 	Observe that the objective is linear in each $\boldsymbol{\gamma}_j$ and in $\boldsymbol{\xi}_{K+1}$, and the constraint is a strictly convex quadratic form. Hence, the optimal solution lies in the direction of the gradient of the linear objective.
 	
 	Define scalar weights $c_j$ and $c$ such that
 	\[
 	\boldsymbol{\gamma}_j = c_j \mathbf{u}_j, \quad \boldsymbol{\xi}_{K+1} = c \mathbf{v}.
 	\]
 	
 	Substituting into the objective:
 	\[
 	\left( \sum_{j=1}^K c_j \mathbf{y}^\top \Theta_j B \mathbf{u}_j + c \mathbf{y}^\top Z \mathbf{v} \right)^2 = \left( \sum_{j=1}^K c_j a_j + c b \right)^2,
 	\]
 	where we define $a_j := \mathbf{u}_j^\top B \mathbf{u}_j$ and $b := \| \mathbf{v} \|^2$.
 	
 	The constraint becomes:
 	\[
 	\sum_{j=1}^K c_j^2 \, d_j + c^2 b = 1,
 	\]
 	where $d_j := \mathbf{u}_j^\top (B + \mu_j B^{\prime\prime}) \mathbf{u}_j$.
 	
 	Now we define the Lagrangian:
 	\[
 	\mathcal{L}(c_1, \ldots, c_K, c; \lambda) = \left( \sum_{j=1}^K c_j a_j + c b \right)^2 - \lambda \left( \sum_{j=1}^K c_j^2 d_j + c^2 b - 1 \right).
 	\]
 	
 	Taking partial derivatives and setting them to zero:
 	\begin{align*}
 		\frac{\partial \mathcal{L}}{\partial c_j} &= 2 \left( \sum_{j'=1}^K c_{j'} a_{j'} + c b \right) a_j - 2\lambda c_j d_j = 0, \quad \forall j, \\
 		\frac{\partial \mathcal{L}}{\partial c} &= 2 \left( \sum_{j'=1}^K c_{j'} a_{j'} + c b \right) b - 2\lambda c b = 0.
 	\end{align*}
 	
 	Let $s := \sum_{j'=1}^K c_{j'} a_{j'} + c b$, then the optimality conditions reduce to:
 	\[
 	s a_j = \lambda c_j d_j, \quad s b = \lambda c b.
 	\]
 	
 	Solving for $c_j$ and $c$:
 	\[
 	c_j = \frac{a_j}{d_j} \cdot \frac{s}{\lambda}, \quad c = \frac{s}{\lambda}.
 	\]
 	
 	Substitute into the constraint:
 	\begin{align*}
 		\sum_{j=1}^K \left( \frac{a_j}{d_j} \cdot \frac{s}{\lambda} \right)^2 d_j + \left( \frac{s}{\lambda} \right)^2 b &= 1, \\
 		\left( \frac{s}{\lambda} \right)^2 \left( \sum_{j=1}^K \frac{a_j^2}{d_j} + b \right) &= 1.
 	\end{align*}
 	
 	Let $S := \sum_{j=1}^K \frac{a_j^2}{d_j} + b$, then
 	\[
 	\left( \frac{s}{\lambda} \right)^2 = \frac{1}{S}, \quad \Rightarrow \quad \frac{s}{\lambda} = \frac{1}{\sqrt{S}}.
 	\]
 	
 	Hence,
 	\[
 	c_j = \frac{a_j}{d_j \sqrt{S}}, \quad c = \frac{1}{\sqrt{S}}.
 	\]
 	
 	\textbf{Conclusion}: The optimal coefficients are
 	\[
 	\boldsymbol{\gamma}_j = \frac{a_j}{d_j \sqrt{S}} \mathbf{u}_j, \quad \boldsymbol{\xi}_{K+1} = \frac{1}{\sqrt{S}} \mathbf{v},
 	\]
 	where
 	\[
 	a_j = \mathbf{u}_j^\top B \mathbf{u}_j, \quad d_j = \mathbf{u}_j^\top (B + \mu_j B^{\prime\prime}) \mathbf{u}_j, \quad \mathbf{u}_j = \Theta_j^\top \mathbf{y}, \quad \mathbf{v} = Z^\top \mathbf{y},
 	\]
 	and
 	\[
 	S = \sum_{j=1}^K \frac{a_j^2}{d_j} + \| \mathbf{v} \|^2.
 	\]
