...
For $k = 1, \ldots, K$, we project the $k$th functional predictor onto 
$\operatorname{span}(\Phi^{(k)})$:
\begin{equation*}
	X_i^{(k)} \approx \sum_{m=1}^M c_{im}^{(k)} \phi_m^{(k)}, \quad i = 1, \ldots, n,
\end{equation*}
...
\begin{equation*}
		\widetilde{\mathbf{C}}^{(k)} := ( c_{im}^{(k)} ).
\end{equation*}

Importantly, the core computation of our algorithm involves the inner product between $n$ hybrid predictor observations and a single hybrid object (the PLS direction), defined via the inner product in Definition \ref{def:hilbert_space}.
 With the basis expansion, this reduces to simple matrix operations. 
For each $k = 1, \ldots, K$, let $\boldsymbol{\Phi}^{(k)} := (\phi^{(k)}_{m \ell })$
denote the	$M \times M$ symmetric matrix with entries  the inner products between basis functions 
\begin{equation}\label{def:gram_basis}
\phi^{(k)}_{m \ell } 
:= 
\langle \phi^{(k)}_m,  \phi^{(k)}_\ell \rangle
=  
\int_{\mathcal{T}_k} \phi^{(k)}_m(t_k) \, \phi^{(k)}_\ell(t_k) \, dt_k.
\end{equation}

\begin{definition}[Hybrid inner product between $n$ observations and a single object]\label{def:n_one_inner_product}
Let $\boldsymbol{\xi} = (\psi, \mathbf{s}) \in \mathcal{H}$ be a hybrid object with $\psi \in \mathcal{F}$ and $\mathbf{s} \in \mathbb{R}^p$, and let $\mathbf{a}^{(1)}, \ldots, \mathbf{a}^{(K)}$ denote the basis coefficients of $\psi$ with respect to basis $\Phi^{(1)}, \ldots, \Phi^{(K)}$. Then the inner product between $\boldsymbol{\xi}$ and the $n$ basis-approximated hybrid observations $\widetilde{\mathbf{W}}_1, \ldots, \widetilde{\mathbf{W}}_n$, concatenated as a column vector, is denoted $\langle \widetilde{\mathbf{W}}, \boldsymbol{\xi} \rangle_{\mathcal{H}} \in \mathbb{R}^n$ and computed as
	\begin{equation*}
		\langle \widetilde{\mathbf{W}}, \boldsymbol{\xi} \rangle_{\mathcal{H}}
		:=
		\begin{bmatrix}
			\langle \widetilde{\mathbf{W}}_1, \boldsymbol{\xi} \rangle_{\mathcal{H}} \\
			\vdots \\
			\langle \widetilde{\mathbf{W}}_n, \boldsymbol{\xi} \rangle_{\mathcal{H}}
		\end{bmatrix}
		=
		\mathbf{Z} \mathbf{s}
		+
		\sum_{k=1}^K \widetilde{\mathbf{C}}^{(k)} \boldsymbol{\Phi}^{(k)} \mathbf{a}^{(k)}. 
	\end{equation*}
\end{definition}
...
We now describe the strategy for computing the PLS direction at the $l$-th iteration. For simplicity, we omit the iteration index and denote the residualized response by $Y$, with finite-sample observations given by $\mathbf{y} = (y_1, \ldots, y_n) \in \mathbb{R}^n$. Let $\widetilde{W} \in \mathcal{H}$ denote the residualized, basis-approximated hybrid predictor, and let $\widetilde{\mathbf{C}}^{(1)}, \ldots, \widetilde{\mathbf{C}}^{(k)} \in \mathbb{R}^{n \times M}$ denote the basis coefficient matrices of its functional part in the finite-sample observations. Our strategy seeks the direction $\boldsymbol{\xi} \in \mathcal{H}$, whose functional parts lying in 
$\operatorname{span}(\Phi^{(1)}), \ldots, \operatorname{span}(\Phi^{(K)})$
, that maximizes the empirical covariance between the resulting PLS component $\langle \widetilde{W}, \boldsymbol{\xi} \rangle_{\mathcal{H}}$ and the response.

For the computation of the estimated PLS direction  $\hat{\boldsymbol{\xi}}$, 
we obtain the basis coefficients of its functional part, denoted by $\mathbf{d}^{(1)}, \ldots, \mathbf{d}^{(K)} \in \mathbb{R}^{M}$,
and the scalar part  
...
\mathbf{d}^{(1)} 
=
(\widetilde{\mathbf{C}}^{(1)} \boldsymbol{\Phi}^{(1)})^\top
\mathbf{y},
\ldots
,
\boldsymbol{\Phi}^{(K)}
\mathbf{d}^{(K)} 
=
(\widetilde{\mathbf{C}}^{(K)} \boldsymbol{\Phi}^{(K)})^\top
\mathbf{y},
~\text{and}~
\mathbf{d} := \mathbf{Z}^\top \mathbf{y} .
\end{equation*}
We then normalize these coefficients and scalar parts by the hybrid Hilbert norm (Definition
\ref{def:hilbert_space}),
which can be computed as
\begin{equation*}
\|	\hat{\boldsymbol{\xi}} \|_{\mathcal{H}} = 
\bigl(
	\mathbf{d}^\top \mathbf{d} + 
	\sum_{k=1}^K (\mathbf{d}^{(k)})^\top  \boldsymbol{\Phi}^{(K)}  \mathbf{d}^{(k)} 
	\bigr)^{1/2}.
\end{equation*}
Therefore, the estimated PLS direction is the tuple $\hat{\boldsymbol{\xi}} \in \mathcal{H}$, defined as
\begin{equation}
	\hat{\boldsymbol{\xi}} :=
	\frac{1}{
	\bigl(
	...
	\biggl(
	\sum_{m=1}^M d^{(1)}_m \phi^{(1)}_m,
	\ldots,
	\sum_{m=1}^M d^{(K)}_m \phi^{(K)}_m,
	\mathbf{d}
	\biggr).
\end{equation}
Finally, the estimated $l$th PLS scores for the $n$ observations,
denoted $\hat{\boldsymbol{\rho}} \in \mathbb{R}^n$
 is computed simultaneously as the hybrid inner product (Definition \ref{def:n_one_inner_product}):
	\begin{equation*}
	\langle \widetilde{\mathbf{W}}, \hat{\boldsymbol{\xi}} \rangle_{\mathcal{H}}
	=
	\frac{1}{\|	\hat{\boldsymbol{\xi}} \|_{\mathcal{H}}}
	\biggl(
	\mathbf{Z} \mathbf{d}
	+
	\sum_{k=1}^K \widetilde{\mathbf{C}}^{(k)} \boldsymbol{\Phi}^{(k)} \mathbf{d}^{(k)}
	\biggr)
	. 
\end{equation*}
...
The proof of Proposition \ref{proposition: orthnormality of PLS scores} is provided in Appendix \ref{section:proof:proposition: orthnormality of PLS scores}.
 ...
 This notation can be further extended to simultaneously express the basis expansions of $K$ functional predictors as 
	...
	\begin{align*}
		...
		&= n^{-1} \widetilde{\mathbf{y}}^\top \left[ \sum \limits_{k=1}^K \widetilde{\mathbf{C}}^{(k)} \left\{ \int_{\mathcal{T}_k} \mathbf{b}^{(k)}(t_k) \mathbf{b}^{(k)}(t_k)^\top dt_k \right\} \mathbf{a}^{(k)} \right] + n^{-1}\widetilde{\mathbf{y}}^\top \mathbf{Z} \boldsymbol{\theta} \\
