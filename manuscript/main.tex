...
Given a twice-differentiable basis $\{b_m(t)\}$ of $L^2([0,1])$,  the $j$th functional predictor, regression coefficient, and PLS direction (suppressing iteration indices) are represented as
	\begin{equation*}
		...
		b_m(t)
	\end{equation*}	
In practice, the full set of coefficients can not be obtained, as functional data are measured on a finite grid. Thus, we truncate the expansion at $M$ terms and define the truncated basis space
\begin{equation}
...
We choose a moderately large $M$ (e.g., 15 or 20) to capture functional variation without fine-tuning, as smoothness is handled via penalization (see Section~\ref{sec: Regularization of PLS Components}).
	...
	\end{equation*}
The $i$th hybrid predictor, with its functional parts projected onto the $\mathcal{B}$, is represented by the tuple
	\begin{equation*}
		\widetilde{W}_i := 
		(
