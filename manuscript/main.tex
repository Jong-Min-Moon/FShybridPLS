...
:= \arg \max_{\xi \in (\mathcal{B}^K \times \mathbb{R}^p) \subset \mathcal{H}} \widehat{\textnormal{Cov}}^2 
\left( 
...
$( \hat{\mathbf{d}}_1^\top, \ldots, \hat{\mathbf{d}}_K^\top, \hat{\boldsymbol{\zeta}}^\top )^\top$
...
\begin{equation}	\label{eq: Regularized generalized rayleigh quotient equation}
	\left(
	\hat{\mathbf{d}}_1^\top, \ldots, \hat{\mathbf{d}}_K^\top, \hat{\boldsymbol{\zeta}}^\top
	\right)^\top
	=
...
Since $V$ is a rank 1 matrix, the solution of the generalized eigenvalue problem is easily obtained by matrix multiplication and normalization:
	...
	We can compute  $\hat{\mathbf{d}}_1, \ldots, \hat{\mathbf{d}}_K$ and $\hat{\boldsymbol{\zeta}}$ in Proposition \ref{proposition:eigen_noregul} as 
\begin{equation*}
	...
	\quad
	%%%%%%%%%%%%%%%%
	\hat{\boldsymbol{\zeta}}  = \frac{1}{\sqrt{\sum_{j=1}^K 
			\mathbf{y}^\top
			\Theta_j
			B \Theta_j^\top \mathbf{y} 
			+ 
...
Although the unnormalized coefficients are computed separately for the functional and scalar part of the PLS component direction, the normalization step couples them, allowing the components to influence each other. This enables the procedure to account for the correlation between functional and scalar components.
	...
	Although Proposition \ref{proposition:eigen_noregul} provides an efficient and principled way to compute the PLS components $\xi$, their functional part $\psi=(\psi^{(1)}, \ldots, \psi^{(K)})$ may not be smooth, which can lead to overfitting and unstable predictions. Thus, in this section, we provide a modification of Proposition  \ref{proposition:eigen_noregul} that yields regularized (smoothed) PLS components.
...
Therefore, the optimization problem \eqref{optim:summation_form}, which is equivalent to the squared covariance maximization problem, reduces to the generalized eigenvalue problem \eqref{eq: Regularized generalized rayleigh quotient equation}. This completes the proof of Proposition~\ref{proposition:eigen_noregul}.
 ...
 \begin{proof}
	...
	\begin{equation*}
		...
		Z^\top \mathbf{y},
	\end{equation*}
	which are the same as  $\hat{\mathbf{d}}_1, \ldots, \hat{\mathbf{d}}_K$ and $\hat{\boldsymbol{\zeta}}$. This completes the proof of  Proposition \ref{proposition:linear_noregul}.
	\end{proof}
	...
	\subsection{Proof of Propositions \ref{proposition:eigen_noregul} and \ref{Proposition: Regularized generalized rayleigh quotient} }\label{section:proof:eigenproblem_derivation}
	%
	%
	\begin{align*}
		\widehat{\textnormal{Cov}} (
	...
	\begin{equation}\label{def:eigenproblem_smoothed}
		\arg \max \limits_{\xi^* \in \mathbb{R}^{MK+p}} \; \frac{\xi^{*\top} \mathbf{V}^\ast \xi^*}{\xi^{*\top} J^\ast \xi^*}, \quad \textit{or equivalently}, \quad \arg \max \limits_{\xi^* \in \mathbb{R}^{MK+p}} \;  \xi^{*\top} \mathbf{V}^\ast \xi^* \quad \textnormal{subject to}  \quad \xi^{*\top} J^\ast \xi^*=1,
	\end{equation}
