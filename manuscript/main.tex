...
\end{equation}
utilizing the estimated PLS component direction $\widehat{\xi}^{[l]}$ obtained from Propositions \ref{proposition:eigen_regul} and \ref{proposition:linear_regul}. 
...
The following proposition shows that the solution of the problem \ref{def:argmin_pensse} can be obtained by solving linear equations.
 ...
 \begin{lemma}[Closed-form  solution]\label{proposition:closed_form_orthgonalization}
 	...
 	\quad\text{and} \quad
 			\bar{\rho}^{[l]} := (\hat{\boldsymbol{\rho}}^{[l]})^\top \hat{\boldsymbol{\rho}}^{[l]}.
 	\end{equation*}
 	The solution to the optimization problem \eqref{def:argmin_pensse}, denoted
 	$\hat{\delta}^{[l]}   \in \tilde{\mathcal{H}}$, has basis coefficients and scalar parts given by:
\begin{equation*}
  ...
  B
	...
	\hat{\boldsymbol{\chi} }
	=
	(\mathbf{Z}^{[l]})^\top  \hat{\boldsymbol{\rho}}.
\end{equation*}
 \end{lemma}
...
\section{Proof of Proposition \ref{proposition:closed_form_orthgonalization} }\label{section:proof:proposition:closed_form_orthgonalization}
...
\tau
 B^{\prime \prime}.
$ 
Since we assumed in Section \ref{sec: finite basis approximation} that both $B'$ and $B''$ are positive definite, $\operatorname{PENSSE}( \delta)$ is convex. convex. Consequently, the gradient vanishes at its unique minimizer:
\begin{align*}
...
\end{equation*}
This completes the proof of Proposition \ref{proposition:closed_form_orthgonalization}.
