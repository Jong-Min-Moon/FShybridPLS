...
\]
We compute $\hat{\mathbf{d}}_1, \ldots, \hat{\mathbf{d}}_K$ and $\hat{\boldsymbol{\zeta}}$ as follows.
First, we obtain the unnormalized coefficient vectors. For each $j$, the unnormalized vector $\hat{\mathbf{d}}_j'$ is computed by solving a linear equation:
\begin{equation*}
	B \hat{\mathbf{d}}_j' = (\Theta_j B)^\top \mathbf{y},
\end{equation*}
The unnormalized scalar component is given by
\begin{equation}
	\hat{ \boldsymbol{\zeta} }' := Z^\top \mathbf{y}.
\end{equation}
...
\begin{equation}\label{def:computed_PLS_direction}
	...
	\frac{1}{
		\left(
		\sum_{j=1}^K \hat{\mathbf{d}}_j'^\top B \hat{\mathbf{d}}_j' +
		(\hat{ \boldsymbol{\zeta} }')^\top \hat{ \boldsymbol{\zeta} }'
		\right)^{1/2}
	} \hat{\mathbf{d}}_j', \quad j = 1, \ldots, K,
\end{equation}
and
\begin{equation}
	\hat{\boldsymbol{\zeta}} :=
...
For any $\boldsymbol{\xi} = (\xi_1, \ldots, \xi_K, \boldsymbol{\zeta}) \in \mathcal{H}$, where each functional component $\xi_j \in L^2([0,1])$ lies in the span of the basis functions $\{b_1(t), \ldots, b_M(t)\}$ and is expressed as
\begin{equation*}
	\xi_j(t) := 
	\sum_{m=1}^M d_{jm} \, b_m(t), \quad j = 1, \ldots, K,
\end{equation*}
with coefficient vectors $\mathbf{d}_j := (d_{j1}, \ldots, d_{jM})^\top \in \mathbb{R}^M$, the squared norm of $\boldsymbol{\xi}$ in the hybrid Hilbert space $\mathcal{H}$ is given by
...
\begin{align*}
	...
	\biggr).
\end{align*}	
Both expressions in \eqref{...} and \eqref{...} can be represented as inner products in the product space $(\mathbb{R}^M)^K \times \mathbb{R}^p$, which is defined as follows:
...
By the definition of the inner product on $\mathcal{H} = \mathbb{R}^{KM} \times \mathbb{R}^p$, we have
\[
\langle \boldsymbol{\xi}, \boldsymbol{\xi} \rangle_{\mathcal{H}} = \sum_{j=1}^K \langle \xi_j, \xi_j \rangle_{L^2([0,1])} + \|\boldsymbol{\zeta}\|^2.
\]
Since $\xi_j(t) = \sum_{m=1}^M d_{jm} b_m(t)$, it follows that
\[
\langle \xi_j, \xi_j \rangle_{L^2([0,1])} 
= \int_0^1 \left( \sum_{m=1}^M d_{jm} b_m(t) \right)^2 dt 
= \mathbf{d}_j^\top B \mathbf{d}_j,
\]
where $B$ is the Gram matrix of the basis. Substituting this into the definition yields
\[
\langle \boldsymbol{\xi}, \boldsymbol{\xi} \rangle_{\mathcal{H}} = \sum_{j=1}^K \mathbf{d}_j^\top B \mathbf{d}_j + \boldsymbol{\zeta}^\top \boldsymbol{\zeta}.
\]
