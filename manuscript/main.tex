...
For $k = 1, \ldots, K$, 
we assume that the functional predictors $X_{ik}(t)$ are observed on the domain $0 \leq t \leq 1$.and belong to $L^2([0,1])$, the Hilbert space of square-integrable real-valued functions with respect to the Lebesgue measure $dt$ on $[0,1]$.
...
\begin{equation}\label{def:hybid_predictor}
		...
		\in \mathcal{H} :=  L^2([0,1])^{\otimes K} \times \mathbb{R}^p
\end{equation}
