...
\end{equation*}
Without loss of generality, we show that the $i$th entry of $\Theta^{[l_1] }
\mathbb{B}
\hat{\boldsymbol{\xi}}_{l_2}$ is 0.
From the construction in \eqref{def:full_theta}, \eqref{def:theta} and \eqref{def:gram_block}, the $i$-th entry is
\begin{align*}
	(\Theta^{[l_1] }
	\mathbb{B}
...
\rangle_{\mathcal{H}},
\end{align*}
where the last equality uses the computation of the $i$th entry in \eqref{score_inner_product_all_n}.
 
We can express the $l_1$-th residualized hybrid predictor in terms of its previous $(l_1-1)$-th version:
\begin{equation*}
