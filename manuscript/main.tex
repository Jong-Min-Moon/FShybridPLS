...
Let $\hat{\mathbf{d}}_j := (\hat{d}_{j1}, \ldots, \hat{d}_{jM})^\top \in \mathbb{R}^M$ and $\mathbf{y} := (y_1, \ldots, y_n)^\top$. 
...
\]
We compute $\hat{\mathbf{d}}_1, \ldots, \hat{\mathbf{d}}_K$ and $\hat{\boldsymbol{\zeta}}$ as 
\begin{equation*}
	...
	Z^\top \mathbf{y}.
\end{equation*}
The derivation of this formula is provided in Appendix~\ref{section:proof:eigenproblem_derivation}.
...
\begin{proposition} \label{Proposition:linear_system_unregularized}
aaa
\end{proposition}
Proof of is provided in Appendix \ref{section:proof:Proposition:linear_system_unregularized}.
...
\subsection{Proof of Proposition \ref{Proposition:linear_system_unregularized}}\label{section:proof:Proposition:linear_system_unregularized}
\begin{comment}
...
presented in Proposition \ref{Proposition:linear_system_unregularized} is equivalent to the following constrained optimization problem on the Euclidean space:
\begin{align*}
...
\begin{equation*}
	\mathbf{d}_j^\ast = \frac{1}{\sqrt{\sum_{j=1}^K 
		 ...
		 Z^\top \mathbf{y},
\end{equation*}
which are the same as  $\hat{\mathbf{d}}_1, \ldots, \hat{\mathbf{d}}_K$ and $\hat{\boldsymbol{\zeta}}$. This completes the proof of  Proposition \ref{Proposition:linear_system_unregularized}.


\subsection{Proof of Lemma \ref{lemma:valid_inner_product_tuple}}\label{section:proof:lemma:valid_inner_product_tuple}
\begin{proof}
	We verify that the function
	\[
	\langle u, v \rangle_{B} := \sum_{j=1}^K \mathbf{d}_j^\top B \mathbf{g}_j + \boldsymbol{\zeta}^\top \boldsymbol{\omega},
	\]
	defines a valid inner product on $ (\mathbb{R}^M)^K \times \mathbb{R}^p$, where $u = (\mathbf{d}_1, \ldots, \mathbf{d}_K, \boldsymbol{\zeta})$ and $v = (\mathbf{g}_1, \ldots, \mathbf{g}_K, \boldsymbol{\omega})$ are elements of $(\mathbb{R}^M)^K \times \mathbb{R}^p$.
	\paragraph{Symmetry}: Since $B$ is symmetric and the Euclidean inner product is symmetric,
	\[
	\langle u, v \rangle_{B} = \sum_{j=1}^K \mathbf{d}_j^\top B \mathbf{g}_j + \boldsymbol{\zeta}^\top \boldsymbol{\omega} = \sum_{j=1}^K \mathbf{g}_j^\top B \mathbf{d}_j + \boldsymbol{\omega}^\top \boldsymbol{\zeta} = \langle v, u \rangle_{B}.
	\]
	\paragraph{Linearity in the first argument}: Let $u^{(1)} = (\mathbf{d}_1^{(1)}, \ldots, \mathbf{d}_K^{(1)}, \boldsymbol{\zeta}^{(1)})$ 
	and
	$v = (\mathbf{g}_1, \ldots, \mathbf{g}_K, \boldsymbol{\omega}) $
	be the elements of $(\mathbb{R}^M)^K \times \mathbb{R}^p$.
	Let $a, b \in \mathbb{R}$. Then we have
	\begin{align*}
		\langle a u^{(1)} + b u^{(2)}, v \rangle_{B}
		&= \sum_{j=1}^K (a \mathbf{d}_j^{(1)} + b \mathbf{d}_j^{(2)})^\top B \mathbf{g}_j + (a \boldsymbol{\zeta}^{(1)} + b \boldsymbol{\zeta}^{(2)})^\top \boldsymbol{\omega} \\
		&= 
		a  \sum_{j=1}^K (\mathbf{d}_j^{(1)})^\top B \mathbf{g}_j 
		+
		b \sum_{j=1}^K  (\mathbf{d}_j^{(2)})^\top B \mathbf{g}_j
		+
		a ( \boldsymbol{\zeta}^{(1)})^\top \boldsymbol{\omega}
		+
		b ( \boldsymbol{\zeta}^{(2)})^\top \boldsymbol{\omega}
		\\
		&= 
		a  
		\biggl(
		\sum_{j=1}^K (\mathbf{d}_j^{(1)})^\top B \mathbf{g}_j 
		+
		( \boldsymbol{\zeta}^{(1)})^\top \boldsymbol{\omega}
		\biggr)
		+
		b 
		\biggl( 
		\sum_{j=1}^K  (\mathbf{d}_j^{(2)})^\top B \mathbf{g}_j
		+
		( \boldsymbol{\zeta}^{(2)})^\top \boldsymbol{\omega}
		\biggr)
		\\
		&= a \langle u^{(1)}, v \rangle_{B} + b \langle u^{(2)}, v \rangle_{B}.
	\end{align*}
	
	
	
	\paragraph{Positive definiteness.}: we have
	$
	\langle u, u \rangle_{B} = \sum_{j=1}^K \mathbf{d}_j^\top B \mathbf{d}_j + \boldsymbol{\zeta}^\top \boldsymbol{\zeta}
	$
	Since $B$ is positive semi-definite, each term $\mathbf{d}_j^\top B \mathbf{d}_j \geq 0$, and $\boldsymbol{\zeta}^\top \boldsymbol{\zeta} \geq 0$, with equality if and only if $\mathbf{d}_j = 0$ for all $j$ and $\boldsymbol{\zeta} = 0$. This completes the proof of Lemma \ref{lemma:valid_inner_product_tuple}.
\end{proof}
